...
- W1-T18 SHIPPED the general mechanism: every worker's `HOME` is redirected to a Remudero-controlled scratch dir (`config.workerHomeDir`, `<root>/worker-home`) holding ONLY empty rc files Remudero itself writes on every spawn, with `.claude`/`.config/gh`/`.gitconfig` symlinked back individually (never a wholesale HOME copy) — so isolation no longer depends on the operator's real `~/.bashrc` being absent, only on Remudero owning the path HOME points at. WS-0 FIELD FINDING 11c already established the Keychain OAuth token resolves off `USER`, independent of this redirection. Whether Claude Code ALSO needs `~/.claude/` state under the REAL (non-redirected) HOME to authenticate is UNVERIFIED LIVE — no subscription OAuth in CI (Standing rule 18/20) — so the `.claude` symlink-back is a defensive default, not an empirically-confirmed requirement yet; the live drill (planted `~/.bashrc` + still authenticates + `gh` still works) is W1-T12e. FALLBACK if the live drill finds the symlink-back insufficient: drop HOME-redirection for that one path and fall back to the W1-T17 preflight-probe-only posture (fail closed on contamination) while keeping the redirection for isolation itself. [W1-T18, this PR]
...
- A **conditionally-skipped required check DEADLOCKS merge forever** (a `paths:`/`if:`-gated job that doesn't run reads as "expected, pending", never green; a skipped REUSABLE-workflow caller creates no check run at all under that nested context name, deadlocking it identically). The fix is ONE always-runs **CI-gate aggregator** job that make only the aggregator the required context — but a `needs:`-based aggregator only sees jobs WITHIN ITS OWN workflow file, so once required checks live in separate workflows (security/quality/architecture tiers), the aggregator must instead READ THE PR HEAD COMMIT'S CHECK-RUNS across all workflows via `gh api` (`checks: read`) and fail only on a real failure conclusion (W1-T24, corrected fleet pattern verified on craigoley/synthwatch). [operator fleet: SynthWatch/neon-drift/OleyArcade]
...
## SDK result-envelope field locations (the W1-T6 exploration tax)

- The SDK `type:"result"` envelope carries the telemetry a ledger task must log — verified from the installed `sdk.d.ts` (SDK 0.3.209): `num_turns`, `total_cost_usd`, `usage` (NonNullableUsage aggregate), and `modelUsage: Record<MODEL_NAME, ModelUsage>` (the MODEL(S) used are the map KEYS; each `ModelUsage` has `inputTokens`, `outputTokens`, `cacheReadInputTokens`, `cacheCreationInputTokens`, `costUSD`, `contextWindow`). **EFFORT is NOT in the envelope** — it is an INPUT (`Options.effort`/the mount), so log the configured effort. W1-T6 spent ~33 of 61 turns re-discovering this; it is now an injectable learning tagged to worker.ts/ledger.ts/run-task.ts, and W1-T6 carries a `files:` field so the matcher selects it (before, repo-wide, it lost the budget tie and was DROPPED). [PR #42/W1-T6, DIAGNOSIS.md]

## Headless-worker fitness & tier detection (W1-T9)

- ★ A criterion requiring LIVE OPERATOR confirmation ("operator confirms", "user selects") is STRUCTURALLY UNFIT for a headless worker — there is no TTY and no operator. W1-T9 burned its last ~15 turns writing readline-reproduction scripts (`rl-repro.mjs`) trying to conjure an operator that isn't there, then died `error_max_turns` — it would overrun at ANY budget. Redesign interactive steps for no-TTY: expose `--tier`/`--yes`-style FLAGS + a TTY-ABSENT SAFE DEFAULT (log the chosen value), prompt ONLY when `process.stdin.isTTY`, and test the non-interactive path via injected input/flags — never a live human. A new error class, distinct from over-scoping and looping. [DIAGNOSIS.md diag/w1t9-max-turns, W1-T9] [subsystem: cli · files: src/lib/init.ts, src/run-task.ts, bin/rmd]
- `~/.claude.json` exposes tier-relevant KEYS locally with no network call — `hasAvailableSubscription`, `cachedUsageUtilization`, `modelAccessCache`, `oauthAccount` (tier discovery rung 2); the `/usage` output (rung 1) is parsed by `headroom.ts` (W1-T4) and is machine-readable headless. Feed BOTH as pure INPUTS to `detectTier` so detection is unit-testable over fixtures (max5x/max20x/ambiguous) with no live calls — the weak-residue learning the diagnosis flagged, so tier detection isn't first-time-discovered again. [DIAGNOSIS.md diag/w1t9-max-turns, MASTER-PLAN §9(h)] [subsystem: platform · files: src/lib/tier.ts, src/lib/init.ts, src/lib/headroom.ts]

## The daemon split & forward-only rule enforcement (W1-T12)

- The daemon task (W1-T12) bundled THREE concerns (scheduler loop + launchd unit + crash-recovery) AND three HEADLESS-UNFIT criteria — a live OVERNIGHT drain, a `launchctl`-LOAD into a real session, and a live KILL-and-recover — then died `error_max_turns` at 81 turns / $10.27 (the 4th such event). A headless worker cannot verify launchd/boot/kill behavior; those are `verify:human` COMMISSIONING steps. Split the daemon into headless unit-tested LOGIC (scheduler loop over an injected clock+runner; plist GENERATION + in-process boot assertion; recovery over a SEEDED interrupted state) and a separate live-commissioning task — the machine builds + unit-proves, the human commissions live (draft→playtest→ready). [DIAGNOSIS.md diag/w1t12-max-turns, W1-T12] [subsystem: architecture · files: src/lib/daemon.ts, src/lib/launchd.ts, src/run-task.ts]
- Standing rules 18/19 were enforced FORWARD-ONLY: W1-T12 pre-existed them and violated both, yet slipped through to a worker because nothing re-graded the already-authored queue. A new sizing/fitness rule must RETROACTIVELY sweep every open task; `rmd retro` re-grades the queue against every standing rule and files corrective tasks (Standing rule 20). A rule the queue is never swept against protects only tasks written after it. [DIAGNOSIS.md diag/w1t12-max-turns, W1-T12] [subsystem: process · files: plan/tasks.yaml]

## Task quality is a gate, not a diagnosis (§5C)

- The runner was the FIRST line of task-quality defense, so FOUR malformed tasks (W1-T6, W1-T9, W1-T12) reached workers and burned budget before the pattern was caught. Task quality must be gated BEFORE dispatch — a deterministic linter, FAIL-CLOSED (`verdict=blocked_illformed`, no worker spawned) — and re-graded continuously by the retro plan-health sweep. Reactive diagnosis after an 81-turn / $10 burn is the anti-pattern this project exists to kill; the runner is the LAST line, not the first. [W1-T12, this session] [subsystem: architecture · files: src/lib/task-linter.ts, src/run-task.ts, src/lib/retro.ts]
- Most task-quality failures are catchable DETERMINISTICALLY — no LLM: criteria count (sizing, Rule 19), subsystem count, forbidden live-context verbs (headless-fitness lexicon, Rule 18), proof-shape (observable proof, not a vibe). Reserve the Architect LLM review for genuine ambiguity ("is this really one concern?", "is a criterion secretly a task-inside-a-task?"). Cheap deterministic checks first; the LLM layer is advisory and does not block. [W1-T12, this session] [subsystem: architecture · files: src/lib/task-linter.ts]

## Control surface must fail loud; STOP is one-shot (unattended-daemon hazards)

- An unknown CLI command / unrecognized subcommand fell THROUGH to launching a drain — `rmd daemon install --dry-run` silently ran the daemon and merged a task unattended (bogus `install`/`--dry-run` were ignored) — and STOP was a persistent latch that silently blocked every future `rmd drain` until manually cleared. Both are unattended-daemon HAZARDS. Fix: a control surface must FAIL LOUD on bad input (unknown command/arg → usage + non-zero exit + spawn NOTHING; spawning commands validate flags BEFORE touching config/locks/workers), and STOP must be ONE-SHOT (auto-consumed on the halting run's terminal verdict; a no-op-that-warns when nothing is running), genuinely distinct in lifecycle from PAUSE (a persistent maintenance hold cleared only by `rmd resume`). [fix/cli-safe-control-surface, W1-T14/T15 incident] [subsystem: cli · files: src/run-task.ts, src/lib/fleet-control.ts]

## How the daemon selects its repo (repo targeting)

- `rmd daemon` originally read its plan from the CHECKOUT it ran in and hardcoded the status gateway to `ghGateway(owner, "remudero")` — so it could only drain the main repo and an unattended run would SILENTLY self-target. It now takes an EXPLICIT target: `--repo <name>` scopes the gateway to `<owner>/<name>` and reads the plan from a synced clone (`<root>/repos/<name>/plan/tasks.yaml`, `reset --hard origin/main` so it's current); `--plan <path>` overrides; `--dry-run` previews (spawns nothing, logs `daemon.target`). A SELF-TARGET GUARD refuses to drain the daemon's own source repo unattended without `--allow-self-target`. `rmd daemon-plist --repo <name>` bakes `--repo` into the launchd unit's ProgramArguments so the unit drains the intended repo, not an implicit default. (Task EXECUTION already derived its repo from `task.repo` via runTask — only the daemon's plan source + gateway were pinned.) [fix/daemon-repo-targeting, W1-T12d] [subsystem: cli · files: src/run-task.ts, src/lib/launchd.ts]

## Daemon commissioning: launchd KeepAlive restart behavior (W1-T12d)

- The launchd unit runs the daemon with **KeepAlive**, so launchd restarts it on **any** exit. This IS the crash-recovery path — a kill-9 → KeepAlive restart → the daemon re-derives merged state from GitHub and goes idle (reconstructOrphan, W1-T12c, handles a real in-flight orphan), with **no duplicate task run**. **Gotcha:** a `--max N`-bounded daemon under KeepAlive **restart-loops** — it drains N, exits `max_reached`, launchd restarts it, it finds nothing runnable (`daemon.idle`), exits, restarts… (observed in commissioning: 3 `daemon.start` lines, the 2nd/3rd immediately idle). For a real unattended run use **no `--max`** (persistent poll loop); reserve `--max`/`--dry-run` for a bounded manual drill and expect KeepAlive to bring it back. Plist: `~/Library/LaunchAgents/com.remudero.daemon.plist`; commission with `launchctl load <path>`. [chore/t12d-commissioning-record, W1-T12d] [subsystem: platform · files: src/lib/launchd.ts, src/lib/daemon.ts]
