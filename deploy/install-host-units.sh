...
IMAGE=${IMAGE}
REVIVAL_LOG=${REVIVAL_LOG}
# W1-T3269 — the dedicated control checkout this host converges FROM, and the heap the installer
# requires. The daemon's bind-mounted checkout is detached at its boot SHA while the daemon is
# healthy, so it cannot prove it is the reviewed current tree. The dedicated install is the one
# checkout that deploy-run owns and can prove clean/on-main; it is the only safe source for a
# root-capable unit rewrite. Rendered rather than re-derived so the check and install share inputs.
CONTROL_CHECKOUT=${STATE_DIR}/daemon-install
...
converge_host_units() {
  ...
  [ -d "\$CONTROL_CHECKOUT/.git" ] || { echo "rmd-relaunch: units -- \$CONTROL_CHECKOUT is not a control checkout; not converging."; return 0; }
  if [ -n "\$(git -C "\$CONTROL_CHECKOUT" status --porcelain -uno 2>/dev/null)" ]; then
    echo "rmd-relaunch: units -- checkout has tracked changes; not converging (an unreviewed tree must never become root config)."
    return 0
  fi
  head_sha=\$(git -C "\$CONTROL_CHECKOUT" rev-parse HEAD 2>/dev/null || echo unknown)
  main_sha=\$(git -C "\$CONTROL_CHECKOUT" rev-parse origin/main 2>/dev/null || echo unknown)
  ...
  if [ -n "\$INSTANCE_NAME" ]; then
    if env "\${INSTALLER_ENV[@]}" "\$CONTROL_CHECKOUT/deploy/install-host-units.sh" "\${INSTALLER_ARGS[@]}" >/dev/null 2>&1; then
      return 0
    fi
  elif env "\${INSTALLER_ENV[@]}" "\$CONTROL_CHECKOUT/deploy/install-host-units.sh" >/dev/null 2>&1; then
    return 0
  fi
  ...
  if [ -n "\$INSTANCE_NAME" ]; then
    if sudo -n env "\${INSTALLER_ENV[@]}" "\$CONTROL_CHECKOUT/deploy/install-host-units.sh" --install "\${INSTALLER_ARGS[@]}"; then
  ...
  elif sudo -n env "\${INSTALLER_ENV[@]}" "\$CONTROL_CHECKOUT/deploy/install-host-units.sh" --install; then
    printf '%s units-converged sha=%s\\n' "\$(date -u +%Y-%m-%dT%H:%M:%SZ)" "\$head_sha" >> "\$REVIVAL_LOG" 2>/dev/null || true
  else
    echo "rmd-relaunch: units -- converge FAILED; the next tick re-asks." >&2
  fi
