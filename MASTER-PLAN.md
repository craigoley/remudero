...
- rule-efficacy:CLAUDE.md#investigation-discipline:bound-fires-on-healthy-condition (31 recurrences reported since 2026-08-06 — RATIFIED AT THE CLASS, NOT THE ASK: the count is an ACTIVITY count, not a violation count, because both matched steps are emitted by the POST-FIX call sites (`ci.stalled` by W1-T382's derivative predicate, run-task.ts:4603-4639; `deploy.idle_ceiling_forced` by W1-T380's honest ceiling row, deployer.ts:753-757) and the metric's own doc admits they are "logged at the exact call sites that fixed them" — so the metric set a threshold over a population it never observed, which is the very defect the rule names; W1-T418's note predicted "a wrong signature table manufactures evidence against innocent rules" and its before/after-date falsifier cannot bite, these rows being genuinely post-date. W1-T4271 makes a signature declare VIOLATION vs ACTIVITY and forbids an activity signature from reaching REPEATING or escalating, preserving the escalation for violation-shaped rules; W1-T4272 mechanizes the measurement the rule actually asks for — the subject-and-outcome census W1-T382 performed by hand once (21 of 21 booked PRs later merged) and seven shards have cited from memory since — counting episodes not rows and refusing a split behind a positive control. NO bound's value is tuned, no rule text is edited, and no verdict is drawn from the new census here) — RATIFIED 2026-09-02 -> W1-T4271/W1-T4272.
