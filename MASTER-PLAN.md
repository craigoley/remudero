...
- codeql-quality:js/unneeded-defensive-code (RawAlert/renderAlertRaw never captured the flagged code's location, so the rule cleanup was undispatchable — no implement worker has gh or WebFetch, and no repo-tracked artifact named the site; split into the location-capture fix and the single-alert cleanup #144 it unblocks) — RATIFIED 2026-09-13 -> W1-T4273/W1-T4274.
