...
- skill-draft:bf9fbed9e9207c08 (a mined procedural skill — clean_single_strike + fully_executed_proof, evidence-backed across 27 merged implement runs and already safety-scanned; `rmd approve`'s own writer has no path to `.claude/skills/` today — `RatificationPayload` carries only a task fragment and a stamp — so ratification lands the file as a task rather than relying on an approve-time write that does not exist; the proposal's own reachability caveat also names the dead settingSources path rather than the live W1-T3101 prompt-injection route) — RATIFIED 2026-09-11 -> W1-T4343.
