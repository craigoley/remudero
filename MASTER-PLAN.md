...
- skill-draft:62aff41d7a5fd038 (implement-clean-single-strike-fully-exec — a 26-run-evidenced procedure, clean_single_strike + fully_executed_proof; its staged 'NOT reachable by a worker today' note describes only the closed native settingSources route — the already-shipped W1-T3101 approved-tree injection route (loadInjectableSkills -> selectSkillsForTask -> renderSkillsPart) delivers this draft's full body on the strength of its own applies-to: implement frontmatter, so ratifying it is sufficient by itself; W1-T4336 separately fixes the reachability assessment's hardcoded [] literal and its silence about this route for every future candidate) — RATIFIED 2026-09-11 -> W1-T4335/W1-T4336.
