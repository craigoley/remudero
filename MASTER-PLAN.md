# REMUDERO — Master Plan (v2.79 · synced 2026-09-20)
**FOCUS — TRANSIENT FAILURES RETURN WHILE SHIPPING STILL OUTRUNS RUN VERDICTS.** R66 observed 29 runs:
`blocked_transient`×3 + `incomplete`×22 + `merged`×2 + `no_pr`×2. The shipped union resolves five task
ships, including three gate-side merges whose runs ended `incomplete`; the blocked classes route to P42, so no
proposal is minted and the highest prior active header remains P81.

**R66 state:** W1-T49 remains the sole plan-health finding with an empty declared scope. Replay and
...
receipt. See W1-T3852 for the bounded public contract that lets a private console compose these
...
condition or new activity. W1-T3853 and W1-T3854 make these rules durable and source-labeled.
...
★ **CURRENT R66 (RETRO-1789871155474, 2026-09-20): 29 OBSERVED RUNS YIELDED TWO VERDICT MERGES AND FIVE OWNERSHIP-ASSERTED SHIPS.** The deterministic census is implement×9, retro×6 and triage×14; verdicts are `blocked_transient`×3 + `incomplete`×22 + `merged`×2 + `no_pr`×2. Reconciliation credits W1-T3736 and W1-T3807, while the shipped union additionally resolves two triage runs and W1-T3819 gate-side. MAST maps two failures to `verification`, leaves `blocked_transient`×1, `blocked_transient:openweight_error`×2 and `incomplete`×19 explicitly unmapped, and excludes the three verdict-blocked gate-side merges from the failure census.

★ **CURRENT NET STATE:** W1-T49 remains the sole plan-health defect: its absent `files:` declaration serializes co-dispatch through fail-closed overlap. Plan coherence is clean across 1,709 shards and 269 monolith records; the plan-state truth rung resolves its one asserted-unbuilt id as still unmerged after examining 38 assertion-bearing lines and explicitly skipping 7 proposal-subject lines, and SHIPS-UNWIRED names no unreached NET STATE symbol. Containment and isolation fired zero times at 18 and 26 markers, but replay recorded no candidate-harness run, so retirement remains unverified. Mutation-ratchet history is still unmeasured at N=0 because no verdict row has ever been emitted. LEARNINGS remains at 79 with none added in-window; ratification is 0 approved / 0 reframed over 126,857 rows since 2026-09-17T19:20:57.612Z, never evidence of zero opportunity.

★ **CURRENT CALIBRATION:** implement is 9 runs / 2 merged / $0.041 average / 20.111 average turns / $0.366 total; retro is 6 runs and triage is 14, both with no merge or measured spend. The `src` task-class row reads 2 of 9 merged (22%); its $0.122 per merge, 60.333 turns/merge and 69,323.667 output tokens/merge use the stated shipped source n=3. Weekly burn attributes all 1,118 measured turns to sonnet while 91 unresolved runs carry zero measured turns. G-17's capability-half history is 332 Architect-lane rows and $0.79 notional, 28.2% of 409 rows / $2.79 notional; its newest row is 2026-09-20T02:04:09.274Z, so this is stale historical notional share and moves no tier or mount row.
...
★ **PRIOR R58 CALIBRATION:** `implement` reads **14 runs / 7 merged / $0.858 avg / 71.857 avg turns / $12.013 total** — **50%** — and the `src` class row reads **14 runs / 7 merged / 50%**: **the two rates AGREE for the first time in this series' history, and they agree because the `diagnose` lane that the class mapping folds in contributed NO runs this window** (67%-vs-71%, 71%-vs-63%, now **50%-vs-50%**). ★ **A discrepancy whose cause is a fold does not end when the folded population happens to be empty — it goes unobservable** (**P73**, HELD; rule 18 forbids reading the agreement as repair, and R58-2 registers the distinction). **★ `triage` READS 8 RUNS AT EXACTLY $0.000 FOR AN EIGHTH CONSECUTIVE CYCLE**, now across populations of 20, 4, 6, 5 and 8 — **the lane that escaped this shape last cycle (`diagnose`) left the table entirely, so the constant-price class has one confirmed escape and one member that has never moved** (**P74**). **★ THE THREE MERGE-COUNT COLUMNS READ `src` 7 / 7 / 7 AGREED, `triage` 0 / 0 / 0 AGREED, `retro` 0 / 1 / 1 DISAGREED — a THIRD consecutive identical pattern**, so the retirement clause (*two consecutive all-AGREE cycles*) stands at **0 of 3 opportunities taken**; `turns/merge` 143.714 and `output tokens/merge` 84,824.286 divide by the 7 the row states (1006/7, 593,770/7). **★ P72 HOLDS — the coverage stamp is absent a SIXTH consecutive cycle**, and the closure table again REFUSES two rows on a population floor (`retro` n=1, `triage` n=0, *"below floor 5, P48"*) while `src`'s five derived columns go out unmarked. **★ P73 HOLDS AT THE DOLLAR ALTITUDE EVEN AS IT VANISHES AT THE RATE ALTITUDE: $39.482 over 24 runs** (`BY TASK TYPE`), **$168.126 over 94 runs** (`BY MODEL CLASS`, 49 of them `unresolved`) and **$809.28 over 582 rows** (G-17) — a **20.5× spread** after 42.6×, 56×, 34×, 15×, 1.4× and 35.9×. **The mount table stays FROZEN** on P65's changed-files classifier, unshipped (DR-26). `retro` reads **2 runs / 0 merged / $13.734 avg / $27.469 total** against **$70.49 across 6 `retro.synthesized` rows** (**P69**); `plan.synthesized` and `triage.synthesized` price **ZERO rows at $0.00 a SECOND consecutive cycle** — the emptied constant-price rows stay empty (**P74**, **P54**). **★ THE CONTRAST ROWS HOLD AT THREE — forty-six cycles, no row past n=5 that can answer W1-T167**, and `docs` has merged NOTHING ever at **42 open** (38 last cycle); `plan-lint` 0 of 18. G-17 reads **451 Architect-lane rows / $596.38 / 73.7%** of a 582-row / $809.28-notional corpus — **rows UP 547 → 582 while dollars fell $926.99 → $809.28, so the "second consecutive same-direction move" ENDS BY DIVERGING** (**P54**, confirmed a thirteenth time; rule 18 — neither a repeat nor a reversal is a convergence). **★ THE OPENWEIGHT LANE EXTENDS ITS MAJORITY A FOURTH TIME:** `inbox_draft` reads **gpt-5-nano×354 + sonnet×91** over 445 rows — **80% openweight**, after 38%, 59% and 73%. NOTIONAL, HISTORICALLY scoped, capability-half only; **no mount row moves on it**. **★ THE UNATTRIBUTED MODEL ROW RETAKES THE MAJORITY — 49 of 94 runs (52%) resolve to no model class and to 0.0% of the burn objective**, after giving the majority back last cycle. Replay (*no run recorded*, thirty-fifth) and mutation-ratchet (*N=0*, fortieth) remain **P48 absences, never zeros**. **★ P38's JUDGE CROSSES ITS OWN BOUNDARY A SECOND TIME, IN THE OPPOSITE DIRECTION:** `askuserquestion-auto-resolves-empty-headless`, **PROMOTED at 0.73 last cycle**, is **DECLINED this cycle at 0.90** on text nobody edited; `a-roster-error` rises **0.32 → 0.74**; and `a-rigged-probe-measures-nothing` (0.92) and `an-error-names-its-cause-not-its-check` (0.87) are **re-PROPOSED A SECOND CONSECUTIVE CYCLE**, having been "promoted" twice and landed nowhere (**P71**).
...
**Inventory (R66, 2026-09-20): 2 merged-since-marker task rows, one repeated; the ownership-asserted
shipped union also credits TRIAGE-fb-1789845640231-c117a2/#6135,
TRIAGE-fb-1789845848265-8919ba/#6148 and W1-T3819/#6176 gate-side.** W1-T3736/#6141 is a repeated
ledger-credit rescue and W1-T3807/#6154 is newly logged. The run window reads `blocked_transient`×3 +
`incomplete`×22 + `merged`×2 + `no_pr`×2 (**P47**); the two reasoned openweight rows and one generic
transient row stay explicitly unmapped rather than being assigned a guessed MAST cause (**P42/P48**).
...
(`pruneSelfReferentialFollowups`, the `worktree.*` fields' absent status-board latch, `namedTaskId`, the
`rmd sweep` label undercount, the `docs/docs-index.json` deadlock — **P63, exercised**). The
**golden-task replay suite (T165/#2232) IS STILL DARK.**
...
table below is the row that re-bases it — **and the mount table stays FROZEN.**
**★ THE FREEZE — ONE CLAIM, AND IT IS NOT A NUMBER (R52 folded three NET-STATE blocks to three sentences,
R54 deleted two of them, and R57 deletes the per-cycle justification that kept regrowing under the
third).** **THE FREEZE'S RELEASE CONDITION IS UNCHANGED AND UNMET.** R32 cut it onto TASK G, R33 onto TASK
L, R43 onto **P65's CHANGED-FILES CLASSIFIER**, which has not shipped — **a release condition is released
only by the thing it names** (DR-26), never by a window that happened to read well, and a freeze forbids
re-BASING, never publishing. **No per-cycle sentence may stand here again**: the two that did were each
stale within one cycle, and the freeze needs no window to justify it because its release condition is a
SHIP, not a reading.
...
row in R58's gather — 0 of 38 rows lifetime — and that guard population was empty again in R58,
...
**ONE OPEN task still declares no `files:` scope** (W1-T49 — the current plan-health sweep returns it
alone again; **it is NOT re-proposed as a corrective task**, because R50-5's arm fired on a third
consecutive MISS, and it sits in the standing DEBT line below as **P71**'s primary exhibit);
...
a TWENTY-SIXTH cycle — though the current plan-coherence rung reads CLEAN across **1692 shards** and 269
...
**★ R58's HARVEST RUNS TO TWELVE BULLETS AND IS DOMINATED BY THREE CAUSES, NOT TWELVE** (deduplicated by
operational cause, each counted ONCE): **(a)** **W1-T3693**, named by FIVE of the twelve bullets across
two rounds — its criteria 1–3 need a `test/*.test.ts` path its declared `files:` list does not contain,
and its criterion 4 demands what its OWN falsifier authorises skipping; the first half is **P63/P78**, the
second is **this cycle's mint, P81**; **(b)** the `unit test:` bare-title proof dialect, where a test whose
title merely PARAPHRASES its proof text grades identically to a missing test — *"test theater"*, two
review cycles paid on W1-T3694, routed to **P78** as the discrimination half of the same defect;
**(c)** a REQUIRED ratchet (`test/fixture-copy-census.test.ts`) whose baseline moves under UNRELATED PRs
on shared counters, so a PR can be red for another PR's arithmetic — **P79**'s shape with a different
artefact. **Last cycle's four causes — the `gh` read floor, the mid-session `GH_TOKEN` expiry, W1-T3680
and W1-T3705, the fixed `/tmp/<literal>` test roots — are NOT re-derived: this gather names none of them,
and a cause not printed is not a cause repaired** (P48).
**★ R58's COMPRESSION, NAMED SO THE ACT IS INSTRUMENTED (R35's lesson) — FIVE NAMED DELETIONS.**
**(i)** the Calibration section's **whole R57 prose block** is **DELETED** into the R44–R57 fold line —
its findings survive in P73, P74, P54 and the pre-registration score, each in exactly one place.
**(ii)** R57's six-row pre-registration table is **DELETED after scoring** (a scored bar is evidence
exactly once); only the MISSED row is carried verbatim as R58-4 and P80's bar re-registered with its
trigger corrected. **(iii)** the R57 SHIPPED head entry folds **33 lines → 11**. **(iv)** the two
per-cycle ORDINAL-CORRECTION annotations R57 left inside the `Still PLANNED` paragraph — *"this ordinal
was published here as 31…"*, *"this population was published here as 1506…"* — are **DELETED**, on
**DR-9**: a correction written inside the section the scanner reads is not a correction, and republishing
it every cycle is the accretion the correction was for. **(v)** the `Retro proposals` section's
ten-line republished EXPLANATION of the deleted live ranking is **DELETED to two sentences** by its own
doctrine — an explanation of a deletion republished every cycle is exactly what the deletion was for.
**(vi)** the **RETRO-1789533558153 and RETRO-1789482309645 SHIPPED entries are FOLDED INTO ONE (34 lines
→ 16)**, every id, PR link, cost and turn count preserved, on the precedent R48 and R55 set.
★ **THE ACCOUNTING, STATED RATHER THAN CLAIMED — because this rung has already mis-stated its own
machinery twice in three cycles.** The six deletions remove **~482** lines and the pass adds **~623**, so
**the file grows by ~141**: a mint costs a canonical entry, a registered bar, a seventh scorecard row and
a DEBT item, and this cycle bought one. **Every section this retro rewrote is net-negative or flat except
the two the mint pays for**, and the honest reading of R35's bar is that the ACT be instrumented, which it
is here to the line. *A retro that reported itself net-negative by choosing which sections to count would
be exactly the authored population R56 was minted on.*
★ **AND R57-5's CONTRADICTION SEARCH RETURNS ONE — INSIDE A CANONICAL ENTRY, WHERE IT HAD NEVER LOOKED.**
The search extracted every ordinal phrase (*FOURTH…FORTY-Nth, consecutive/scored/cycle/time*) across NET
STATE, the scorecard, SHIPPED, Calibration and Retro proposals, cross-read the twelve headline figures
this file states 2–5 times, and — **widened this cycle to the `AGE N` annotations the proposal entries
carry** — found **P77's age published as *4* in its own header and as *1* in its own body, one entry, one
quantity, two values**. Both are resolved here by stating it ONCE, in the header, at its true **5**;
P44's and P80's entry-side ages are refreshed in the same pass so the DEBT line and the entries cannot
drift again. ★ The find matters beyond its size: **the previous two cycles searched only the prose above
this section, so a clean reading was a statement about where the search had been pointed.**

It is the FOURTEENTH consecutive cycle in which this document loses a claim it made about ITSELF: R45
deleted a DARK-roster line sixteen SHIPS-UNWIRED scans had agreed with, R46 an emission claim G-17
readings had agreed with, R47 R46's five-day-staleness residue, R48 the SEPARATION of two entries, R49 a
REPAIR CLAIM that survived exactly one cycle, R50 a sentence that located the turn in the wrong cycle,
R51 its own repeated offer to fix the plan, R52 its habit of saying each thing twice, R53 a retirement
condition it had written one cycle earlier, R54 its own RANKING, R55 a block it had been publishing two
cycles out of date, R56 the check it wrote to catch exactly that, R57 the retirement counter of the
successor check, and **R58 A KILL TRIGGER ITS OWN TIE-BREAK FORBIDS FROM FIRING — R57-6 named one
reading (*a harvest with no `gh` refusal in it*) UNAVAILABLE in one clause and A COUNT OF ZERO in the
next, and the entry it governs advances toward retirement or does not depending purely on which clause
is read first** (**DR-30**). ★ **Three cycles running, the loss has been in the RETIREMENT MACHINERY
rather than in a reading** — R56's condition, R57's counter, R58's trigger. *A rung that writes its own
exit conditions faster than it scores them accumulates exits that cannot be taken.*
...
**R46 adds the twenty-ninth, and it is the first one about IDENTITY rather than sight, contention or
time: the harness gave one of its own lanes a name instead of a key.** Every retro run in this window —
twenty-five of them, $151.507, the most expensive lane in the ledger — is keyed `RETRO`, a constant. A
single merged PR that ever carried that string (#5183, `codex/retro-preflight-child-env`, REST-verified)
therefore resolves as the trailer match for **every** retro run that has ever run and every one that ever
will, so the credit resolver rejects the whole lane, forever, one run at a time, with a sentence that
reads like a per-run accident. R40's turn was that a branch name can lie about a run existing; R41's that
the absence of one is not the absence of work; R42's that the board has no owner column; R43's that the
ledger has no second reading; **R46's is that the harness's most expensive lane has no primary key.** The
constraint is therefore: **a run identifier that is constant across runs is not an identifier — it is a
...
**R49 adds the thirty-first, and it is about the HONESTY LABEL rather than the reading: the harness knows
which of its rows are untrustworthy and says so on only one of them.** `turns/merge` and
`output tokens/merge` arrive stamped **⚠ 29% coverage — DO NOT USE**. `avg $`, `avg turns` and `total $`
arrive clean. All five are computed over the SAME 38 rows, and the holes are per-ROW — a run missing
`num_turns` is overwhelmingly the same run missing `cost_usd` (25 of this cycle's 104 credited pairs
print `$0.000`, most of them beside `0 turns` or `1 turns`). **The stamp is therefore attached to the
derivation rather than to the population**, and the one column a reader is steered toward is the one
W1-T5's mount table keys off. R46's turn was that a lane keyed by a constant has no primary key; R47's
that the record has none either; **R49's is that the harness has a working coverage sensor and wires it
to one output.** The constraint is therefore: **a caveat that is per-column and not per-population is
worse than no caveat at all, because it certifies by omission every sibling it did not mark.** The
practical test costs nothing: **for every derived column in a table, print the count of rows that
actually carried its input — and if you cannot, do not print the column.**
**R50 adds the thirty-second, and it is R49's turn stripped of its excuse: the harness prints two
numbers for the same quantity, forty times apart, on facing pages of one document, and owns no rule
that they must agree.** R49's finding was that a caveat was attached to the wrong columns. **This cycle
there is no caveat at all and the number is simply WRONG** — `implement` totals 27 runs at **$3.907**
while the credit list three sections away prices one of those very runs at **$12.322**, and the 28
in-window members it names sum to **≈$157.485 / ≈2,371 turns** against a row claiming ≈53 turns. The
same row prices 673,516 output tokens at $3.907 — **$5.80 per million output tokens, below any list
price the mounts route to**, which is a contradiction internal to ONE ROW and needs no second table to
see. R46's turn was that a lane keyed by a constant has no primary key; R47's that the record has none
either; R49's that the harness wires its coverage sensor to one output; **R50's is that the harness has
no INVARIANTS — only outputs.** The constraint is therefore: **an instrument with no cross-check between
its own published figures cannot be wrong in a way it is able to detect, and will report a total smaller
than its largest member with exactly the confidence it reports a correct one.** The practical test is
one assertion and costs nothing: **for every aggregate, assert `total ≥ max(member)` and `n ≥ 1`, and
print the assertion's result beside the aggregate.**
**R51 adds the thirty-third, FOLDED TO ITS STATEMENT BY R52 (the argument was made once, in the cycle
that minted it, and git holds it): the harness cannot tell a channel that is quiet from a channel that is
dead, because a re-emission looks exactly like a report** — 27 pairs, 27 of 27 already held, all seven
new merges arriving through the path the plan calls supplementary (**P74**, **P70**).
**R52 adds the thirty-fourth, FOLDED TO ITS STATEMENT BY R53 (the argument was made once and git
holds it): acquiring the missing evidence changes nothing, because no rule requires two of the harness's
own readings to be compared** — a credit list of 22 pairs with 0 new, three columns away from
`merged`×16 of 40 and an 80% `src` merge rate (**P75**).
**R53 adds the thirty-fifth, and it is the thirty-fourth's answer arriving and proving worthless: the
harness's own readings AGREED this cycle, and the agreement carried no more information than the
disagreement did.** `src` read **4 / 4 / 4** across all three merge-count columns — the join P75 asked
for, satisfied by measurement — while `retro` and `triage` "agreed" only because one of the three
columns REFUSED to answer on a population floor. **Nothing consumed the agreement**, no rank moved on it,
and the one entry it could have retired survived only because that entry's own retirement clause
contradicted itself. R50's turn was that the harness has no invariants, only outputs; R51's that it
cannot tell a quiet channel from a dead one; R52's that a disagreement is only information where a JOIN
exists; **R53's is that a JOIN is only information where something CONSUMES it.** The constraint is
therefore: **agreement between two instruments is not evidence of health unless some rung would have
acted on their disagreement — otherwise both readings are decoration, and a document can be internally
consistent and still describe nothing.** The practical test costs nothing: **for every cross-check this
file publishes, name the rung that changes its behaviour when the check FAILS; where none exists, print
the check as DECORATIVE.**
**R54 adds the thirty-sixth, and it is this section's own machinery turned on itself: the harness can
CLOSE a finding without fixing it, because a retirement condition written over a READING is satisfied by
the world and not by the work.** P75 asked for exactly one thing — that some column be declared
authoritative — and closed this cycle at 2 of 2 with no column declared, no join built and no code
changed, purely because `src` happened to agree with itself twice while the other two lanes were empty.
**And P71 guarantees the other exit is shut:** nothing converts a ratified proposal into a filed task, so
in this section's whole history entries leave by RETIREMENT, by FOLDING, or not at all. R52's turn was
that a disagreement is only information where a JOIN exists; R53's that a join is only information where
something CONSUMES it; **R54's is that a finding whose only available exit is retirement measures the
drafting of its own bar and never the defect.** The constraint is therefore: **a proposal register with
no filing path does not track repairs — its throughput is the rate at which its own retirement clauses
come true, which is a fact about the writer.** The practical test costs nothing and is one clause:
**every kill trigger must name the ARTIFACT that must exist for the entry to close — a file, a column, a
field, a merged PR — and never a reading a quiet window can supply.**
**R55 adds the thirty-seventh and R56 the thirty-eighth, and they are one argument in two places: a GATE
can be unsatisfiable by a correct worker, and a CHECK can be unsatisfiable by a correct auditor.** R55's:
`proof-discrimination` demanded a head-vs-base difference from a criterion whose entire content is that a
file does NOT differ, and demanded novelty from a proof an already-merged sibling had authored — **the
gate's pass condition contradicting the task's correctness condition**, with both escapes on the plan
side of a fence the implement lane may not cross (**P78**; **P79** is the same shape with the wall clock
as its input). R56 turns that on this rung: **R55-5 registered a freshness check whose population was a
list it typed**, and the two blocks it missed were the two contradicting the paragraphs directly above
them. R53's turn was that a join is only information where something CONSUMES it; R54's that a finding
whose only exit is retirement measures its own bar; **R55's is that a gate a correct implementation
cannot pass is a queue, and R56's is that a check whose population is AUTHORED rather than DERIVED is an
opinion.** The constraint is therefore: **every gate and every self-audit must be able to name the
artifact that would satisfy it — and where that artifact cannot exist inside the fence the same system
draws, the check is not measuring the work, it is measuring the fence.** The practical test costs
nothing: **for each check, write down one concrete diff that passes it; where you cannot, the check is a
queue and must be published as one.**
**R57 adds the thirty-ninth, and it is R55/R56's argument moved from the GATE to the THROTTLE: a control
that is advisory, self-stamping and unlogged converts obedience into blindness.** Four rounds honoured
the 180-second `gh` read floor; each refusal re-stamped the clock the next attempt was measured against,
so the compliant worker never got its read — *159s → blocked → 12s → blocked → 20s → blocked → 33s →
blocked → 47s* — while a worker that ignored the floor would have paid nothing. **The harness recorded
none of it**: zero guard fires, zero infrastructure events, and the unverified PR body and unread
REQUIRED-check log appear in no column. R55's turn was that a gate a correct worker cannot pass is a
queue; R56's that a check with an authored population is an opinion; **R57's is that a throttle whose
clock advances on REFUSAL is a lockout wearing a rate limit's name, and it selects precisely against the
agents that respect it.** The constraint is therefore: **a self-stamping cooldown makes compliance
indistinguishable from failure, and the harness's own instruments will show neither.** The practical test
costs nothing: **stamp a cooldown on the ALLOWED action only, and publish the count of refusals it
issued — a throttle that cannot say how often it said no is not measured at all.**
**R58 adds the fortieth, and it moves R55/R56/R57's argument off the GATE, the CHECK and the THROTTLE and
onto the RECORD they all read: one artifact addressed to two readers is two artifacts, and only the one
with a gate behind it has any force.** A task record carries acceptance criteria AND a falsifier. The
falsifier is written for an Architect and says when the task should be CLOSED UNBUILT; the criteria are
read by a mechanical reviewer that never receives the falsifier text. So W1-T3693's rounds could verify
the falsifier's condition holds, act on it correctly, and still be graded as having failed criterion 4 —
**there is no diff, in scope or out, that both honours the falsifier and satisfies the criterion**, and
the round said exactly that. R55's turn was that a gate a correct worker cannot pass is a queue; R56's
that a check with an authored population is an opinion; R57's that a throttle stamped by refusal is a
lockout; **R58's is that a document with two audiences and one gate has silently promoted one clause to
law and demoted the other to commentary — and nobody chose which.** The constraint is therefore: **where
a record's clauses are consumed by different readers, the clause no reader consumes is not a weaker rule,
it is not a rule**, and an agent obeying it is indistinguishable from an agent failing. The practical
test costs nothing: **for every field a task record carries, name the reader that consumes it; any field
with no consumer must be deleted from the schema or wired into the gate — never left as advice.**
KICK ORDER (mirrored as a comment atop plan/tasks.yaml).
**★ EFFECT PRE-REGISTRATIONS (P43(i) — stored here until it is stored as data. ★ **The per-cycle
"R<N>'s rows scored below, R<N+1>'s registered there" sentence is DELETED by R57**: it named two fixed
cycles and was stale the moment either moved. The STANDING rule replaces it — **the newest retro scores
its predecessor's table, deletes it, and registers its own below.**)**
...
**R43's through R49's pre-registrations — SCORED, THEN DELETED** (R44 scored R43's four-hit /
two-miss / one-unresolvable set; R46 scored R44's nine at 3 HIT · 1 MISS · 5 UNRESOLVABLE; each
subsequent cycle deleted its predecessor's table on scoring; **R58 deletes R57's**).
**A scored table is evidence exactly once**, the rule is applied to this rung's own predecessors as
readily as to anyone's, and the dispositions travel in the entries they govern.

**★ R57's SIX ROWS, SCORED BY R58 FROM THIS CYCLE'S GATHER — 5 HIT · 1 MISS.**
**★★ R57-1 HIT, AND ITS FALSIFIER ADVANCES A SECOND TIME.** Stated: `blocked_budget` reads **0 of 24
(0.0%)**, the window's whole measured spend is **$39.482**, the published ceiling is **$500/day
committed**, and n=**24**. Limit id, cap value and spend-at-block are **UNAVAILABLE**, and this time
**VACUOUSLY** — the class has no member at all, `Infrastructure events` reads *None*, and MAST's single
row names `verification`, not a budget. The arm requires two CONSECUTIVE in-band zero windows and
6.5–26 runs is the band; **n=24 is in band, so the counter moves to 1 OF 2.** ★ Scored under R57-1's own
NEW CLAUSE: this window is evidence *for the entry's retirement*, and is worth strictly LESS than last
cycle's member-bearing window was as evidence *for the entry* — the clause works in both directions and
is honoured here in the direction that costs this rung something.
**★★ R57-2 HIT — AND THE QUANTITY IT WAS BUILT TO CATCH READS A GAP OF ZERO FOR THE FIRST TIME.**
`src` **7 / 7 / 7 AGREED**, `triage` **0 / 0 / 0 AGREED**, `retro` **0 / 1 / 1 DISAGREED** — a THIRD
consecutive identical pattern, the retro lane's gate-side merge seen by the closure table and by
`merge source` and by neither `merged` cell. Derived columns divide by **7**, which is what the `src` row
states (1006/7 = 143.714; 593,770/7 = 84,824.286). The double-published quantity is named with both
values and its sign: **50%** (`implement`, 7 of 14) and **50%** (`src`, 7 of 14) — **gap 0, sign
undefined**, after 67%-vs-71% and 71%-vs-63%. ★ **The row's ★ MAY-NOT-READ-AGREEMENT-AS-REPAIR clause is
what makes this scorable at all:** the two rates coincide because the `diagnose` lane the class mapping
folds into `src` contributed NO runs, so the fold that causes the discrepancy had nothing to fold. **P73
is HELD on a zero reading**, and the retirement clause (two consecutive ALL-AGREE cycles) is untouched at
**0 of 2** — `retro` has now disagreed in three consecutive readings.
**★★ R57-3 HIT ON BOTH HALVES, WITH A 100% REPLACEMENT RATE ON (i) FOR A SECOND CONSECUTIVE CYCLE.**
(i) Count **4**, every member named and **all four NEW**, all inside **W1-T3693**: criteria 1–3, whose
literal-substring proofs cannot resolve to any test because the task's declared `files:` list contains no
`test/` path, and criterion 4, whose implementation the same record's falsifier authorises skipping.
**Last cycle's members W1-T3680 and W1-T3705 are not named by this gather** — neither repaired nor
carried, and unmeasured is not repaired (P48); **the replacement rate is 100%, twice running, which is
the membership clause R57-3 was widened to catch.** (ii) Count **1** — `test/fixture-copy-census.test.ts`,
a REQUIRED ratchet whose baseline moves under unrelated PRs on shared counters, so the red is another
PR's arithmetic; NEW, no repair landed. ★ Both falsifiers silent: no non-change criterion passed
`proof-discrimination` unmodified, no repair landed from inside an ordinary task's declared scope.
**★ R57-4 MISS — P71's BAR, A TENTH CONSECUTIVE TIME.** Arm (a): no `incomplete` row in
`plan/mast-mapping.yaml` — and `incomplete` reads **×15 unmapped, a THIRTEENTH scored cycle and the
largest count the series has recorded** (×6 last cycle). Arm (b): the plan-health sweep returns
**W1-T49, alone, a THIRTY-FOURTH cycle**, and proposes the same corrective task this rung may not file.
Arm (c): no proposals-ratified-vs-filed column; telemetry **0 / 0 / 0%** a sixth cycle **over a row count
that FELL AGAIN, 190,394 → 186,621** — a THIRD consecutive shrink in a figure the gather calls
cumulative. **The falsifier did NOT fire**, so **P71 is HELD** and the bar is carried VERBATIM as
**R58-4** (rule 14).
**★★ R57-5 HIT ON ITS SECOND OUTING — COUNT 1, FOUND WHERE THE ROW HAD NEVER LOOKED.** The derived
search (ordinal-phrase extraction across NET STATE, the scorecard, SHIPPED, Calibration and Retro
proposals, a cross-read of the twelve headline figures this file states 2–5 times, and — **widened this
cycle — the `AGE N` annotations the canonical proposal entries carry) returns **ONE** pair, named with
both values: **P77's age is published as *4* in its own entry header and as *1* in its own entry body.**
Resolved in this same commit by stating it ONCE, in the header, at its true **5**; P44's and P80's
entry-side ages are refreshed in the same pass. ★ **The widening is the finding.** The previous two
readings of this row searched only the prose ABOVE the proposals section, so their clean results were
statements about where the search had been pointed, not about the document. Kill trigger: **0 of 2,**
reset by a non-zero count. ★ Its falsifier still cannot fire — the row reports no block clean — and this
is the distinction R58's self-loss turns on: **this trigger counts a search PERFORMED; R57-6's counts a
silence OBSERVED, and only the first can ever reach a second reading.**
**★★ R57-6 HIT ON ITS DECLARATION BRANCH — AND ITS KILL TRIGGER IS STRUCK AS UNREACHABLE.** Stated: the
harvest (twelve bullets, non-empty) names **NO** round reporting a `gh` read-cadence or rate-limit
refusal; the refused-then-succeeded question is therefore **not answerable this window**; and **no gather
column prints a refusal counter**, a second consecutive cycle. Under the row's TIE-BREAK that reading is
**UNAVAILABLE, not zero** (P48) — so the row scores HIT. ★ **But its KILL TRIGGER reads *two consecutive
cycles at a count of zero, where the harvest is non-empty*, and this window is exactly a non-empty harvest
with no refusal in it.** The tie-break calls it UNAVAILABLE; the trigger calls it ZERO. **One reading, two
names, and P80 advances toward retirement or does not depending on which clause is read first.** R58
resolves it in favour of P48 — silence is absence — and **DELETES the zero-count trigger**, replacing it
with one readable off a POSITIVE control (see R58-5). P80 is HELD, unexercised, and its evidence is
unchanged from mint.

**★ R58 REGISTERS SEVEN ROWS — the largest table this rung has ever published, and the reason is R58's own turn: a trigger abandoned mid-count is an exit that cannot be taken. R57'S TABLE IS DELETED RATHER THAN CARRIED — a scored bar is evidence once —
with four stated exceptions. (1) R57-4 MISSED and is carried VERBATIM (as R58-4, rule 14). (2) P80's bar returns as R58-5 with its KILL TRIGGER STRUCK AND REPLACED — a trigger the row's own tie-break forbids from firing was never scored, so re-registration is not a re-use. (3) R57-5's kill trigger stands at 1 of 2 and is carried as R58-7 UNWIDENED, because dropping a counter at 1 is how an exit condition quietly stops existing. (4) P81 registers its own bar at mint (rule 15), as R58-6.**

| # | metric | prediction | condition |
|---|---|---|---|
| R58-1 | **★ P44's BAR, CARRIED WITH ITS FALSIFIER AT 1 OF 2 AND ITS BAND UNCHANGED (rule 15, DR-10, DR-26)** — *state `blocked_budget`'s count and share of the window, the window's total measured spend, the published daily ceiling, AND THE WINDOW'S RUN COUNT; then state whether the gather names WHICH limit fired, its value, and the run's spend at that instant — or declare all three unavailable, and say whether the declaration is VACUOUS (no member) or NOT (a member the instrument cannot attribute)* (**this cycle: 0 of 24 = 0.0%, $39.482, $500/day, n=24 — the last three UNAVAILABLE, VACUOUSLY**) | **all four figures stated, the availability of the last three answered, and the vacuous/non-vacuous distinction declared** | **SCORABLE FROM THE VERDICT CENSUS, THE TYPE TABLE AND THE CAPABILITY SNAPSHOT — no network read.** **TIE-BREAK:** a window with zero `blocked_budget` members scores on the spend figures and the availability answer alone. **FALSIFIER, AT 1 OF 2:** two CONSECUTIVE zero windows whose run counts fall within **12–48 runs** (50–200% of 24) return P44 to the tail; a window outside that band is UNRESOLVABLE and must be declared so. **★ THE CLAUSE EARNED LAST CYCLE STANDS AND CUTS BOTH WAYS:** a window WITH a member that cannot name the limit is evidence FOR the entry; a VACUOUS window is evidence for its retirement and must be scored as such even when that costs this rung a finding. |
| R58-2 | **★ THE CLOSED ENTRY'S READING, RE-REGISTERED WITHOUT ITS ID A FOURTH TIME, PLUS AN OBSERVABILITY CLAUSE THIS CYCLE EARNED (P75 stays CLOSED; DR-8, DR-26)** — *for each `task_class` row state its `merged` cell, its `merge source (n=…)` and its closure-table merged count, label each AGREED or DISAGREED, state the denominator each derived column divides by, and where one document publishes one quantity twice, name BOTH values AND the SIGN of the gap* (**this cycle: `src` 7/7/7 AGREED, `triage` 0/0/0 AGREED, `retro` 0/1/1 DISAGREED; denominators 7; 50% vs 50%, gap 0**) | **every row labelled, the denominator named rather than assumed, any double-published quantity printed with both values and the direction of its gap — and, ★ NEW, where the gap is ZERO, the MEMBERSHIP of the folding class stated, so an unobservable discrepancy is never published as an absent one** | **SCORABLE FROM THE BY-TASK-TYPE, BY-TASK-CLASS AND CLOSURE TABLES, all printed.** **TIE-BREAK:** a cycle with one class row scores on the denominator half alone. **★ MAY NOT RE-MINT P75 OR ANY SUCCESSOR ID**, and **★ MAY NOT READ AGREEMENT AS REPAIR** (rule 18) — this cycle proves the sharper case: the two rates agreed because the folded lane was EMPTY. **RETIREMENT, AT 0 OF 2:** two consecutive ALL-AGREE cycles — `retro` has disagreed in three consecutive readings — **and** a column DECLARED authoritative in this file. |
| R58-3 | **★ P78's AND P79's BAR, CARRIED WITH ITS MEMBERSHIP CLAUSE (rule 15)** — *state (i) the count of acceptance criteria a correct diff cannot satisfy, NAMING EVERY MEMBER and marking each CARRIED or NEW, and (ii) the count of REQUIRED checks red for a reason outside the PR's own diff, naming the check and the artefact; for BOTH, state whether a repair landed, and print the REPLACEMENT RATE on (i) — or declare the count unavailable* (**this cycle: (i) 4, all NEW, all in W1-T3693 — criteria 1–3 (no `test/` path in `files:`) and criterion 4 (its own falsifier forbids the build); replacement rate 100%, twice running; (ii) 1 — `test/fixture-copy-census.test.ts`'s baseline moved by unrelated PRs; no repair either half**) | **both counts stated, EVERY member named and marked carried/new, the replacement rate printed, and the repair question answered for each** | **SCORABLE FROM THE FOLLOW-UP HARVEST ALONE.** **TIE-BREAK:** an empty harvest declares the counts UNAVAILABLE and is NOT unresolvable. **FALSIFIER (i):** a non-change criterion passing `proof-discrimination` unmodified retires P78. **FALSIFIER (ii):** a repair landing from inside an ordinary task's declared scope retires P79. **KILL TRIGGER, READ ON MEMBERSHIP:** two consecutive cycles at zero retires a half — **a half whose count holds while 100% of its members are replaced may NOT be scored as stable**, and two consecutive 100% replacement rates (this cycle's reading) must be published as the class RECRUITING, never as the class holding. |
| R58-4 | **★ P71's BAR, CARRIED VERBATIM AFTER A TENTH MISS (rule 14, DR-26) — EVERY CONSEQUENCE IT REGISTERED HAS FIRED AND THE ENTRY STILL DOES NOT RETIRE** — *does ANY arm move: (a) `plan/mast-mapping.yaml` gains an `incomplete` row, (b) any id in `plan/tasks.yaml` names a subject this section PROPOSED — W1-T49's `files:` scope the standing test case, or (c) the gather gains a proposals-ratified-vs-filed column* (**this cycle: none; `incomplete`×15 unmapped a THIRTEENTH scored cycle and its largest count yet, W1-T49 scope-less a THIRTY-FOURTH, no such column, telemetry 0/0/0% over a row count that FELL A THIRD TIME**) | **at least ONE arm moves** — *this is the table's rule-14 row: its HIT requires MOVEMENT, not a reading* | **SCORABLE FROM THE GATHER's MAST section, its plan-health sweep and its own column list.** **TIE-BREAK:** a cycle with no unmapped class scores arms (b)/(c) alone; a cycle whose plan-health sweep does not run IS unresolvable. **FALSIFIER, unchanged:** if the mapping ships while nothing about filing changes, P42 was merely a slow ticket and **P71 RETIRES.** **★ STILL NO NEW ARM — a consequence this rung can pay painlessly is the selection DR-26 forbids.** |
| R58-5 | **★ P80's BAR, RE-REGISTERED WITH ITS KILL TRIGGER STRUCK AND REPLACED (rule 15; the struck clause is R58's own self-loss)** — *state the count of rounds this window that report a read-cadence/rate-limit REFUSAL, naming each; state whether ANY round reports a refused call followed by a successful one in the same session; and state whether the gather prints a refusal counter at all — or declare the count unavailable* (**this cycle: UNAVAILABLE — a non-empty twelve-bullet harvest names no `gh` refusal; the refused-then-succeeded question is unanswerable; no refusal counter exists in any gather column, a second cycle**) | **the count stated with its rounds named OR declared unavailable, the refused-then-succeeded question answered or declared unanswerable, and the absence of a counter declared rather than passed over** | **SCORABLE FROM THE FOLLOW-UP HARVEST ALONE.** **TIE-BREAK, now the ONLY reading of silence:** a window whose harvest names no `gh` refusal declares the count UNAVAILABLE — silence here is P48's absence, never a zero. **FALSIFIER:** one round reporting a completed `gh` read after waiting out the floor WITHOUT a bypass retires P80 as a misdiagnosis. **★ KILL TRIGGER, REPLACED — IT MUST NAME AN ARTIFACT (R54's constraint), NOT A SILENCE:** P80 retires when `deny-floor.sh` stamps the ALLOWED call only **or** any gather column prints a refusal counter — *the old trigger ("two consecutive cycles at a count of zero, harvest non-empty") is DELETED as unreachable: its own tie-break renames every such zero UNAVAILABLE.* |
| R58-6 | **★ P81's BAR, REGISTERED VERBATIM AT MINT (rule 15)** — *state the count of OPEN tasks whose falsifier (or equivalent implementation-waiver clause) is satisfied while an acceptance criterion still demands the implementation, NAMING each and its criterion; state whether the reviewer's input carries the falsifier text at all; and state whether any Architect withdrawal or rewording landed — or declare the count unavailable* (**R58 historical reading: 1 — W1-T3693, criterion 4; R59 correction: W1-T3693 is merged via #5852, so the current open count is 0**) | **the count stated with each task and criterion named, the reader question answered, and the withdrawal question answered** | **SCORABLE FROM THE FOLLOW-UP HARVEST ALONE.** **TIE-BREAK:** a window whose harvest names no such task declares the count UNAVAILABLE and advances NO counter (P48). **FALSIFIER:** one round reporting a reviewer that reads falsifier text, or grades a correctly-exercised falsifier as MET, retires P81 as a mis-read of the gate. **KILL TRIGGER — AN ARTIFACT, NOT A READING:** P81 retires when the reviewer's prompt surface provably carries the task's falsifier, **or** when the plan gate refuses at filing time a criterion the same record's falsifier can close. |
| R58-7 | **★ THE CONTRADICTION ROW, CARRIED WITH ITS SEARCH SURFACE WIDENED TO THE CANONICAL ENTRIES, WHERE THIS CYCLE'S ONLY PAIR WAS HIDING** — *search this document for any quantity it states twice with different values — INCLUDING per-cycle ORDINALS ("a THIRTY-FIRST cycle", "AGE: N cycles", and ★ NEW, the `AGE N` annotations inside each canonical proposal entry) and any RETIREMENT COUNTER against the label in its own cell; state the COUNT found, name each pair with BOTH values, PUBLISH THE SEARCH PERFORMED, and delete or refresh each in the same commit* (**this cycle: 1 — P77's age published as *4* in its entry header and as *1* in its entry body, both resolved to 5 here**) | **the count stated, each pair named with both values, the SEARCH SURFACE published — which sections were read — so that a clean result is a statement about the document and not about where the search was pointed (P48), and each pair resolved in this same commit — a pair merely NOTED is a MISS** | **SCORABLE FROM THIS DOCUMENT ALONE — no gather column, no network read.** **TIE-BREAK:** none needed. **FALSIFIER:** a later cycle finding a contradiction inside a block THIS row reported clean retires it — **a procedure reports no block clean, so this arm can only fire if the row starts listing.** **KILL TRIGGER: 0 OF 2, RESET BY THIS CYCLE'S NON-ZERO COUNT** — two consecutive cycles at a count of zero, ★ and a zero may only be counted where the search surface COVERS the canonical entries, because two prior clean readings did not. ★ This trigger IS reachable, and the contrast with R58-5's struck one is the rule: **a trigger that counts a SEARCH PERFORMED can reach its second reading; a trigger that counts a SILENCE OBSERVED is renamed away by its own tie-break before it ever counts one.** |

Running calibration: **n=210 · hit 118 · miss 63 · unresolvable 28 · falsified 1.** ★ **`falsified` stays
at exactly one member** — R55-5, the only row this rung has ever retired on its own arm; a column opened
for one outcome and honestly left at one is not a column that has stopped working (P48).
...
15-of-17 lifetime, **and R55 WIDENS it to FALSIFIERS: R53-1's arm named 25–40 runs, the fleet dispatched
15, and the arm could not score in either direction — a falsifier sized on the current point is as
fragile as the prediction it guards.** **(11)** *a scope sized by a volatile measurement is sized on the maximum ever
...
whose `num_turns` was written, and one did not. **A claim about a MECHANISM cannot be supported by a
series of readings over populations the claimant does not control** — that is rule 18 (membership,
never improvement) pointed at an instrument instead of a class, and it is the error rule 18 exists to
forbid, committed by the rung that wrote rule 18 down. The practical test is one question, asked before
the word *repaired* is typed: **which merged PR changed the writer?** If there is no answer, the correct
sentence is *this window was covered*, never *this column is fixed*. Its corollary governs the reverse
direction too: **a stamp that returns is not a regression either**, and reading it as one would be the
...
**★ Rules 28–29, FOLDED TO THEIR STATEMENTS BY R51 — the same disposition R40 applied to rules 20–23 and
R47 to rules 25–27; each was argued at length once, in the cycle that minted it, and git holds that
argument. Only the current reading travels.**
**(28, R44’s)** *A VERDICT CLASS IS NOT A FAILURE CLASS — CHECK WHICH OF ITS MEMBERS MERGED BEFORE YOU
PUBLISH IT AS A TAXONOMY.* Its corollary generalises: an instrument whose only input is another
instrument’s output must publish its input’s error rate beside its own result, or it is laundering —
**which this cycle applies to the procedural miner’s 25 commendations over a 27-of-27 re-credited
population.**
**(29, R46’s)** *A RUN IDENTIFIER THAT IS CONSTANT ACROSS RUNS IS NOT AN IDENTIFIER — IT IS A COLLISION
THE CREDIT RESOLVER WILL CHARGE YOU FOR, ONE RUN AT A TIME.* Before reading any lane’s credit rate,
check whether its runs share an identifier. **R51 WIDENS THE TEST RATHER THAN THE RULE (P74): also check
whether the lane’s runs are RE-DISPATCHED, because a unique id that is re-issued over time collides with
itself just as thoroughly.** Its corollary stands: the instrument built to catch this is the retro, and
the retro is a lane it happens to.
...
did not fire, or was re-registered. **Paid a seventh time this cycle by R47-4/R48-4.**
...
### RETRO-1789871155474 (2026-09-20) — two reconciled task ships; one newly logged

The merged-since-marker feed repeats W1-T3736/#6141 and newly logs W1-T3807/#6154. Both are
ledger-credit rescues of runs whose terminal verdict did not say `merged`.

- **W1-T3736** [#6141](https://github.com/craigoley/remudero/pull/6141) $0.000·8t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3807** [#6154](https://github.com/craigoley/remudero/pull/6154) $0.006·11t — newly logged ledger-credit rescue of a run observed `blocked_ci`.

### RETRO-1789863309633 (2026-09-20) — six reconciled task ships; all repeated

The merged-since-marker feed repeats six task/PR pairs already recorded. The run window reports five
`merged` verdicts, so the six ledger-credit rows remain reconciliation evidence rather than six new ship
identities.

- **W1-T3722** [#6096](https://github.com/craigoley/remudero/pull/6096) $0.000·2t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3784** [#6094](https://github.com/craigoley/remudero/pull/6094) $0.000·2t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3787** [#6099](https://github.com/craigoley/remudero/pull/6099) $0.001·49t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3735** [#6119](https://github.com/craigoley/remudero/pull/6119) $0.005·19t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3791** [#6129](https://github.com/craigoley/remudero/pull/6129) $0.000·1t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3736** [#6141](https://github.com/craigoley/remudero/pull/6141) $0.000·8t — repeated ledger-credit rescue of a run observed `incomplete`.

### RETRO-1789858570652 (2026-09-19) — seven reconciled task ships; one new identity

The merged-since-marker feed repeats six task/PR pairs already recorded and adds W1-T3736/#6141.
The run window itself reports only one `merged` verdict, so these are merge-credit reconciliation,
not seven successful run verdicts.
...
- **W1-T3735** [#6119](https://github.com/craigoley/remudero/pull/6119) $0.005·19t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3791** [#6129](https://github.com/craigoley/remudero/pull/6129) $0.000·1t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3736** [#6141](https://github.com/craigoley/remudero/pull/6141) $0.000·8t — newly logged ledger-credit rescue of a run observed `incomplete`.

### RETRO-1789853462330 (2026-09-19) — six reconciled task ships plus one gate-side triage ship

All 40 run verdicts are `incomplete`. The merged-since-marker feed repeats five task/PR pairs already
recorded and adds W1-T3791/#6129; the ownership-asserted shipped union additionally credits
TRIAGE-fb-1789845848265-8919ba/#6148 from current main.
...
- **W1-T3787** [#6099](https://github.com/craigoley/remudero/pull/6099) $0.001·49t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3735** [#6119](https://github.com/craigoley/remudero/pull/6119) $0.005·19t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3791** [#6129](https://github.com/craigoley/remudero/pull/6129) $0.000·1t — newly logged ledger-credit rescue of a run observed `incomplete`.
- **TRIAGE-fb-1789845848265-8919ba** [#6148](https://github.com/craigoley/remudero/pull/6148) $0.000·0t — gate-side union credit; the run ended `incomplete`.

### RETRO-1789847242565 (2026-09-19) — four repeated ledger credits plus one gate-side union ship

All 40 run verdicts are `incomplete`. The merged-since-marker feed repeats four task/PR pairs already
recorded by R61; the ownership-asserted shipped union additionally credits W1-T3735/#6119 from current
main, where its run ended `incomplete`.

- **W1-T3674** [#6005](https://github.com/craigoley/remudero/pull/6005) $0.019·8t — repeated ledger credit; the originating run was observed `blocked_ci`.
- **W1-T3722** [#6096](https://github.com/craigoley/remudero/pull/6096) $0.000·2t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3784** [#6094](https://github.com/craigoley/remudero/pull/6094) $0.000·2t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3787** [#6099](https://github.com/craigoley/remudero/pull/6099) $0.001·49t — repeated ledger-credit rescue of a run observed `incomplete`.
- **W1-T3735** [#6119](https://github.com/craigoley/remudero/pull/6119) $0.005·19t — gate-side union credit; the run ended `incomplete`.

### RETRO-1789844881088 (2026-09-19) — four repeated ledger-credit task ships; no new ship identity

All 40 run verdicts are `incomplete`. The merged-since-marker feed repeats four task/PR pairs already
recorded by R60, so they remain credited without being mistaken for new work.
...
`incomplete`×7 + `merged`×5 = 13.** Six NEW credits, with links (rule 13): **W1-T3669**
[#5770](https://github.com/craigoley/remudero/pull/5770) $0.405·131t — *the `diagnose` lane's first merge
and its first non-zero price* · **W1-T3665**
[#5768](https://github.com/craigoley/remudero/pull/5768) $0.403·116t · **W1-T3640**
[#5777](https://github.com/craigoley/remudero/pull/5777) $0.725·81t — *`recordFilingBranch`, shipped
UNWIRED by its own file fence* · **W1-T3697**
[#5794](https://github.com/craigoley/remudero/pull/5794) $0.673·69t · **W1-T3702**
[#5820](https://github.com/craigoley/remudero/pull/5820) $0.828·121t — all five `verdictSource=ledger-credit`
on `incomplete` runs — plus UNION-ONLY **RETRO** [#5769](https://github.com/craigoley/remudero/pull/5769)
$11.432·0t (**P47**, a fifth cycle). It **MINTED P80** (the advisory 180-second `gh` read floor stamped by
the BLOCKED attempt as well as the allowed one, four rounds refused, *159s → blocked → 12s → blocked → 20s
→ blocked → 33s → blocked → 47s*), recorded P44's falsifier RESET to 0 of 2 on the class's return to one
member, and scored R56 at **4 HIT · 1 MISS**.

### RETRO-1789567300802 (2026-09-16) — the merge column doubles and is drawn entirely from the failure column; a freshness row is falsified by its own arm: W1-T3622 + W1-T3623 + W1-T3626 + W1-T3629 + W1-T3639 + the gate-side RETRO plan-sync (5 ledger `merged` / **6 credited** / **6 NEW** / 1 union-only, across a ~9-hour window) — ★ FOLDED BY R57 (33 lines → 11)

★ 15 runs over a **~9-hour** window (marker 2026-09-16T04:37:02.332Z): **`blocked_ci`×1 + `incomplete`×9
+ `merged`×5 = 15**, the failure column 11 → 10. Six NEW credits, with links (rule 13): **W1-T3622**
[#5730](https://github.com/craigoley/remudero/pull/5730) $0.403·75t · **W1-T3623**
[#5733](https://github.com/craigoley/remudero/pull/5733) $0.744·135t · **W1-T3626**
[#5732](https://github.com/craigoley/remudero/pull/5732) $0.823·88t · **W1-T3629**
[#5734](https://github.com/craigoley/remudero/pull/5734) $0.416·74t · **W1-T3639**
[#5736](https://github.com/craigoley/remudero/pull/5736) $0.818·94t — all five `verdictSource=ledger-credit`
on `incomplete` runs — plus UNION-ONLY **RETRO** [#5727](https://github.com/craigoley/remudero/pull/5727)
$11.727·0t (**P47**, a fourth cycle). It **MINTED P79** (a REQUIRED check red on a wall-clock date over
`test/an-open-pr-does-not-rot-while-it-waits.test.ts`, repairable by no task in flight), **FALSIFIED its
own R55-5** freshness row — a hand-listed census that missed two stale blocks inside the section it
audited — recorded P44's first falsifier advance (0 of 15, since RESET), and scored R55 at **3 HIT · 1
MISS · 1 FALSIFIED**.

### RETRO-1789533558153 + RETRO-1789482309645 (2026-09-15 → 09-16) — dispatch volume halves out of a falsifier's own band and two gates refuse work that is already correct; then the failure column empties, a fired arm deletes this section's ranking and a proposal closes unfixed: W1-T3604 + W1-T3605 + W1-T3611 + W1-T3607 + the gate-side RETRO plan-sync, then W1-T3560 + W1-T3561 + W1-T3569 + W1-T3583 (3 then 4 ledger `merged` / 5 then 4 credited / 5 then 4 NEW / 2 then 0 union-only) — ★ TWO ENTRIES FOLDED INTO ONE BY R58 (34 lines → 16)

★ **R56's window: 15 runs / ~14 hours** (marker 2026-09-15T14:22:36.336Z): **`blocked_budget`×1 +
`incomplete`×10 + `merged`×3 + `no_pr`×1 = 15**, half the prior cycle's 30 — which put R53-1's falsifier
outside its own 25–40 band and RESET it. Five NEW credits, with links (rule 13): **W1-T3604**
[#5647](https://github.com/craigoley/remudero/pull/5647) $0.827·75t · **W1-T3605**
[#5646](https://github.com/craigoley/remudero/pull/5646) $0.750·90t · **W1-T3611**
[#5680](https://github.com/craigoley/remudero/pull/5680) $0.415·88t — all three `verdictSource=ledger-credit`
on `incomplete` runs — plus UNION-ONLY **W1-T3607**
[#5672](https://github.com/craigoley/remudero/pull/5672) $8.268·80t (the openweight ladder renamed to
`cash`) and **RETRO** [#5648](https://github.com/craigoley/remudero/pull/5648) $8.623·0t. It **MINTED
P78**, ROUTED the invalid `GH_TOKEN` to **P77**, and scored R54 at **4 HIT · 1 MISS**.
★ **R55's window: 30 runs / ~17 hours** (marker 2026-09-14T21:02:32.626Z): **`blocked_ci`×2 +
`incomplete`×24 + `merged`×4 = 30**, `blocked_budget` at 0 of 30 one cycle after 7 of 28. Four NEW
credits, with links: **W1-T3560** [#5567](https://github.com/craigoley/remudero/pull/5567) $8.568·101t
(`isMerged` threaded into `buildOpenPrViews`) · **W1-T3583**
[#5566](https://github.com/craigoley/remudero/pull/5566) $5.584·85t · **W1-T3561**
[#5583](https://github.com/craigoley/remudero/pull/5583) $15.230·166t · **W1-T3569**
[#5605](https://github.com/craigoley/remudero/pull/5605) $5.203·80t (the openweight provider on
`synthesis.inbox_draft`, whose consequence is now **80%** of that lane) — **all four
`verdictSource=ledger-credit` on runs that ended `blocked_ci`** (P47). **TWO pre-registered consequences
fired in one pass:** R53-4 MISSED a sixth time so **the LIVE RANKING was DELETED**, and **P75 CLOSED at
2 of 2 with its remedy unbuilt** — a closure R55 found refuted one cycle later. It **MINTED P77** and
scored R53 at **4 HIT · 1 MISS**.

### RETRO-1789419910391 + RETRO-1789371541053 (2026-09-14) — the failure column returns and its largest class is a resource; and the best-measuring window in the log credits nothing new: W1-T3559 + W1-T3564 + W1-T3573 + W1-T3579, then the gate-side RETRO-1789367285330 (4 then 16 ledger `merged`; 4 NEW then **0** NEW / 1 union-only) — ★ TWO ENTRIES FOLDED INTO ONE BY R55 (30 lines → 12)

★ **R53's window: 28 runs, ~12 hours** (marker 2026-09-14T08:37:22.689Z): `blocked_budget`×7 +
`blocked_ci`×4 + `incomplete`×13 + `merged`×4 = 28 — the `blocked_*` population RETURNED after the
log's first window with none, **`blocked_budget`×7 its single largest failure class**, spend **$99.311**
under a **$500/day** ceiling with no limit id, cap value or spend-at-block anywhere (**P44**, promoted
37 → 3). Four NEW credits, with links (rule 13): **W1-T3559** [#5519](https://github.com/craigoley/remudero/pull/5519)
$1.394·14t · **W1-T3564** [#5542](https://github.com/craigoley/remudero/pull/5542) $0.764·9t ·
**W1-T3573** [#5549](https://github.com/craigoley/remudero/pull/5549) $9.136·173t · **W1-T3579**
[#5562](https://github.com/craigoley/remudero/pull/5562) $0.850·150t — all four `verdictSource=ledger-credit`
(**P47**). It **MINTED P76**, DEMOTED P75 4 → 13, and recorded the thirty-fifth turn.
★ **R52's window: 40 runs, ~22 hours** (marker 2026-09-13T09:33:47.846Z): `incomplete`×24 +
`merged`×16 = 40 — **the first window in this log with NO `blocked_*` member at all**, and **22 pairs
listed with 0 new by set-difference**, every one inside credit this file already held. **The ONE new
credit was [#5499](https://github.com/craigoley/remudero/pull/5499)** ($15.395·0t) — the retro lane's own
plan-sync PR, union-only, and the single PR all 13 constant-key rejections named (**P69**). **It minted
P75** on one lane counted three ways, and recorded the strongest verdict column this log has published
beside a credit list whose new count was zero: **the thirty-fourth turn.**

### RETRO-1789367285330 (2026-09-14) — the primary credit path returns a full list and zero new information, and a SECOND lane proves uncreditable by a different mechanism: the gate-side RETRO-1789358574280 + six `TRIAGE-fb-*` merges (5 ledger `merged` / 27 pairs listed / **0** not previously credited / 7 union-only, across a ~30-hour window) — ★ FOLDED TO TWELVE LINES BY R52

★ 40 runs over a **~30-hour** window (marker 2026-09-13T00:18:29.024Z): `incomplete`×32 + `merged`×5 +
`blocked_containment`×2 + `no_pr`×1 = 40. **Its ledger half was empty by measurement** — 27 listed pairs,
**0** new — so all seven new credits arrived through W1-T51's union, with links (rule 13):
**RETRO-1789358574280** [#5483](https://github.com/craigoley/remudero/pull/5483) $17.909·0t — R50's own
plan-sync PR, and the single PR all 13 of that cycle's constant-key rejections named (**P69**) — and SIX
`TRIAGE-fb-*` gate-side merges, every one on a run that ended `incomplete`:
[#5382](https://github.com/craigoley/remudero/pull/5382) $0.251 ·
[#5401](https://github.com/craigoley/remudero/pull/5401) $0.366 ·
[#5416](https://github.com/craigoley/remudero/pull/5416) $0.338 ·
[#5417](https://github.com/craigoley/remudero/pull/5417) $0.451 ·
[#5426](https://github.com/craigoley/remudero/pull/5426) $0.261 ·
[#5492](https://github.com/craigoley/remudero/pull/5492) $0.592. An eighth union row, **W1-T3397**
[#5357](https://github.com/craigoley/remudero/pull/5357) $0.683·10t, falls inside the folded span.
**Its disposition:** R51 minted **P74** (*a second whole lane is uncreditable, by RE-DISPATCH rather than
by a constant key*), scored R50 at 4 HIT · 1 MISS, wrote the thirty-third turn, and executed R50-5's arm
by replacing its per-cycle corrective-task proposal with one standing DEBT line.

### RETRO-1789358574280 (2026-09-14) — a published total is smaller than one of its own members, and the credit list's selector is proven at three of three: W1-T3167 + W1-T3483 + W1-T3523 + the gate-side W1-T3505 and RETRO-1789349310781 (24 ledger `merged` / 58 pairs listed / 3 not previously credited, across a ~2.5-day window) — ★ FOLDED TO TWELVE LINES BY R51

★ 40 runs over a **~2.5-day** window (marker 2026-09-11T16:14:03.956Z): `merged`×24 + `incomplete`×14 +
`blocked`×1 + `blocked_containment`×1 = 40, the last of those a verdict class new to this log.
**NEW CREDIT, WITH LINKS (rule 13):** **W1-T3167**
[#5437](https://github.com/craigoley/remudero/pull/5437) $6.149·92t · **W1-T3483**
[#5445](https://github.com/craigoley/remudero/pull/5445) $3.844·76t · **W1-T3523**
[#5456](https://github.com/craigoley/remudero/pull/5456) $12.322·137t — each a
`verdictSource=ledger-credit` gate-side merge on a run separately observed `blocked_ci` or `blocked`
(**P47**). **Union-only gate-side additions:** **W1-T3505**
[#5448](https://github.com/craigoley/remudero/pull/5448) $4.068·88t · **RETRO-1789349310781**
[#5471](https://github.com/craigoley/remudero/pull/5471) $11.424·0t — R49's own plan-sync PR, and then
the single PR all 28 of that cycle's constant-key rejections named (**P69**). **Its disposition:** R50
minted **P73** (*a published total is smaller than one of its own members* — $3.907 for 27 runs beside
one in-window member at $12.322), scored R49 at 3 HIT · 1 MISS · 1 UNRESOLVABLE, wrote the thirty-second
turn, and took the largest compression pass in this log (six named deletions). 55 of its 58 listed pairs
were already credited here; the remaining span and extremes are one section lower.

### RETRO-1789341105445 + RETRO-1789349310781 (2026-09-13 → 09-14) — the proposal list is write-only, the retro lane's poison PR moves a third and fourth time, and the coverage stamp returns on two columns of five: W1-T3434 + W1-T3449 + W1-T3472 + W1-T3492 + W1-T3512 + W1-T3506 + the gate-side W1-T3442 and RETRO-1789341105445 (30 then 25 ledger `merged` / 116 then 104 pairs listed / 3 new in each) — ★ TWO ENTRIES FOLDED TO ONE BY R50, ON P70'S NOW-PROVEN EVIDENCE

★ Two adjacent 40-run windows: **~5.9 days** from marker 2026-09-08T02:37:47.959Z, then **~5.0 days**
from 2026-09-09T00:40:38.687Z. Censuses: `merged`×30 + `incomplete`×7 + `blocked_ci`×3 = 40, then
`merged`×25 + `incomplete`×12 + `awaiting_merge`×1 + `blocked`×1 + `blocked_ci`×1 = 40.
**★ WHY ONE ENTRY AND NOT TWO — THE FOLD IS THE MINT'S OWN EVIDENCE APPLIED TO THIS FILE, FOR THE
SECOND TIME.** R48 folded two entries on a 128-of-129 overlap. R49 measured 101 of 104, R50 measures 55
of 58, and shrink-on-advance is now **3 of 3** — so **P70's selector is PROVEN**, and two headers
publishing one credit population is no longer a suspicion but the mechanism. Keeping them apart would
be this document committing in structure the accretion it describes in prose.
**NEW CREDIT ACROSS BOTH WINDOWS, WITH LINKS (rule 13):** **W1-T3434**
[#5433](https://github.com/craigoley/remudero/pull/5433) $14.656·137t · **W1-T3449**
[#5441](https://github.com/craigoley/remudero/pull/5441) $2.841·42t · **W1-T3472**
[#5443](https://github.com/craigoley/remudero/pull/5443) $1.382·15t · **W1-T3492**
[#5446](https://github.com/craigoley/remudero/pull/5446) $0.762·19t · **W1-T3512**
[#5451](https://github.com/craigoley/remudero/pull/5451) $0.717·127t · **W1-T3506**
[#5454](https://github.com/craigoley/remudero/pull/5454) $8.800·143t — every one a
`verdictSource=ledger-credit` gate-side merge on a run separately observed `blocked_ci` or `incomplete`
(**P47**, rank 1). **Gate-side additions beyond the ledger's credit rows** (W1-T51's union):
**W1-T3442** [#5436](https://github.com/craigoley/remudero/pull/5436) $0.752·9t · **RETRO-1789341105445**
[#5455](https://github.com/craigoley/remudero/pull/5455) $9.012·0t — the retro lane's own plan-sync PR,
merged gate-side while its run ended `incomplete`, and then the PR all 42 of R49's constant-key
rejections named.
**Their dispositions:** R48 minted **P71** (*nothing converts a ratified proposal into a filed task*),
lifted P69 above P67 on a pre-registered arm, and folded two retro entries into one. R49 minted **P72**
(*the coverage stamp is per-column where the hole is per-row*), wrote **DR-18(i)**, and deleted R48's
one-cycle-old "repaired INSTRUMENT" claim rather than annotating it. Across the two windows the poison
PR moved twice (#5449 → #5455), the Discrepancies section ran 47 then 42 constant-key rejections, and
the foreign-lane credit hole held at 16 members while its MEMBERSHIP turned over — the first sighting of
P70's re-emission in the rejection list as well as the credit list.

### RETRO-1789326941957 + RETRO-1789332224777 (2026-09-13) — the ledger's own credit row becomes the primary credit path, then that credit list proves non-incremental: W1-T201 + W1-T2653–T3532 + W1-T3396 (13 then 21 ledger `merged` / 146 then 129 union pairs) — ★ TWO ENTRIES FOLDED TO ONE BY R48, ON P70'S OWN EVIDENCE

★ Two adjacent 40-run windows: **~10.7 days** from marker 2026-09-03T02:24:39.835Z, then **~6.6 days**
from 2026-09-07T06:05:19.656Z. Censuses: `incomplete`×26 + `merged`×13 + `blocked_ci`×1 = 40, then
`merged`×21 + `incomplete`×14 + `failed`×2 + `blocked_ci`×1 + `no_pr`×1 + `pr_attribution_failed`×1 = 40.
**★ WHY ONE ENTRY AND NOT TWO — THE FOLD IS THE MINT'S OWN EVIDENCE APPLIED TO THIS FILE.** R47's
measurement was that **128 of its 129 credited pairs were already in R46's list**. Two headers were
therefore publishing ONE credit population twice, which is precisely the defect **P70** names; keeping
them apart would have been this document committing in structure the accretion it describes in prose.
**THE CREDIT SPAN: #4316 → #5431**, across **W1-T201, W1-T2653–T2925, W1-T2999–T3100, W1-T3130–T3282,
W1-T3296–T3532 and W1-T3396**. The ledger's own credit rows and each subsequent gather's merge-state
join remain the authority for any individual pair — this file carries the span, the ledger carries the
row. Largest by cost:
...
**W1-T3396** [#5431](https://github.com/craigoley/remudero/pull/5431) $1.498·1t (R47's sole new pair).
...
[#5357](https://github.com/craigoley/remudero/pull/5357) · **RETRO-1789326941957**
[#5439](https://github.com/craigoley/remudero/pull/5439) — the retro lane's own plan-sync PR, merged
gate-side while its run ended `incomplete`, and then the PR all 59 of R47's constant-key rejections
named · five `TRIAGE-fb-*` merges ([#4159](https://github.com/craigoley/remudero/pull/4159), [#5382](https://github.com/craigoley/remudero/pull/5382), [#5401](https://github.com/craigoley/remudero/pull/5401), [#5417](https://github.com/craigoley/remudero/pull/5417), [#5416](https://github.com/craigoley/remudero/pull/5416), [#5426](https://github.com/craigoley/remudero/pull/5426)).
**Their mints:** **P69** (R46, on ~60 rejections all naming #5183 on `codex/retro-preflight-child-env`,
REST-verified MERGED 2026-09-12T00:49:43Z — the only out-of-band read either cycle spent) and **P70**
(R47, on the 128/129 overlap). R46 also promoted **P42** on its first exercised input in twenty-two
cycles and deleted P53's fourteen-cycle "no emission call site" clause as FALSE.
...
### RETRO-1788374498685 (2026-09-02) — eight `blocked_ci` verdicts, seven of them already merged: the dispatch-only decline, the self-referential follow-up refusal, the canonical-checkout drift measure, the DECLINE-verdict census, the gate-release condition, the worktree ledger fields, the reap-branches split, the base-record reap & the divergence cause: W1-T2613–T2629 (3 credited / 10 by union / 20 in-window `run-W1-*` merged) — ★ FOLDED TO TWELVE LINES BY R46

★ 11 runs / **~6.1 h** / **$62.200** — $5.655/run, $20.733 per ledger-credited ship, $6.220 per union
ship, $3.110 per hand-verified run-lane merge (DR-25: all four divide by ONE lane). Census: 3 `merged` +
8 `blocked_ci` = 11. **57 in-window merges verified BY HAND over REST** — 20 `run-W1-*`, 1 `run-RETRO-*`
(#3639), 36 hand-named, FIVE of those changing `src/` or `test/`.
**Ships:** the follow-up router refuses self-referential proposals ([#3659](https://github.com/craigoley/remudero/pull/3659)) · a recon cannot propose a follow-up naming its own task ([#3661](https://github.com/craigoley/remudero/pull/3661)) · `worktreeAdd` records canonical-checkout drift ([#3662](https://github.com/craigoley/remudero/pull/3662), [#3680](https://github.com/craigoley/remudero/pull/3680)) · the sweep releases on the gate rather than on `behind` ([#3673](https://github.com/craigoley/remudero/pull/3673)) · `rmd reap-branches` splits named from credited ([#3681](https://github.com/craigoley/remudero/pull/3681)) · `pruneStaleRuns` reaps the `.base` record ([#3697](https://github.com/craigoley/remudero/pull/3697)) · `resolvePlanCriteriaAtHead` reports its divergence cause ([#3692](https://github.com/craigoley/remudero/pull/3692)) · the dispatcher declines a dispatch-only prompt ([#3651](https://github.com/craigoley/remudero/pull/3651)) · the daemon-policy census records its own DECLINE ([#3666](https://github.com/craigoley/remudero/pull/3666)) · the worktree-base provisioning audit, **P63's exercised case** ([#3672](https://github.com/craigoley/remudero/pull/3672)).
**Credited by nothing — TEN, every one named (TASK G):** six malformed epochs (#3657/T2703, dated ~6.4 h
AFTER its own merge; #3688/T2717, #3685/T2713, #3675/T2709; #3671/T2712 and #3676/T2692 at SECOND
precision), two straddlers (#3629/W1-T2610 landing, #3692/W1-T2623 leaving) and two DAEMON fix-rung
branches (#3663/W1-T2704, #3684/W1-T2711). **Nothing was rejected, which is P48's shape and not a
repair.** Six ships name their own missing wire; the DARK roster in NET STATE carries them.
...
### RETRO-1787858337550 / RETRO-1787828128305 (2026-08-27) — the label that was wrong thirteen times, then the credit that was fabricated: the fix rung's message-set comparison & the ledger read intent, then the resolver-divergence detector, the provenance reader & the repeat-escalation rung: W1-T2299–W1-T2393 (2 credited / 3 by union / 7 in-window merged, then 8 / 14 / 15) — ★ TWO ENTRIES FOLDED TO ONE BY R36, AND ITS SURVIVING R33 HALF — THE LONGEST ENTRY LEFT IN THIS TAIL — FOLDED TO TWELVE LINES BY R50
...
★ **R33 (…828128305), FOLDED TO TWELVE LINES BY R50 — the longest entry in this tail at 57 lines,
standing between neighbours of 10 and 14, and the same disposition R49 applied to the entry directly
below; no PR link is lost.** 24 runs, ~13.2 h (marker 2026-08-26T21:40:13.368Z), **$131.824**
($5.493/run); census 8 `merged` + 8 `blocked_ci` + 3 `blocked_containment` + 3 `no_pr` + 2 `blocked` =
24. The merge set was hand-verified over REST **and every credited PR's diff was read** — the first such
cycle in this log (DR-4, DR-16): 15 in-window `run-W1-*` merges, 13 of them code, TWO plan-only.
**LEDGER-CREDITED — 8, $51.729:** W1-T2345 *$9.618* [#3048](https://github.com/craigoley/remudero/pull/3048) · W1-T2346 *$9.172* [#3046](https://github.com/craigoley/remudero/pull/3046) · W1-T2299 *$7.488* [#3022](https://github.com/craigoley/remudero/pull/3022) · W1-T2315 *$7.267* [#3049](https://github.com/craigoley/remudero/pull/3049) · W1-T2314 *$5.801* [#3035](https://github.com/craigoley/remudero/pull/3035) · W1-T2303 *$5.313* [#3034](https://github.com/craigoley/remudero/pull/3034) · W1-T2306 *$4.202* [#3045](https://github.com/craigoley/remudero/pull/3045) · W1-T2312 *$2.868* [#3033](https://github.com/craigoley/remudero/pull/3033).
**UNION-RESCUED (gate-side merges the ledger called `blocked_ci`) — 5, $26.401:** W1-T2305 *$8.613* [#3040](https://github.com/craigoley/remudero/pull/3040) · W1-T2344 *$6.383* [#3030](https://github.com/craigoley/remudero/pull/3030) · W1-T2333 *$3.934* [#3015](https://github.com/craigoley/remudero/pull/3015) · W1-T2335 *$3.849* [#3038](https://github.com/craigoley/remudero/pull/3038) · W1-T2319 *$3.622* [#3014](https://github.com/craigoley/remudero/pull/3014).
**★★ FALSELY CREDITED — 1, AND IT IS THE CYCLE'S FINDING, KEPT LABELLED RATHER THAN DELETED:** W1-T2318
*$5.102/76t* recorded SHIPPED via [#3059](https://github.com/craigoley/remudero/pull/3059), **whose
entire diff is one `plan/tasks.d/` shard** amending the plan to record that the real work merged in
[#2972](https://github.com/craigoley/remudero/pull/2972), an untrailered hand-named branch no resolver
can reach — after THREE dispatches of the same task. **That is the only instance of the defect P56 was
minted on, which is why the line survives the fold.** PRINTED-BUT-UNCREDITED — 1
([#2971](https://github.com/craigoley/remudero/pull/2971), plan-only); STRADDLERS — 1 (the same PR,
−6.58 h, its cost in R31's population, rule 7); ABSENT RUNS — 0. Three-valued cost bracket $16.478 /
$9.416 / $8.788; residue 61% pre-union, 37% post; turn coverage 17%; 3 guard-fired containment blocks
counted as ONE host storm (rule 8); mutation N=0; replay dark.

### RETRO-1787778937848 (2026-08-26) — the residue that is all seam: the fix rung's stand-down, the prompt fingerprint & the replay verb: W1-T2293–W1-T2325 (4 credited / 9 by union / 12 in-window merged) — ★ FOLDED TO TWELVE LINES BY R49; its claim to be "THE LAST UNFOLDED ENTRY IN THIS TAIL" is STRUCK BY R50, which found a 57-line entry two headers above it

★ 18 runs / **~7.6 h** / **$60.081** ($3.338/run); census `blocked_containment`×8 + `blocked_ci`×6 +
`merged`×4 = 18, above a body naming 23 run ids — the excess is exactly five `DAEMON-*` lane runs the
census excludes, **unlabelled rather than miscounted** (TASK K's remedy is one label per population).
**12 in-window merges hand-verified over REST** (DR-4): the ledger credits 4 (33%), W1-T51's union 9
(75%), five of them gate-side rescues. **Residue $39.651 / 66% of the window bought no ledger-credited
ship** — a third consecutive majority — falling to $1.281 / 2.1% after the union.
**Ships (ledger-credited):** negative-arm recogniser fixtures ratcheted ([#2987](https://github.com/craigoley/remudero/pull/2987)) · diff-coverage stops blocking RELOCATED lines ([#2990](https://github.com/craigoley/remudero/pull/2990)) · the prompt parts a worker actually received are fingerprinted ([#2996](https://github.com/craigoley/remudero/pull/2996)) · a deterministic ledger-read-count invariant ([#2964](https://github.com/craigoley/remudero/pull/2964)).
**Union-rescued:** rollup start times + stale-check arm ([#3001](https://github.com/craigoley/remudero/pull/3001)) · the fix rung stands down on an unread-body infrastructure fault ([#2979](https://github.com/craigoley/remudero/pull/2979)) · the daemon stops booting on the operator's personal credential ([#2967](https://github.com/craigoley/remudero/pull/2967)) · `rmd replay` narrates a ledger window ([#2986](https://github.com/craigoley/remudero/pull/2986)) · the acceptance-repair trigger widened ([#2977](https://github.com/craigoley/remudero/pull/2977)).
**Printed-but-uncredited — 1** (W1-T2304/[#2952](https://github.com/craigoley/remudero/pull/2952)), whose harvest line pairs the PR with `DAEMON-1787749707742` rather than the `run-W1-T2304-…` branch that
merged — which is exactly why the join failed. **Straddlers — 3**, deepest **−13.49 h**
(W1-T2268/#2895), the largest per-run straddler this log has stated. **Turn coverage 6%** (107 turns on
one run, itself a union rescue); **guard fires 8**, one host storm counted once (rule 8), 0 of 8
carrying T2238/#2794's `(numTurns, maxTurns)` pair; mutation gate **N=0**; replay **no run recorded**.
...
### RETRO-1787714349337 / RETRO-1787654213224 (2026-08-25 → 08-26) — the evidence under the verdict, the reaper's scope & the ledger's full corpus, then the probe's own budget, the containment anchors & the recon artifact: W1-T2211–W1-T2279 (14 credited then 18 / 27 + 28 in-window merged) — ★ TWO ENTRIES FOLDED TO ONE BY R31, AND THAT FOLD — 42 LINES — FOLDED TO TWELVE BY R52

★ 92 runs across both, all `implement`/`src`, over ~16.3 h then ~20.6 h. **$360.491** ($3.92/run). Peaks
**W1-T2257 $12.083/0t** and **W1-T2213 $11.950/0t**. Credit **14 of 27 (52%) with the union rescuing
NOTHING — the first such cycle in five** — then **18 of 28 (64%), union 20 (71%)**, both hand-verified over
REST at the marker (DR-4). **★ THE RESIDUE FIRST INVERTED HERE:** $87.114 credited against $101.172
uncredited, 54% of the window buying no credited ship. Guard-fired blocks **18** then **16**, all
`outside-cwd-denial` / `turns-exhausted` across ELEVEN tasks each — ONE host storm per cycle, counted once
(rule 8), with 9-of-11 then 10-of-11 storm-hit tasks merging anyway, so both fold-line readings are
**UNCOMPUTABLE without TASK H** rather than estimated (P48). Mutation N=0; replay *no run recorded*.
**★ THE 53 CREDITED, RESCUED AND PRINTED-BUT-UNCREDITED PRs — #2714–#2898 — ARE PRESERVED AS A SPAN,
their per-task lines DELETED BY R52** (R13's doctrine). **FIVE ships are named because the DARK roster
still indicts them:** **T2231/#2801** (`dispatchStarted` populated by test fakes only), **T2235/#2786**
(third review state, `REVIEW_STATE_LABELS` still has no badge), **T2212/#2730** (bounded-retry risk judge,
still unwired in live dispatch), **T2216/#2765** (`NETWORK_KEYS`/`FILESYSTEM_KEYS` over-strict by one
level), **T2238/#2794** (the probe's `(numTurns, maxTurns)` pair, TASK N's subject, still 0 guard rows).
**Its structural finding survives the fold: 0 foreign trailers over ~66 observations** — the count that
made R25 strike the word — beside ONE correct sibling rejection that cost nothing.

### RETRO-1787578143187 / RETRO-1787502627029 (2026-08-23 → 08-24) — the credit surface, the ratchets' type discipline & the doctor's sweep-liveness arm, then the containment storm, the ledger's declared intent & the push lease: W1-T1207–W1-T2206 (25 credited then 17 / 31 + 31 in-window merged) — ★ TWO ENTRIES FOLDED TO ONE BY R30, AND THAT FOLD — 45 LINES — FOLDED TO TWELVE BY R52

★ 83 runs across both, all `implement`/`src`. **$377.312** ($4.55/run). Peaks **W1-T1227 $15.623/0t**
and **W1-T1282 $12.059/0t**. Credit **14 of 31 (45%), union 25 (81%)** then **17 of 31 (55%), union the
SAME 17 — the first cycle in this log's history in which the union rescued NOTHING**, both hand-verified
over REST at the marker (DR-4; the first forced onto REST by a GraphQL bucket exhausted at 0 of 5000).
Guard-fired blocks **0** then **23** — ONE host storm, counted once (rule 8). Mutation N=0 in both;
replay *no run recorded*. **★ THE 53 CREDITED AND RESCUED PRs — #2376–#2710 — ARE PRESERVED AS A SPAN,
their per-task lines DELETED BY R52** (R13's doctrine). **FIVE ships are named because later cycles still
depend on or indict them:** **T1281/#2685** made the containment guard NAME its unproven state — the merge
that broke R27's *constraint is a law of nature* reading; **T2204/#2691** shipped `mainHealthFromRollup`,
for three cycles the only line SHIPS-UNWIRED returned; **T1214/#2520** shipped `credit-surface-gate` with
no CI job step, still; **T1263/#2657** shipped a state-citation gate script never wired into CI, still;
and **T1249/#2612** shipped the bounded promotion judge — the instrument that ended a six-cycle dark leg,
itself credited to no cycle. **Its two structural findings survive the fold:** the 08-24 fold-line is
UNCOMPUTABLE without TASK H (18 of 23 guard rows belong to tasks that merged anyway — ONE event, never 18
readings, rule 8), and five ABSENT RUNS (#2526, #2529, #2532, #2602, #2604) put P51's quarantine list at 52.

### RETRO-1787290856852 / RETRO-1787419805720 (2026-08-21 → 08-22) — the containment storm + attribution spine + review/proof lane, then the fix rung's park/rebase, the sweep's stand-down vocabulary & the doctor's repair arm: W1-T1016–T1209 (29 credited / 40 merged, then 27 / 33) — ★ TWO ENTRIES FOLDED TO ONE BY R30, AND THAT FOLD — THE LONGEST ENTRY LEFT IN THIS TAIL AT 53 LINES — FOLDED TO TWELVE BY R52

★ 95 runs across both, all `implement`/`src`. **$506.935** ($5.34/run). Credit **29 of 40 (73%)** then
**21 of 33 (64%), union 27 (82%)** — both all-time highs, hand-verified at the marker (DR-4). Peaks
**W1-T1044 $37.084/0t** then **W1-T1082 $12.323/0t**; three of the second window's merges are
**STRADDLERS** whose runs and costs sit in NO window's total, the first time this log carried that class.
Guard-fired blocks **13**, all `outside-cwd-denial` inside a **41-minute band on one host** — **the event
that minted DR-8** — then 2 `isolation/inherited-functions`. Rejected trailers 22 then 5, **ZERO foreign in
both, which fired R24-2 and STRUCK the word `foreign`**. Mutation N=0 in both; replay *no run recorded*.
**★ THE 56 CREDITED PRs — #2265–#2515 — ARE PRESERVED AS A SPAN, their per-task lines DELETED BY R52**
(R13's doctrine: ids and PR numbers survive a fold, per-task costs and descriptive prose do not; git holds
them). **FOUR ships are named because later cycles still depend on or indict them:** **T1202/#2515** moved
harness PR creation off GraphQL `--fill` onto REST, the transport every later hand-verification uses;
**T1209/#2511** shipped `judgeRepairStall` with no production reader, so `rmd doctor` still prints
`repair-stall: OK` unconditionally; **T1038/#2321** shipped the fail-open memory governor UNWIRED by
design, still without a `DaemonDeps`/`DrainDeps` wire; and **T1085/#2357** carried its trailer in the
COMMIT MESSAGE ONLY — TASK G(iv)'s origin. **The uncredited-run remainder INVERTED across the pair:**
$0.87/run against a $4.504 window average (13 of those 25 runs killed by the containment guard — the
price of a run that could not spend, never thrift: DR-8), then $7.85/run against a $6.432 average.

### RETRO-1787193680272 / RETRO-1787106875391 (2026-08-19 → 08-20) — the pacer spine + credit predicate + diagnose lane, then arm integrity + the id allocator + the self-harness leg (18 credited / 31 merged, then 17 credited / 30 merged) — ★ TWO ENTRIES FOLDED TO ONE BY R28, AND THAT FOLD — 44 LINES — FOLDED TO TWELVE BY R52

★ 63 runs across both, all `implement`/`src`. **$360.029** ($5.71/run). Peaks **W1-T7B $10.593/144t**
and **W1-T949 $14.890/0t**. Turns 28% then 47%, the dark rows a contiguous PREFIX in BOTH windows — the
reading that killed R21's gate-side-credit shape. **0 guard-fired blocks in either window**; mutation N=0;
replay *no run recorded* — the suite shipped in the second window, dark. **★ THE 53 CREDITED, RESCUED AND
HAND-RECOVERED PRs — #2049–#2263 — ARE PRESERVED AS A SPAN, their per-task lines DELETED BY R52** (R13's
doctrine). **SIX ships are named because later cycles still depend on or indict them:** **T7B/#2178**
wired `runDiagnoseThenRetry` into implement dispatch, ending five cycles of zero `diagnose` runs;
**T1004/#2152** shipped the plan-only merge-credit predicate that makes six of that window's uncredited
merges CORRECT; **T74/#2186** shipped next-unused proposal ids plus a CI uniqueness gate — the discipline
every later P-mint is checked by; **T165/#2232** shipped the golden-task replay suite MECHANISM-AND-SEAM
ONLY, R23's sting and **still dark today**; **T1012/#2240** committed the `Remudero-Task` trailer so
squash-merge keeps it; and **T71/#2182** — P17's ratified `rmd receipt <pr>` — STRADDLED, belonging to no
cycle, left uncredited by six consecutive syncs. **Its structural finding survives the fold:** thirteen of
eighteen PRs merged-but-credited-by-nothing had their URL printed in their OWN gather's follow-up harvest,
which R21-2 and R22-2 both predicted at zero — the ancestor of R27-1/R28-1.

### RETRO-1786966159317 / RETRO-1786867677764 (2026-08-16 → 08-17) — the rate-limit floor, the sweep transport & the identity seam, then the review three-state, the install root & the retro instrument's own repair: W1-T502–T533 + T901/T903 + T905–T936 (14 credited / 30 merged, then 19 / 31) — ★ TWO ENTRIES FOLDED TO ONE BY R33, AND THAT FOLD FOLDED AGAIN TO TWELVE BY R52

★ 31 runs then 27, all `implement`/`src`. **$198.797** ($6.413/run) then **$182.927** ($6.775/run); peak
credited runs **W1-T504 $26.380** and **W1-T913 $16.345/195t**. Turns 4 of 14 lit (29%) then **15 of 19
(79%) — the high-water mark of the whole series.** Mutation-gate N=0 in both. **The 08-16 window is where
this log first carried ships the instrument never named** — 16 of 30 recovered by hand with
`gh pr list --state merged` at the marker. **★ THE 61 CREDITED, RESCUED AND HAND-RECOVERED PRs —
#1856–#2037 — ARE PRESERVED AS A SPAN, their per-task lines DELETED BY R52** (R13's doctrine). **FIVE are
named because later cycles still turn on them:** **T529/#1933** shipped the `gh` pace FLOOR **DARK**;
**T531/#1929** the Cloudflare Access identity seam; **T525/#1927** the metered `gh` seam; **T934/#2024**
the REQUIRED licence gate; and **T513/#1888**, whose branch names the task while its body carries NO
trailer — a P33 shape, and the ancestor of every later credit-resolver finding. **Its structural finding
survives the fold:** at least 7 of the first window's 17 uncredited runs own one of the merges above,
which is why R20's apparent 1-in-13 `blocked_*` ratio was an ARTIFACT and not a failure rate.
...
**★ FOLDED TO FAMILY LINES BY R13, AND TO SIX LINES BY R50 — every id and PR number preserved,
descriptive clauses deleted.** **P19 CLOSED** — parallel dispatch, all three rungs (W1-T170/#888 ·
T171/#890 · T172/#896, $19.010; rung 2 stays BANKED until a rung-1 escape appears in the ledger) ·
**P34 CLOSED** — presence-aware autonomy (W1-T248/#903 · T249/#907 · **T250/#898, the per-model-class
weekly-burn accounting the Calibration model table runs on** · T251/#899, $30.386) · **P37 CLOSED** —
the Tier-1 policy file (W1-T252/#901 · T253/#921, $25.801) · ESCALATION INTEGRITY (W1-T195/#969 ·
T196/#972 · T186/#931 · T198/#933, $39.256) · DECISIONS + WRITE-PATH HYGIENE (W1-T191/#966, $23.119) ·
REMAINDER, 11 (W1-T168/#884 · T174/#917 · T175/#923 · T176/#926 · T180/#928 · T202/#892 · T225/#935 ·
T236/#936 · T237/#938 · T238/#940 · T261/#885, $106.747) — **W1-T202/#892 at $35.384 is still the
second-costliest run ever recorded, behind R11's W1-T193 by $0.259.**

### RETRO-1785341166059 (2026-07-29) — the gate-integrity ladder + onboarding + the flywheel (94 tasks shipped) — ★ FOLDED TO TEN LINES BY R53, ITS PR NUMBERS PRESERVED AS SPANS
...
**★ R13 folded this entry to ten family lines; R53 folds those lines to their SPANS** — the same
treatment R52 applied to the five longest entries in this log's tail, and for the same reason: after
eight weeks the per-family id lists are a census no later cycle has cited, while the spans and the four
ships that ARE still cited remain. **THE FAMILIES (tasks → $):** gate/review integrity 13 → $67.218 ·
proof-dialect + plan-lint 6 → $68.406 · brownfield onboarding 4 → $43.563 (**P24 CLOSED**) · learning
flywheel + miners 8 → $111.191 (**P12/P13/P14/P18/P20/P23 CLOSED**) · sweep/reconciler 12 → $80.425 ·
daemon durability + worker spawn 11 → $141.752 · ledger/inbox/write atomicity 12 → $95.159 · console +
digest 10 → $176.278 (the cycle's costliest arc) · CI/commit/mount hygiene 11 → $58.619 · docs +
operator surface 7 → $25.375. **PR SPAN: #479–#870.** **Named individually because later cycles still
cite them:** **T167/#606** (model/effort routed by task risk and class — the row the Calibration table
exists to measure) · **T160/#853** (the retro's own cadence trigger) · **T254/#720** (the work PR R13's
four no-op closes keep rediscovering) · and the three PRs whose entire content is a no-op close of a
stale re-dispatch — **W1-T7/#772, W1-T12a/#725, W1-T99/#731** — **P39's ground truth.**


### RETRO-1784512714705 / …556575522 / …626054083 (2026-07-19 → 07-21) — the knowledge holes, the W3 panel/console/intake lane and the console-instrument + ratification-inbox + governor cycle (28 + 21 + 23 merges) — ★ THREE ENTRIES FOLDED TO ONE BY R25, THEIR PER-DAY ID LISTS FOLDED TO SPANS BY R53

★ These three consecutive July cycles were already family-folded by R12/R14/R16 and header-folded by
R25; **R53 folds the three surviving per-day id lists to SPANS**, on R52's precedent and for its reason —
the lists were a census no later cycle has cited, while the findings and the individually-cited ships
are kept. **THE FINDINGS THAT SURVIVE:** (1) **28/28 of 07-19's merges were GATE-SIDE and the run-verdict
column was wrong every time** — still P30's ground truth, and W1-T51/#97's union half is the sole reason
those entries exist at all; (2) 07-20 posted **the first non-zero credit column since R7 (13 of 21)**;
(3) 07-21 recorded the **inversion that mined P35 (now folded into P38): only 8 of 23 were
ledger-credited and W1-T150/#358, the fix for exactly that, merged FIRST.** Two live organs are named
here and nowhere else: **W3-T8/#305's `panel-skill-run` POST route is still unregistered** (a P38
instance now ten cycles old), and **T111/#373's approval telemetry has read `0/0/0%` or a byte-identical
constant every cycle since** (P40(ii)) — **and this cycle it reads 0/0/0% again, on a window whose row
count MOVED**, which is where that thread is answered.
- **2026-07-19 — 28 merges, ALL gate-side, $151.952. PR SPAN #194–#282.** The knowledge-hole family
  W1-T29–T40 ($65.597 · 954t; **W1-T29 took ×10 redispatches before credit** — P29's exemplar), the
  fleet/quality remainder, the descriptionless trio, the W2/W3 remainder — and **W1-T1/#255**, the task at
  the centre of the redispatch storm (~130 dispatches, ONE owned merge).
- **2026-07-20 — 21 merges, 13 credited, $171.534. PR SPAN #291–#346.** The W3 panel + plan verbs (peak
  **W3-T5/#300**, $16.141/154t), the `rmd serve` console family, the intake lane, the W3-T2 dashboard,
  security/hygiene and triage/plumbing — **W1-T52/#308 the FIRST `diagnose`-typed run ever to reach
  calibration**, and **W3-T8/#305** the unregistered route named above.
- **2026-07-21 — 23 merges, 8 credited, $247.388. PR SPAN #358–#457.** Floor + verdict integrity,
  including **T128/#414 — THE DEAD PROOF FLOOR, 101 of 126 runnable-dialect proofs that could never
  execute**; the console/live-state family; **the P25 ratification inbox end to end** (W1-T110/#368,
  **T111/#373**, T192/#457) → ★ **P25 CLOSED**; credit + the plan-PR emitter (**W1-T150/#358**); and the
  layered-knowledge + governor pair.
...
## Calibration (observed — current R66 first; prior-cycle evidence follows)

**RETRO-1789871155474 (2026-09-20) — observed task-type calibration. THE GATHER'S OWN TABLE, PRINTED
...
|---|---|---|---|---|---|
| implement | 9 | 2 | $0.041 | 20.111 | $0.366 |
| retro | 6 | 0 | $0.000 | 0 | $0.000 |
| triage | 14 | 0 | $0.000 | 0 | $0.000 |

**R66 reads:** the task-type table totals 29 runs and $0.366 notional, with both merges in the
implement lane. The corresponding `src` task-class row reads 2 of 9 merged (22%); its $0.122 per merge,
60.333 turns/merge and 69,323.667 output tokens/merge use the stated shipped source n=3. Weekly burn attributes all 1,118 measured turns
to sonnet while 91 unresolved runs carry zero measured turns. G-17 is historical and notional: 332
Architect-lane rows / $0.79, or 28.2% of 409 rows / $2.79; its newest row is already stale, so no tier
or mount row moves on it. No replay run occurred and no mutation verdict exists, so neither absence is
a pass rate.

**Prior R65 follows as historical evidence.**

**RETRO-1789863309633 (2026-09-20) — observed task-type calibration. THE GATHER'S OWN TABLE, PRINTED
EXACTLY AS PRODUCED. This is the row W1-T5's mount table needs; P65's existing freeze still forbids a
mount re-base from a window reading alone.**

| task_type | runs | merged | avg $ | avg turns | total $ |
|---|---|---|---|---|---|
| implement | 14 | 5 | $0.030 | 25.143 | $0.424 |
| retro | 7 | 0 | $0.000 | 0 | $0.000 |
| triage | 19 | 0 | $0.000 | 0 | $0.000 |

**R65 reads:** the task-type table totals 40 runs and $0.424 notional, with all five merges in the
implement lane. The corresponding `src` task-class row reads 5 of 14 merged (36%), $0.085 per merge,
70.4 turns/merge and 69,295.4 output tokens/merge. Weekly burn attributes all 3,519 measured turns to
sonnet while 109 unresolved runs carry zero measured turns. G-17 is historical and notional: 443
Architect-lane rows / $33.03, or 36.3% of 556 rows / $91.11; its newest row is already stale, so no tier
or mount row moves on it. No replay run occurred and no mutation verdict exists, so neither absence is a
pass rate.

**Prior R59 follows as historical evidence; the superseded R60–R64 calibration blocks were deleted by
R62–R66.**
...
| task_type | runs | merged | avg $ | avg turns | total $ |
|---|---|---|---|---|---|
| implement | 26 | 18 | $0.916 | 59.808 | $23.827 |
| recon | 1 | 1 | $0.657 | 103 | $0.657 |
| retro | 1 | 0 | $17.646 | 0 | $17.646 |
| triage | 12 | 0 | $0.000 | 0 | $0.000 |

**R59 reads:** the task-type table totals 40 runs and $42.130 notional. The corresponding task-class
view reports `src` 18 of 26 (69%), `docs` 1 of 1, `retro` 0 of 1, and `triage` 0 of 12. Only `docs` and
`src` have enough successful work to state a merge result; `retro`'s class row disagrees with its shipped
source, while closure refuses `retro`, `triage`, and `unknown` below P48's population floor. Weekly burn
is sonnet-only among attributed turns (5,805 turns); 119 unresolved runs contribute zero measured turns,
not proof of zero use. G-17 is historical and notional: 607 Architect-lane rows / $80.33, or 29.3% of
774 rows / $273.74, with `retro.synthesized` carrying nearly all Architect-lane dollars. No replay run
occurred and no mutation verdict exists, so neither absence is a pass rate.

**Prior R58 follows as historical evidence; its detailed prose is retained only until the next tail fold.**

**RETRO-1789620579356 (2026-09-17) — observed task-type calibration. THE GATHER'S OWN TABLE, PRINTED
...
| task_type | runs | merged | avg $ | avg turns | total $ |
|---|---|---|---|---|---|
| implement | 14 | 7 | $0.858 | 71.857 | $12.013 |
| retro | 2 | 0 | $13.734 | 0 | $27.469 |
| triage | 8 | 0 | $0.000 | 0 | $0.000 |

**★ THE TABLE SUMS TO $39.482 OVER 24 RUNS, AND THE LANE THAT ESCAPED THE EXACTLY-ZERO SHAPE LAST CYCLE
IS GONE FROM THE TABLE ENTIRELY.** `diagnose` entered two cycles ago at 1 run / $0.000 / 0 turns, left the
zero last cycle at 1 run / 1 merged / $0.405 / 131 turns, and **contributed no runs at all this window**.
`triage` does not move: **8 runs at EXACTLY $0.000, an EIGHTH consecutive cycle priced to the cent**,
across populations of 20, 4, 6, 5 and 8 (**P74**). ★ The pair is now the entry's cleanest statement:
**the constant-price class has one confirmed escape and one member that has never moved, and the escape
left the table before a second reading could confirm it** — a lane that leaves is not a lane repaired.
`implement` reads **14 runs / 7 merged (50%) / $0.858 / 71.857 turns / $12.013** — its cheapest per-run
average in this series by a wide margin, over its largest population since R44; `retro` reads **2 runs at
$13.734 each**, 16× the whole `implement` row's per-run average.
**★ AND THE TWO-RATE PUBLICATION READS A GAP OF ZERO FOR THE FIRST TIME — WITHOUT ANYTHING BEING JOINED.**
The type table's `implement` row reads **7 of 14 = 50%**; the class table's `src` row reads **7 of 14 =
50%**. In the two preceding windows the same pair read 67%/71% and 71%/63%, the gap changing sign; here it
is **zero**, and the reason is membership, not measurement: the class mapping folds `diagnose` into `src`,
and `diagnose` had no runs. **A defect that becomes unobservable when one of its inputs empties has not
been repaired** (**P73**, HELD; rule 18), and **R58-2 now requires the folding class's membership to be
stated whenever the gap reads zero**, so this cycle's reading can never be quoted as an absence.
**What must NOT be read across the cycles is the movement:** $5.674 → $1.772 → $2.631 → $2.324 →
**$0.858** and 95.444 → 87.667 → 98 → 111.333 → **71.857** are adjacent windows of 9, 9, 7, 6 and 14 runs
cut on different markers. Rule 11 forbids sizing anything on a series whose denominator has halved twice
and then more than doubled.

**BY TASK CLASS — the W1-T167 routing question (is the class-routed mount discount paying off?). ★ THE
ANSWER IS STILL NO: `docs` climbs to 42 open and has merged nothing, a forty-sixth cycle.**

| task_class | runs | merged | merge rate | avg $ | avg turns | total $ | output tokens | merge source | turns/merge | output tokens/merge |
|---|---|---|---|---|---|---|---|---|---|---|
| retro | 2 | 0 | **0%** ⚠ **NOT A MERGE RATE — a credit artefact.** The lane keys on the constant `RETRO` (**P69**) and one of its two runs merged gate-side, into every column but this one | $13.734 | 0 | $27.469 | 0 | shipped (n=1) ⚠ **DISAGREES WITH ITS OWN ROW** | 0 ⚠ 0% coverage — DO NOT USE | 0 ⚠ 0% coverage — DO NOT USE |
| src | 14 | 7 | **50%** ⚠ **the SAME seven merges the type table publishes as 50% — the first ZERO gap this pair has recorded, and it is a membership coincidence** | $0.858 | 71.857 | $12.013 | 593770 | shipped (n=7) ✓ **AGREES** | 143.714 | 84824.286 |
| triage | 8 | 0 | **0%** ⚠ **NOR IS THIS ONE — a credit artefact by a DIFFERENT mechanism** (**P74**) | $0.000 | 0 | $0.000 | 0 | shipped (n=0) ✓ **AGREES** | n/a (0 merges) | n/a (0 merges) |

**★ THE THREE MERGE-COUNT COLUMNS REPEAT LAST CYCLE'S PATTERN A THIRD TIME.** `src` **7 / 7 / 7 AGREED**,
`triage` **0 / 0 / 0 AGREED**, `retro` **0 / 1 / 1 DISAGREED**; the derived columns divide by the 7 the row
states (1006/7 = **143.714**, 593,770/7 = **84,824.286**). R57-2's retirement clause asks for **two
consecutive ALL-AGREE cycles**, and `retro` has now disagreed in **three consecutive readings**, so the
counter stands at **0 of 2** with three opportunities spent. ★ Note what the `src` row's AGREEMENT is
worth this cycle: it agrees with two columns *and* with the type table, and the one thing that changed —
the vanished `diagnose` lane — is invisible in every one of them. **Three agreeing columns cannot see the
population that left.**
**★ P72 HOLDS — THE COVERAGE STAMP IS ABSENT A SIXTH CONSECUTIVE CYCLE.** The closure table again REFUSES
two rows outright on a population floor (`retro` at n=1, `triage` at n=0, both *"below floor 5, P48"*) —
the right output, and still not a coverage stamp; `src`'s five derived columns go out unmarked.
**★ THE CLOSURE TABLE AND THE CREDIT LIST DISAGREE BY ~1.1 HOURS — the SMALLEST gap this series has
recorded, for a second consecutive cycle.** `src` reads **7 of 782 closed (0.009), $1.716 per merge, last
merge 2026-09-17T03:02:33.493Z** while the credit list names W1-T3701/#5856 credited at
**04:09:43.990Z**. **P47**'s series: 1.5 days → 8h → ~1h → ~6.6h → ~5.9h → ~4.6h → ~1.8h → **~1.1h** —
eight readings, no rung that ACTS on the gap, and a shrinking gap is a reconciler cadence, not a join
(rule 18).
**`docs` (0 of 42) and `plan-lint` (0 of 18) have merged NOTHING, ever** — **60 open items** in the two
classes the routing discount was built for, the W1-T167 question answered by absence a forty-sixth cycle,
while `src`'s open count climbs 766 → **775**.

**BY MODEL CLASS — weekly-limit burn (W1-T250/#898; P34 clause (d): burn is share of the weekly LIMIT,
never imputed dollars — the dollar column is context only). ★ THE UNATTRIBUTED ROW RETAKES THE MAJORITY
IT GAVE BACK LAST CYCLE, AND STILL CONTRIBUTES NOTHING TO THE OBJECTIVE:**

| model | runs | turns this week | share of weekly burn | $ this week (context only) |
|---|---|---|---|---|
| sonnet | 45 | 3953 | 100.0% | $97.639 |
| unresolved | 49 | 0 | 0.0% | $70.487 ⚠ **52% of the table's runs resolve to no model class and to 0.0% of the burn objective** |

**★ TASK D IS UNRESOLVABLE A TENTH CYCLE, AND THE ATTRIBUTED/UNATTRIBUTED MAJORITY HAS NOW FLIPPED
TWICE.** Attributed runs 46 → 47 → **45**, unattributed 58 → 44 → **49**; the unresolved row held the
majority, gave it back, and has taken it again — **three adjacent windows, two flips, one classifier
nobody edited**. The dollar relation stays in the same direction as last cycle ($70.487 unresolved against
$97.639 resolved), which is the first time in four readings it has NOT changed — and a relation that
repeats once is not a relation that has settled (rule 18). One attributed CLASS still means the share
column reads 100% by construction. **★ P73 AT THIS ALTITUDE:** the same document prices **24 runs at
$39.482**, **94 runs at $168.126** and **582 rows at $809.28** — a **20.5× spread**, after 42.6×, 56×,
34×, 15×, 1.4× and 35.9×. **A spread that halves is not convergence**: the three populations still have
no stated relation to one another.

**★ THE ARCHITECT-LANE SHARE (G-17's capability half, W1-T2239) — 451 rows / $596.38 / 73.7%** of a
582-row / **$809.28-notional** corpus. **★ THE TWO EMPTY `step` KEYS STAY EMPTY A SECOND CYCLE:**
`inbox_draft` 445 (**`gpt-5-nano×354` + `sonnet×91`**), `retro.synthesized` 6 (`claude-opus-5×6`, $70.49 —
**per-row $11.75 against $11.26, $12.73 and $12.77, a figure that has now moved in both directions over
four populations of 5, 5, 4 and 6**), and **`triage.synthesized` and `plan.synthesized` at ZERO rows /
$0.00 again**. ★ `plan.synthesized` remains the exhibit: it printed **$5.16 to the cent over 6, then 4,
then 2 rows**, and has now had **no members for two consecutive cycles** — the constancy was membership
bookkeeping the whole time, never a price (**P74**, **P54**). `implement` reads `sonnet×9`;
`review.reviewer` reads `sonnet×122`.
**★ THE OPENWEIGHT LANE EXTENDS ITS MAJORITY A FOURTH TIME.** `inbox_draft` read `sonnet×343`, then
`gpt-5-nano×174 + sonnet×286` (38%), then 59%, then 73%, and now **`gpt-5-nano×354 + sonnet×91` — 80%
openweight**, its largest population yet (445 rows). ★ Stated with its limit: the gather prices no
counterfactual, the window is explicitly HISTORICAL, and **no mount row moves on it**.
**★ THE SAME-DIRECTION MOVE ENDS BY DIVERGING, WHICH IS ALSO NOT A CONVERGENCE.** Rows **547 → 582**
(UP), dollars **$926.99 → $809.28** (DOWN), over a corpus supposed to be the same rolling span. Last cycle
both fell together and this file refused to read the repeat as a repair; this cycle they part, and the
refusal is the same one — **a corpus that reports its own membership can move its two axes in any
combination it likes**; P54 is confirmed a thirteenth time (rule 18). Every `$` here is **NOTIONAL /
API-equivalent on a subscription install, never billed spend**, and `assertArchitectAboveWorker` still
throws on a same-or-lower tier Architect regardless.

**Prior cycles (FOLDED — trend only; ledger-merged first, real ships in parentheses):** **R44–R57 FOLDED**
...
been present, absent, migrated and absent again over columns that did not change: **the series describes
the SENSOR, not the column.** **R55's, R56's AND R57's prose blocks are DELETED with this fold; the
findings between them (the credit union out-summing its own row, P75's recurrence at 3 / 4 / 4, the 56×,
42.6× and 35.9× dollar spreads, `diagnose` entering at exactly zero and leaving it one cycle later, the
`plan.synthesized` $5.16 row losing its last member) are carried by P73, P74, P54, P75's tombstone and
R58-2, each in exactly one place.**) ·
...
**Derived all-time:** ~1723 runs, ~760 ledger-merged (this cycle adds 24 runs and 7 `merged`; the
credit union adds 1 more the ledger never printed, and **all eight pairs are NEW**, so every one is work
this figure has not already counted — DR-22, DR-30).
...
- **★ THE READ R58 ADDS: A RECORD WITH TWO AUDIENCES AND ONE GATE HAS ALREADY CHOSEN WHICH CLAUSE IS LAW,
  AND NOBODY MADE THE CHOICE.** W1-T3693 carried a falsifier that waived implementation and a
  criterion that demands the build; the falsifier is addressed to an Architect and the criterion to a
  mechanical reviewer, and **only the reviewer has a gate behind it.** So a round can verify the
  falsifier's condition, act on it correctly, and be graded exactly as a round that did nothing. R54's
  read was about how a finding LEAVES this document; R55's about whether a gate a correct worker meets
  can be MET; R56's about whether a self-audit's population is DERIVED; R57's about whether a throttle can
  say how often it said no; **R58's is about whether every field a record carries has a reader — and one
  of ours does not.**
- **★ THE SECOND READ: A DEFECT CAN END BY LOSING ITS POPULATION, AND THE INSTRUMENTS CANNOT TELL THAT
  FROM A REPAIR.** P73's two published merge rates have disagreed in every prior reading (67%/71%,
  71%/63%) and agree exactly here — because the `diagnose` lane the class mapping folds into `src` had no
  runs. Three merge-count columns AGREE on `src`, and not one of them can see the lane that left.
  *A discrepancy whose cause is a fold does not close when the folded set empties; it goes dark* — which
  is why R58-2 now demands the folding class's membership whenever the gap reads zero.
- **★ THE THIRD READ: A RETIREMENT CLAUSE IS A CLAIM, AND THIS RUNG KEEPS SHIPPING THEM UNTESTED.** Three
  cycles running the self-loss has been in the exit machinery, not in a reading: R56's condition satisfied
  by an empty world, R57's counter miscounted beside its own label, and now **R57-6's kill trigger, which
  its own tie-break forbids from ever counting a single cycle.** The distinction R58 extracts is
  mechanical and is now doctrine in R58-5 and R58-6: **a trigger that counts a SEARCH PERFORMED can
  reach a second reading; a trigger that counts a SILENCE OBSERVED will be renamed UNAVAILABLE before it
  counts one, and must instead name an ARTIFACT** (R54's constraint, unlearned twice and re-applied here).
- **★ GUARD-FIRED BLOCKS: NONE (containment 0 / isolation 0, at 8 and 16 markers).** Replay: **NO RUN
  RECORDED** (thirty-fifth); mutation gate **N=0** (fortieth); infrastructure events **NONE** — in a
  window whose harvest names a structurally unsatisfiable task record, a bare-title proof dialect that
  cannot distinguish a real test from a paraphrase, and a REQUIRED ratchet moved by other PRs' arithmetic.
  **A cause no guard watches reads as zero** (P52). The `isolation` retirement is REFUSED a SIXTH time:
  its own precondition is golden-suite verification, and the golden suite did not run.
- **★ RATIFICATION TELEMETRY READS 0 / 0 / 0% A SIXTH CONSECUTIVE CYCLE — OVER A ROW COUNT THAT FELL A
  THIRD TIME, 190,394 → 186,621**, so the "cumulative" reading is not cumulative in any of three
  consecutive windows; and **LEARNINGS STANDS AT 79 WITH ZERO ADDED, A SEVENTH.** Two promotions are
  proposed at `user-overall` — the SAME two proposed last cycle, "promoted" then and landed nowhere — and
  none can be landed from here (**P71**).
...
- **A retro must not average over a spin loop** (R8's lesson); **nor over a window whose merge set it
  ...
  window's ships** (R25's); **nor over a population PARTITIONED BY A KEY 99% OF IT DOES NOT CARRY**
  (R28's); **nor over a population whose SIZE THE SAME DOCUMENT STATES TWICE, DIFFERENTLY** (R30's);
  **nor over a population assembled from branch names, a quarter of which name no run at all** (R40's);
  **nor over a population drawn from ONE AUTHORING LANE while a second lane ships source into the same
  ...
  whose runs answers to the same identifier** (R46's); **nor over a population you have already counted,
  under a different heading, in this same file** (R47's); **nor treat a finding as disposed of because
  it was written down** (R48's); **nor average over a column whose coverage you did not print — a table
  that stamps two of its five derived columns has not disclosed a limitation, it has certified three by
  omission** (R49's); **nor publish a total you have not checked against its own largest member — a sum
  smaller than something it contains is not an imprecise measurement, it is a different quantity wearing
  the label** (R50's); **nor read a full list as a report until you have published how much of it is new
  — a channel whose output looks identical when it has news and when it has none cannot be monitored at
  all** (R51's); **nor publish the same count twice under two headings without naming which is
  authoritative — two readings that are never required to meet are not a cross-check, they are two
  unsourced claims** (R52's); **and R53 adds the one that indicts the CROSS-CHECK itself: nor count an
  agreement between your own instruments as evidence of health unless some rung would have ACTED on
  their disagreement — where nothing consumes the check, a document can be perfectly self-consistent and
  still be describing nothing.**
  **P29(iii)** (annotate credit-rejected runs before they reach the mount table) is still unbuilt — 13
  credit-rejected runs are waiting for it an eighth consecutive cycle, so the entry stays EXERCISED.

## Retro proposals (PROPOSALS ONLY; NOT yet in plan/tasks.yaml)

**★ THE LIVE RANKING IS RETIRED — DELETED, NOT FOLDED (R53-4's arm, fired 2026-09-15).** This section
publishes no ORDER over its open proposals; what replaces it is the standing **DEBT line** below, which
names each outstanding one-file remedy and its **AGE IN CYCLES** — queue depth, not preference. Every
canonical entry keeps its own evidence, bar and kill trigger. **Rank language inside existing entries is
HISTORICAL record; no future cycle may add one.** ★ **R54's five-paragraph account of the deletion was
...
**★ R66 RETRO PROPOSALS — FAILURE MINING (PROPOSALS ONLY; nothing is filed and `plan/tasks.yaml` is
untouched): NO NEW PROPOSAL MINTED; the highest prior active header is P81.** The three blocked failures
are `blocked_transient`×1 and `blocked_transient:openweight_error`×2. They route to P42's existing
mechanism because MAST cannot map or name the runs behind either class; the gather supplies no evidence
for a distinct provider-retry mechanism. **PROPOSED GOLDEN (NOT FILED):** a seeded ledger containing one
generic transient and two openweight-error transient verdicts emits both reason-preserving MAST rows and
one named exemplar per class. **PROPOSED TASK (NOT FILED):** extend `plan/mast-mapping.yaml` for both
blocked classes and make the P42 exemplar renderer preserve the reason suffix rather than collapsing it.
The 19 unmapped `incomplete` rows remain P42 evidence, while the three gate-side merges are excluded from
the defect count under Standing rule 28. The three `user-overall` learning promotions remain candidates
for reviewed landing; the project-specific entry remains declined.

**★ R66 COMPRESSION:** the superseded R64 calibration block and R64 proposal-cycle notes were deleted.
R65 remains as the immediate comparison; durable mechanisms stay in their canonical proposal entries
and linked SHIPPED records.

**★ R65 RETRO PROPOSALS — FAILURE MINING (PROPOSALS ONLY; nothing is filed and `plan/tasks.yaml` is
untouched): NO NEW PROPOSAL MINTED; the highest prior active header is P81.** The verdict census contains
no `blocked_*` member, so there is no blocked failure to turn into a golden or task this cycle. The
`failed:openweight_error` rows and 28 `incomplete` rows stay explicitly unmapped rather than being
guessed into MAST; the six repeated task credits remain P47 evidence. W1-T49's empty `files:` declaration
stays P71's plan-health exhibit rather than being re-proposed. The two `user-overall` learning promotions
remain candidates for a reviewed landing; the two project-specific entries remain declined.

**★ R65 COMPRESSION:** the superseded R63 calibration table and R63 proposal-cycle notes were deleted.
R64 remains as the immediate comparison; older durable mechanisms stay in their canonical proposal
entries and linked SHIPPED records.

**★ R58 RETRO PROPOSALS — FAILURE MINING (PROPOSALS ONLY; nothing below is filed, and `plan/tasks.yaml`
is untouched): MINTED P81 — the highest prior id was P80**, derived by grepping this section's own
`P<N>` headers for the maximum before anything was written (the #125/#118 P21 collision is why this is
reported rather than assumed). The verdict column is `incomplete`×16 + `no_pr`×1 against `merged`×7 —
**and all eight credits are drawn from runs the discrepancy section reports `incomplete`** — so the mining
runs on the window's one non-`incomplete` failure verdict, on the blockers the harvest names, and on this
section's own machinery:
**(a) P81 (MINTED, NO RANK — the ranking was deleted by R53-4's arm and stays deleted) — A TASK RECORD'S
FALSIFIER AND ITS ACCEPTANCE CRITERIA ARE READ BY DIFFERENT READERS, AND ONLY ONE OF THEM GATES.**
GROUND TRUTH (from this gather's harvest alone, two independent rounds on **W1-T3693**): criterion 4
asserts *"an ungrounded openweight claim discards the generation"* while the same record's falsifier reads
*"close (d) without implementing if no lane wants a grounding contract"*; the rounds verified the
falsifier's condition **still holds**, and reported that *"the two cannot both be honored: the mechanical
reviewer never reads the falsifier text, so this criterion will stay UNMET forever unless an Architect
either withdraws/rewords it (rule-15, out of scope here) or a real consumer lane is built first."* A third
bullet states the consequence as a bound: *"as currently scoped this task's review floor cannot reach 0
unmet criteria through any in-scope diff."* **MECHANISM:** one record, two clauses, two audiences — the
falsifier is addressed to an Architect and the criteria to a gate; **the clause with no reader is not a
weaker rule, it is not a rule**, and exercising it correctly is indistinguishable at the gate from
failing. **CONSEQUENCE:** the task cannot pass, cannot be honestly failed, and cannot be fixed by the
worker who holds it — every fix round re-pays the review cost to arrive at the same floor.
**WHY THIS IS NOT AN EXISTING ENTRY:** **P78** is a criterion whose PROOF cannot discriminate or whose
literal string names nothing — the defect is in the proof text, and a correct proof would clear it; here
the proof is perfectly satisfiable and another clause of the same record forbids satisfying it. **P63** is
a scope deadlock a widening clears — a widening clears criteria 1–3 here (which ARE P63) and leaves
criterion 4 exactly as it was. **P79** is a REQUIRED check red on an input outside the diff — a CI
artefact, not a plan record. **P71** is a filing gap between a ratified proposal and a task, not a reading
gap inside one. **THE HONEST COUNTER-ARGUMENT, STATED:** falsifiers are deliberately advisory — they exist
so a human can close work on judgement, and wiring them into a mechanical gate would let a worker
self-authorise skipping anything. **The claim survives it and is narrower:** an advisory clause that
CONTRADICTS a gating clause in the same record is not advice, it is a trap; either the gate must see it or
the plan gate must refuse the pair at filing time. **PROPOSED BAR, REGISTERED VERBATIM AS R58-6 (see
above).** **FALSIFIER:** one round reporting a reviewer that reads falsifier text, or that grades a
correctly-exercised falsifier as MET, retires P81 as a mis-read of the gate. **PROPOSED GOLDEN (plan +
test, NOT FILED):** a fixture task carrying a satisfied falsifier and a criterion demanding the skipped
work is REFUSED at `rmd lint-plan` time rather than graded UNMET at review time. **PROPOSED TASK:** make
the contradiction unfileable — one `task-linter.ts` predicate that refuses a record whose falsifier can
close a criterion the same record asserts.
**(b) P78 (HELD — THE COUNT IS 4 AND THE REPLACEMENT RATE IS 100% FOR A SECOND CONSECUTIVE CYCLE).**
All four members are **W1-T3693**'s: criteria 1–3, whose literal-substring proofs (*"a fenced openweight
reply is extracted rather than rejected"* and siblings) cannot resolve to any test because the task's
declared `files:` list contains no `test/` path — *"structurally impossible for any fix-round to make
these four literal-substring proofs executable"* — and criterion 4, whose second defect is P81's.
**Last cycle's members W1-T3680 and W1-T3705 are not named by this gather at all: unmeasured, not
repaired** (P48). The entry gains this window's second exhibit, from W1-T3694: **the `unit test:`
bare-title dialect cannot distinguish a real test from a paraphrase of one** — *"a test whose behaviour is
correct but whose title paraphrases its proof text grades identically to a genuinely-missing test ('test
theater')"*, two full review cycles paid. **Both halves are the same defect: the proof dialect asserts a
string and nothing checks that the string can ever resolve.**
**(c) P79 (HELD — A NEW ARTEFACT, THE SAME SHAPE).** `test/fixture-copy-census.test.ts`'s baseline *"moves
under unrelated PRs on shared counters"*, so a REQUIRED strict ratchet can be red for another PR's
arithmetic — the input that decides the check lives outside the diff being judged, which is P79's
mechanism with a shared counter in place of a wall clock. Last cycle's member (the expiring-fixture census
over `test/an-open-pr-does-not-rot-while-it-waits.test.ts`) is **not named by this gather** — unmeasured,
not repaired.
**(d) P44 (HELD, ON ITS WEAKER EVIDENCE — AND ITS FALSIFIER ADVANCES A SECOND TIME).** `blocked_budget`
reads **0 of 24**, in band (6.5–26), so the two-consecutive-zero arm moves to **1 of 2**. ★ Scored
honestly under R57-1's own clause: a VACUOUS window is evidence for this entry's RETIREMENT, and last
cycle's member-bearing window was worth more to the entry than this one is. The limit id, cap value and
spend-at-block are unavailable again — but this time because nothing blocked.
**(e) P76 (HELD — AND THE TREND COLUMN NOW MOVES ON A RENAME).** MAST prints `verification`×**1** (trend
+1) while `specification` falls to **0** (trend −1). The population never left **one failure row**; only
its NAME changed. Across four adjacent windows the same one-row table has called its single member
`no_pr`, `blocked_ci`, `blocked_budget` and now `no_pr` again under a different category — **a taxonomy
whose trend column reports its own renaming as ±1 movement.** `incomplete`×**15** remains unmapped a
**THIRTEENTH** scored cycle and at its largest count yet (**P42**).
**(f) P70 (HELD — 8 of 8 NEW, and the union half falls to its lowest reading).** 0% → 40% → 17% → 17% →
**12.5%**. A sixth consecutive all-new cycle keeps R53-3's retirement arm (three consecutive ZERO-new
cycles) reset at zero, and the idempotency-key remedy did not ship (DEBT item 3).
**(g) P60 FIRES ONCE AFTER TWO ZERO CYCLES, AND ITS SUBJECT IS A PR THIS SAME GATHER CREDITS.**
RETRO-1789618247628's trailer names **#5829** from head branch `run-RETRO-1789595616760` — not its own —
and is refused, while the W1-T51 union credits the identical PR to the run that owns that branch.
**P60's series: 10 → 3 → 1 → 0 → 1 → 0 → 2 → 0 → 0 → 1.** ★ This is **P69** seen from the other side: one
constant `RETRO` key, two runs, one PR, credited once and rejected once in one document (DEBT item 4).
**(h) P52 (UNEXERCISED — zero guard fires, retirement REFUSED A SIXTH TIME).** containment 0 (8 markers) /
isolation 0 (16); infrastructure events **None** — in a window whose harvest names a structurally
unsatisfiable task record, a non-discriminating proof dialect and a cross-PR ratchet. The gather flags
`isolation` for retirement and names golden-suite verification as its own precondition; **replay recorded
no run (thirty-fifth)**, so the precondition cannot be met.
**(i) P38 (HELD — THE JUDGE CROSSES ITS OWN BOUNDARY A SECOND TIME, IN THE OPPOSITE DIRECTION).**
`askuserquestion-auto-resolves-empty-headless` reads 0.85 → 0.75 → 0.78 → 0.85 → **0.73 PROMOTED** →
**0.90 DECLINED** — declined at a HIGHER score than it was promoted at, one cycle after being promoted at
a LOWER score than it was refused at, on text nobody edited; `a-roster-error` rises **0.32 → 0.74** having
fallen 0.82 → 0.32 the cycle before. ★ **Two consecutive cycles of boundary crossings in opposite
directions is no longer a noisy judge — it is a judge whose score and whose decision are independent
variables.** `a-rigged-probe-measures-nothing` (**0.92**) and `an-error-names-its-cause-not-its-check`
(**0.87**) are PROPOSED for a THIRD time, twice "promoted" and landed nowhere — **P71 measured on P38's
own instrument.** Both are `user-overall`; neither can be landed from here.
...
**(1)** `plan/mast-mapping.yaml` needs one `incomplete` row — **AGE: 33 cycles**. **(2)** W1-T49 needs
one repo-relative path in `files:` — **AGE: 34 cycles**, its absence serialising the dispatcher's
fail-closed `overlappingPaths` against every co-dispatched candidate (the plan-health sweep returns it
alone again, and proposes the corrective task again; **this rung does NOT re-propose it**, per R50-5's
fired arm). **(3)** the credit row needs an idempotency key (P70) — **AGE: 11 cycles**. **(4)** the retro
lane needs a per-run key (P69) — **AGE: 13 cycles**. **(5)** the ownership assert must resolve across all
runs of a task id (P74) — **AGE: 8 cycles**. **(6)** `blocked_budget` rows must carry the limit that fired
and the reading it fired at (P44) — **AGE: 6 cycles**. **(7)** the id reservation must record a real
branch, admit a self-hand-off, AND survive a mid-session credential expiry (P77) — **AGE: 5 cycles**.
**(8)** the acceptance dialect needs a non-regression proof form, a `grep:` proof aimed inside the task's
own `files:` list, AND a check that a `unit test:` bare title can resolve to a real test (P78) — **AGE: 4
cycles**. **(9)** the expiring-fixture/shared-counter census must warn-and-file rather than block
unrelated PRs (P79) — **AGE: 3 cycles**. **(10)** the `gh` cadence floor must stamp its clock on the
ALLOWED call only and count its refusals (P80) — **AGE: 2 cycles**. **(11) NEW, and it is P81's:** a
record whose falsifier can close a criterion the same record asserts must be refused at filing time, or
the reviewer's input must carry the falsifier — **AGE: 1 cycle**. **Eleven entries, a combined dwell of
120 cycles, not one of them larger than a single file edit** (**P71**).
**(k) ROUTED, NOT MINTED:** the seven ledger-credit rescues and the gate-side merge — **a credit list
drawn ENTIRELY from runs the discrepancy section calls `incomplete`, a sixth consecutive cycle** — belong
to **P47**, and so do the procedural miner's sixth `clean_single_strike` commendation, now over ALL SEVEN
of those runs, and the closure-table/credit-list gap at **~1.1 hours**; `triage`'s eighth consecutive
$0.000 and the two `step` keys empty for a second cycle go to **P74** as the constant-price shape (and to
**P54** for the membership half); the type table's **50%** and the class table's **50%** over the SAME
seven merges go to **P73** — *held on a ZERO gap*, scored by **R58-2**; the `BY MODEL CLASS` majority
flipping back to `unresolved` keeps **TASK D** unresolvable a tenth cycle and routes to **P34(d)**;
**LEARNINGS at 79 with zero added a SEVENTH consecutive cycle** routes to the DARK roster's
`learningDuplicateViolation` line (T420/#1610); the harvest's stale-branch hygiene bullet and its
`ENTANGLEMENT_EXEMPT_INSTRUMENTS` advisory are **noted and NOT routed** — neither recurs and neither has a
mechanism this section owns; the plan-health sweep's W1-T49 finding is **P71**'s standing exhibit (DEBT
item 2); and the two learnings promotions are proposals at `user-overall` until a reviewed PR lands them
there.
...
**★ THE MINT DISCIPLINE, STATED ONCE AND STANDING (R44's per-cycle justification block — six cycles of
"why P68 earned a mint and six other findings did not" — is DELETED BY R50; every one of those findings
now lives in the canonical entry it was routed to, and a rationale restated in the section the scanner
reads is the redundancy HARNESS-COMPRESSION forbids).** Before any id is written, grep this section's
...
the routing is stated in that cycle's mining block so the refusal is auditable. **R66: no new proposal
minted; P81 is the highest active header.** ★ R58 exercised the discipline's converse six times: the
...
**R8–R53, FOLDED TO THEIR BAND BY R56 (git holds the per-cycle arithmetic; forty-six rows of
`credited/window` were themselves the accretion this line exists to name).** The series runs **0 of 28 to
25 of 33 ledger-credited, 0%–64%**, over windows of 7–40 runs, with three rows carrying an annotation the
fold keeps: **R32**'s 14 union pairs contained **ONE FALSE credit** (13 of 13 on code merges); **R39**'s
**2/21** was the series' lowest by 3× and the first row to refute its own claim — five runs were
genuinely deadlocked on a forbidden remedy (**P63**), not write-side at all; **R43**'s **12/14** was its
first CEILING.
**★ THE UNION COLUMN IS NON-INCREMENTAL, AND IS KNOWN TO BE RATHER THAN SUSPECTED.** R47's 129 contained
128 the R46 row had already counted; R48's 116, R49's 104, R50's 58, R51's 27 and R52's 22 SHRANK as
their markers ADVANCED, five times running; **R53's 4 shrank again and was entirely new — the same
mechanism (the reconciler's clock) producing the opposite reading.** **Every union figure double-counts
reconciliation batches** (**P70**, PROVEN; DR-30), said out loud rather than silently re-cutting
twenty-five rows on one cycle's discovery. The ledger-credit column is unaffected: `merged` is per-run.
Each folded row was the GATHER's credit count against that window's REAL ship count. They are
...
and evidence of decay on the way down.**)*
  **★ MEASURED READINGS — FOLDED TO TWO LINES BY R50 (git holds the per-cycle arithmetic).** R18–R32
  span **38%–83%**, R20 bounded, R27–R29 and **R33 UNCOMPUTABLE on rule 6** (a numerator and denominator
  from two populations do not make a percentage). **All prior readings remain FLOORS**; the instruments
  that would make the row exact are still TASK H, TASK K and P57's label split.
  **★ THE STANDING BOUND ON VIVID DEFECTS (2026-08-07; FOLDED TO ITS STATEMENT BY R50 — the argument
  was made once and git holds it).** *A rare, clustered defect may not be cited as the cause of an
  ambient band.* W1-T390's uncreditable slug-form branch ran 2 of 37 merged `run-*` over #1341–#1451;
...
- **★ P53 (R28's mint — THE PARTITION KEY 99% OF THE POPULATION DOES NOT CARRY: A SHARE TABLE
  **★★ R48 EVIDENCE, IN PLACE — THE ENTRY LOSES A KEY RATHER THAN GAINING ONE.** `triage.synthesized`
  has fallen **1 row → 0**, joining `plan.synthesized`, so the partition is now absent on **TWO of four
  `step` values** where R47 recorded one. Meanwhile the implement lane's attribution IMPROVED (**157 of
  171**, 14 unattributed, down from 19 of 168) and the corpus GREW 797 → 827 rows. **A partition key
  that improves on the lane with the most rows while disappearing on the lanes with the fewest is not
  converging** — it is being written by whichever emitter happens to be busy, which is the entry's
  claim restated with a third year of evidence.
...
- **★★★ P80 (process + golden; R57's mint, **NO RANK — the LIVE RANKING was deleted by R53-4's
  ...
  ITS OWN REFUSALS CANNOT BE WAITED OUT, AND THE ONLY WORKER IT STOPS IS THE ONE THAT OBEYS IT.**
  GROUND TRUTH (from this cycle's follow-up harvest alone, no network read — FOUR independent rounds):
  one round records the literal series *"159s → blocked → 12s → blocked → 20s → blocked → 33s → blocked →
  47s"* and concludes the `deny-floor.sh` pre-Bash hook *"appears to treat a blocked `gh` call attempt as
  itself resetting the 180s cadence clock … which can make the floor effectively unwaitable if a caller
  checks readiness by attempting the call"*; a second could not fetch a REQUIRED check's failing log
  (*"each attempt, even a blocked one, appears to reset the floor's own timer"*); a third could not poll
  CI status for #5770 at all and asked the orchestrator to confirm the checks; a fourth could not
  re-verify an edited PR body and fell back to a local `git diff`.
  **MECHANISM:** the cooldown stamps its clock on the ATTEMPT rather than on the ALLOWED call, so probing
  for readiness is indistinguishable from consuming the budget and the wait never converges; the stamp is
  shared across every worker under one `$HOME`, so a compliant worker cannot tell its own cooldown from
  its neighbours'. **CONSEQUENCE:** the cost is not latency but UNVERIFIED WORK — a PR body not re-read
  after an edit, a REQUIRED check's log not read, CI status not polled — and it is paid ONLY by workers
  that respect an ADVISORY control. **The harness records none of it**: zero guard fires, zero
  infrastructure events, and no refusal counter in any gather column, so the control cannot be sized,
  tuned, or shown to be working.
  **WHY THIS IS NOT AN EXISTING ENTRY:** **P77** is a missing PRIVILEGE held in an object no grant can
  change, where here every worker HAS the permission and is refused by a timer; **P79** is a gate whose
  failing INPUT lies outside the PR's diff, where here the input is the worker's own previous attempt;
  **P52** is a cause no guard watches — true of the consequence, but the defect is a stamping rule, not a
  sensor; **P63** is a scope deadlock a widening clears, and a widening changes nothing here.
  **THE HONEST COUNTER-ARGUMENT, STATED:** counting attempts is the SAFE design against a caller that
  hammers a rate-limited API, and GitHub's secondary limits are real and costly. **The claim survives it
  and is narrower:** a floor may refuse without RE-ARMING, and a control that cannot report how often it
  said no is indistinguishable from one that is broken.
  **PROPOSED BAR, CARRIED AS R58-5 (registered at mint as R57-6):** *state the count of rounds this window that report a
  read-cadence/rate-limit REFUSAL, naming each; state whether ANY round reports a refused call followed by
  a successful one in the same session; and state whether the gather prints a refusal counter at all — or
  declare the count unavailable.* **FALSIFIER:** one round reporting a completed `gh` read after waiting
  out the floor WITHOUT a bypass retires P80 as a misdiagnosis.
  **PROPOSED GOLDEN (plan + test, NOT FILED):** a fixture that issues three refused calls inside one
  window and then one call after the window has elapsed sees the fourth call ALLOWED.
  **PROPOSED TASK:** stamp the cadence clock on the ALLOWED call only, and emit a `gh.read_floor_refused`
  ledger line carrying the waited interval — one file (`deny-floor.sh`) plus its counter.
  **★ R58's READING — HELD, UNEXERCISED, AND ITS KILL TRIGGER STRUCK.** This cycle's harvest (twelve
  bullets, NON-EMPTY) names **no** `gh` read refusal at all; under the bar's own tie-break that is an
  **UNAVAILABLE count, never a zero** (P48). **KILL TRIGGER, REPLACED — the old one ("two consecutive
  cycles whose NON-EMPTY harvest names no `gh` read refusal") is DELETED as UNREACHABLE**, because the
  same row's tie-break renames every such window UNAVAILABLE before the trigger can count it; R58 found
  the collision one cycle after R57 wrote it, and it is R58's own self-loss. **The replacement names an
  ARTIFACT, per R54's constraint:** P80 retires when `deny-floor.sh` stamps the ALLOWED call only, **or**
  when any gather column prints a refusal counter — an entry may not be closed by a quiet window.

- **★★★ P81 (process + golden; R58's mint, **NO RANK — the LIVE RANKING was deleted by R53-4's
  fired arm; AGE 2. R59 corrects the founding exhibit's status: W1-T3693 is MERGED via
  [#5852](https://github.com/craigoley/remudero/pull/5852), so the open-member count is zero and the
  mechanism remains proposed for future filings**) — A TASK RECORD'S FALSIFIER AND ITS ACCEPTANCE
  CRITERIA ARE ADDRESSED TO DIFFERENT READERS, AND ONLY ONE OF THEM GATES.**
  HISTORICAL GROUND TRUTH (from R58's follow-up harvest alone, no network read — TWO independent rounds on
  **W1-T3693**, across five bullets): criterion 4 asserts *"an ungrounded openweight claim discards the
  generation"*; the same record's falsifier reads *"close (d) without implementing if no lane wants a
  grounding contract"*; the rounds verified the falsifier's condition **still holds** and reported that
  *"as filed, the two cannot both be honored: the mechanical reviewer never reads the falsifier text, so
  this criterion will stay UNMET forever unless an Architect either withdraws/rewords it (rule‑15, out of
  scope here) or a real consumer lane is built first."* A third bullet states the bound outright: *"as
  currently scoped this task's review floor cannot reach 0 unmet criteria through any in-scope diff."*
  **MECHANISM:** one record carried two clauses written for two audiences — the falsifier for an
  Architect, the criteria for a mechanical reviewer — and only the criteria reach a gate. **A clause no
  reader consumes is not a weaker rule; it is not a rule**, and a worker that obeys it is graded
  identically to one that failed. **R58 CONSEQUENCE:** the task appeared unable to pass or be repaired by
  its worker; R59's merge closes that task-status claim without closing the proposal's reader/gate defect.
  **WHY THIS IS NOT AN EXISTING ENTRY:** **P78** is a criterion whose PROOF cannot discriminate or whose
  literal string resolves to nothing — the defect is in the proof text and a better proof clears it; here
  the proof is satisfiable and a sibling clause forbids satisfying it. **P63** is a scope deadlock a
  widening clears — widening `files:` clears this task's criteria 1–3 (which ARE P63) and leaves criterion
  4 untouched. **P79** is a REQUIRED check red on an input outside the diff, a CI artefact rather than a
  plan record. **P71** is the gap between a ratified proposal and a filed task, a FILING gap, not a
  READING one.
  **THE HONEST COUNTER-ARGUMENT, STATED:** falsifiers are deliberately advisory — they let a human close
  work on judgement, and wiring one into a mechanical gate would let a worker self-authorise skipping
  anything it found inconvenient. **The claim survives it and is narrower:** an advisory clause that
  CONTRADICTS a gating clause of the same record is not advice, it is a trap; either the gate sees it or
  the plan gate refuses the pair at filing time. Advisory is fine; advisory-and-contradictory is not.
  **PROPOSED BAR, REGISTERED VERBATIM AS R58-6:** *state the count of OPEN tasks whose falsifier (or
  equivalent close-unbuilt clause) is satisfied while an acceptance criterion still demands the
  implementation, NAMING each and its criterion; state whether the reviewer's input carries the falsifier
  text at all; and state whether any Architect withdrawal or rewording landed — or declare the count
  unavailable.* **FALSIFIER:** one round reporting a reviewer that reads falsifier text, or that grades a
  correctly-exercised falsifier as MET, retires P81 as a mis-read of the gate.
  **PROPOSED GOLDEN (plan + test, NOT FILED):** a fixture task carrying a satisfied falsifier and a
  criterion demanding the skipped work is REFUSED at `rmd lint-plan` time, not graded UNMET at review.
  **PROPOSED TASK:** one `task-linter.ts` predicate that refuses a record whose falsifier can close a
  criterion the same record asserts — one file plus its test.
  **KILL TRIGGER — AN ARTIFACT, NOT A READING (R54's constraint, and R58-5's lesson applied at mint):**
  P81 retires when the reviewer's prompt surface provably carries the task's falsifier, **or** when the
  plan gate refuses the contradicting pair at filing time. ★ A window whose harvest names no such task
  declares the count UNAVAILABLE and advances NO counter — this entry will not be closed by silence.

- **★★★ P79 (process + golden; R56's mint, **NO RANK — the LIVE RANKING was deleted by R53-4's
  fired arm; it sits in the DEBT line instead, at AGE 3**. ★ **R58's reading: the member changes AGAIN,
  and neither prior artefact is named by this gather** — the expiring-fixture census and the
  `coverage-shard (4/4)` red over a `main`-side file are both unrepaired and UNMEASURED (P48). This
  cycle's member is `test/fixture-copy-census.test.ts`, whose *"baseline moves under unrelated PRs on
  shared counters"*: a REQUIRED strict ratchet going red for another PR's arithmetic — **the same
  mechanism with a shared counter in place of the wall clock.** Count (ii) = 1, no repair landed.**) — A REQUIRED CHECK WHOSE INPUT IS THE WALL
  CLOCK AND A FILE NO TASK OWNS GOES RED FOR THE WHOLE REPOSITORY, AND EVERY TASK IT BLOCKS IS FORBIDDEN
  TO CLEAR IT.** GROUND TRUTH (from this cycle's follow-up harvest alone, no network read): **W1-T3629**'s
  round names *"the two expiring fixtures in
  `test/an-open-pr-does-not-rot-while-it-waits.test.ts:140,245`"* as *"a repo-wide
  `expiring-fixture-census` state that will keep `comment-load-ratchet` red for every PR until repaired
  independently of this branch"*; **W1-T3639** — a different task on a different branch — names lines
  **140, 245, 246**, an expiry of **2026-09-23**, and the census step of the REQUIRED
  `comment-load-ratchet` job as *"already BLOCKED as of today"*, reddening *"every PR touching `src/`"*.
  Both declare the repair OUT OF their declared scope and file a follow-up instead.
  **MECHANISM:** the check's verdict is a function of `Date.now()` and a shared fixture rather than of the
  PR's diff, so a REQUIRED gate fails on unrelated work — and rule 15's file fence, the discipline that
  keeps diffs honest, is precisely what forbids the repair. **CONSEQUENCE:** the block is inherited by
  whoever happens to be in flight; the window's only failure verdict is `blocked_ci`×1, which MAST renames
  `verification`×1 **without naming a check**, so the class cannot be sized even in retrospect.
  **WHY THIS IS NOT AN EXISTING ENTRY:** **P78** is a criterion unprovable for ONE task's own diff, whose
  remedy is an edit to that task's own record — here the criterion is sound and the FAILING INPUT belongs
  to no task; **P63** is a scope deadlock a widening clears, but a widening here puts one unrelated repair
  into N diffs; **P77** is a missing PRIVILEGE, where every worker already has write permission and what
  is missing is a TASK; **P52** is a cause no guard watches, where this cause is watched loudly by the
  correct check. **THE HONEST COUNTER-ARGUMENT, STATED:** expiring fixtures are working as designed — the
  census exists to force re-stamping, and "red until repaired" IS the intended pressure. **The claim
  survives it and is narrower:** the pressure is applied to whoever is in flight, none of whom may relieve
  it, and nothing files the maintenance task, so a bounded duty becomes an unbounded tax on unrelated
  work — and the fleet cannot even count what it costs.
  **PROPOSED BAR, REGISTERED VERBATIM AS R56-3(ii):** *state the count of REQUIRED checks red for a reason
  outside the PR's own diff, name the check and the artefact, and state whether a repair landed — or
  declare the count unavailable.* **FALSIFIER:** a cycle in which the census's remedy lands from inside an
  ordinary task's declared scope retires P79 as a mis-read of the fence.
  **PROPOSED GOLDEN (plan + test, NOT FILED):** a fixture PR whose diff touches only `src/x.ts` sees the
  expiring fixture reported as a REPO-level advisory with a named owner task, and never as that PR's own
  blocking failure. **PROPOSED TASK:** make the expiring-fixture census warn-and-file — auto-open a dated
  maintenance task — instead of blocking PRs whose diff does not touch the expiring file.
  **KILL TRIGGER:** two consecutive cycles whose harvest names no REQUIRED check red for a diff-external
  reason — the pattern was this one fixture's alone and the entry closes.

- **★★★ P78 (process + golden; R55's mint, **NO RANK — the LIVE RANKING was deleted by R53-4's fired
  arm; it sits in the DEBT line instead, at AGE 4**. ★ **R58's reading: the count RISES TO 4 and the
  members are 100% REPLACED A SECOND CONSECUTIVE CYCLE.** W1-T3680 and W1-T3705 are unnamed by this
  gather — unmeasured, not repaired (P48) — and all four members are **W1-T3693**'s: criteria 1–3, whose
  literal-substring proofs cannot resolve because the task's `files:` list contains no `test/` path, and
  criterion 4, whose second defect is P81's. ★ The entry gains its discrimination half from W1-T3694:
  **a `unit test:` bare title that PARAPHRASES its proof grades identically to a missing test — *test
  theater*, two review cycles paid.** Two consecutive 100% replacement rates must be read as the class
  RECRUITING, never as the class holding — R58-3 now requires the rate printed beside the count.) — A CRITERION
  WHOSE SUBJECT IS A NON-CHANGE CANNOT
  PASS A GATE THAT REQUIRES ITS PROOF TO DIFFER, SO THE HARNESS REFUSES THE WORK PRECISELY WHEN THE WORK
  IS CORRECT.** GROUND TRUTH (from this cycle's follow-up harvest alone, no network read, carried from R55): **W1-T3618**
  shipped a complete implementation with passing checks whose FOURTH acceptance criterion —
  `grep: reviewerCodePublicationRefusal in src/lib/review.ts` — is `executed_stale` **by construction**,
  because the correct diff for that task never touches `src/lib/review.ts`; the round's own words:
  *"that proof is `executed_stale` by construction whenever the diff (correctly) never touches
  `src/lib/review.ts`, so this task can never pass `proof-discrimination` as currently filed."*
  SECOND MEMBER, same cycle, opposite direction: **W1-T3613**'s claim-2 proof names a test that
  **W1-T3619 authored and merged first**, so the only green path is adding `"W1-T3613": 1` to
  `scripts/proof-discrimination-baseline.json`, **outside its declared `files:` fence** — declined for
  that reason. **MECHANISM:** `proof-discrimination` asserts *this diff introduced this proof*. A true
  criterion of the form *X REMAINS UNCHANGED* or *X ALREADY HOLDS* has no introduced artifact, so the
  gate's pass condition and the task's correctness condition are **contradictory**, and the two escapes
  (repoint the criterion, extend the baseline) are both PLAN-side edits an implement worker may not make.
  **CONSEQUENCE:** a full CI cycle is paid to rediscover the contradiction, a follow-up asks an Architect
  to repoint the task's own acceptance, and the work waits. The harvest asks, unprompted, whether other
  newly-filed tasks share the pattern — **the population is unknown and nothing counts it.**
  **WHY THIS IS NOT AN EXISTING ENTRY:** **P63** is a scope deadlock a WIDENING clears, and here widening
  is WRONG (touching the file makes the diff incorrect); **P77** needs a PRIVILEGE, where this needs only
  an ordinary plan edit by another lane; **P46** is a premise that ROTTED after filing, where this one was
  unsatisfiable AT filing; **P49** is a field contradicting its own prose, where here field and prose agree
  and the GATE contradicts both. **THE HONEST COUNTER-ARGUMENT, STATED:** writing provable criteria is the
  author's job, so this may be a plan-authoring defect rather than a gate defect. **The claim survives it
  and is narrower:** nothing in the plan gate, the acceptance-author gate, or the authoring convention
  rejects such a criterion AT FILING TIME, so it is always discovered after a worker has been paid.
  **PROPOSED BAR, CARRIED AS R56-3(i) — ONE ROW NOW MEASURES P78 AND P79 TOGETHER, because both
  count work refused for a reason its own diff cannot reach:** *state the count of acceptance criteria this cycle that
  could not be proven because their proof must be ABSENT from a correct diff, name each task, and state
  whether any repair landed — or declare the count unavailable.* **FALSIFIER:** a cycle in which a
  non-change criterion passes `proof-discrimination` unmodified retires P78 as a mis-read of the gate.
  **PROPOSED GOLDEN (plan + test, NOT FILED):** a fixture task whose criterion asserts a file is untouched,
  asserting EITHER that the plan gate refuses it at filing time with a named alternative, OR that
  `proof-discrimination` admits a declared `non_regression:` dialect satisfied by an in-scope test.
  **PROPOSED TASK:** add the non-regression proof form to the acceptance dialect, and have the plan gate
  reject a `grep:` proof aimed at a path outside the task's own `files:` list. **KILL TRIGGER:** two
  consecutive cycles whose harvest names no by-construction-stale proof — the pattern was W1-T3618's
  alone and the entry closes. ★ Per R54-5's bar, that trigger is ARTIFACT-FREE by design and is labelled
  so in advance: closing on it would be **TRIGGER-ONLY**, and P75 is what that costs.
  **★ R56 UPDATE, IN PLACE — THE FIRST ATTEMPTED REPAIR LANDED AND DID NOT CLEAR THE DEFECT.** A MERGED
  PR (#5750) repointed W1-T3613's claim and `proof-discrimination` **still refuses it**: the replacement
  proof names a test an already-merged sibling authored, so the criterion is `executed_stale` for a NEW
  reason. **The entry gains a clause — a repoint is not a remedy while the replacement proof is any
  PRE-EXISTING artifact**; only a proof the task's own diff introduces can discriminate. Count unchanged
  at **2**, both tasks still blocked, W1-T3618 untouched a second cycle.

- **★★★ P77 (process + golden; R54's mint, **AGE 5 in the DEBT line — stated ONCE, here, after R58 found
  it published as 4 in this header and as 1 in this entry's own body**. ★ **R58's reading: UNEXERCISED —
  this gather names no credential, anchor or authority block at all.** That is P48's absence, never a
  zero: R57's fourth member class (a GitHub-App `GH_TOKEN` expiring MID-SESSION, stranding verified
  commits `dab814ab5` and `7b0eb4f10`, which `github-app.ts` documents as *"accepted, not fixed"*) is
  unrepaired and unmeasured. The waiting count remains at least two complete, locally-verified commits.**) — **NO RANK,
  minted into a section whose LIVE RANKING was deleted in the same pass by R53-4's fired arm; it sits in
  the DEBT line instead**) — A GATE
  WHOSE REMEDY REQUIRES AN AUTHORITY NO WORKER HOLDS IS A DEADLOCK THAT RE-DISPATCH CANNOT CLEAR, AND THE
  ONLY CHANNEL TO THE ACTOR WHO COULD CLEAR IT IS A HARVEST THAT FILES NOTHING.** GROUND TRUTH (from this
  gather's follow-up harvest alone, no network read): **PRs #5612, #5615 and #5616** are blocked by
  `task-id-existence`'s holder-conflict check on `refs/rmd-id/W1-T3601` and `refs/rmd-id/W1-T3602` —
  anchors minted `branch=unknown` because the reserve path runs BEFORE `git checkout -b`, so
  `currentBranch()` (`src/lib/task-id-reservation.ts:391-398`) has no `GITHUB_HEAD_REF` and no resolvable
  branch to record. The harvest states **four times across rounds** that the remedy needs *"an operator
  (or the id-reservation tooling, with force-push authority over `refs/rmd-id/*`)"*. SECOND MEMBER:
  `acceptance-author-gate` + `squash-trailer-gate` on commit `c0e8d6adb`, repairable only by REWRITING a
  reachable commit message — force-push again, on a branch the fix rung may not rewrite.
  **MECHANISM:** the gate's precondition lives in an object immutable under a worker's grant, so the
  remedy is outside the fleet's automated reach rather than outside one task's `files:` list.
  **CONSEQUENCE:** every round rediscovers the same block, pays a CI cycle, and appends a harvest line
  while the blocked PRs accumulate — and the harvest is the only path to the actor named, which files
  nothing (**P71**). **WHY THIS IS NOT AN EXISTING ENTRY:** **P63** is a SCOPE deadlock a widening
  clears — **P77 bounds it**, because no file list confers a privilege; **P66** is two lanes colliding
  over one board, where here the rival claimant is the task's OWN bootstrap mint, a self-collision;
  **P44** names a resource ceiling whose actor is unprinted, where P77's actor is named and merely
  unreachable. **THE HONEST COUNTER-ARGUMENT, STATED:** an operator is an actor in this fleet, so "nobody
  can fix it" is false. **The claim survives it and is narrower:** no AUTOMATED rung can, and the fleet's
  only path to a human is this section. **PROPOSED BAR, REGISTERED VERBATIM AS R54-5:** *for every entry
  this section CLOSES, name the ARTIFACT whose existence discharged it — a file, a column, a field, a
  merged PR — or record the closure as TRIGGER-ONLY; and for every open entry blocked on a privilege no
  worker holds, state the actor named and the count of PRs waiting.* **FALSIFIER:** a cycle in which an
  authority-blocked branch is cleared by a worker-side action retires P77 as a mis-read. **PROPOSED
  GOLDEN (plan + test, NOT FILED):** a fixture in which the reserve path runs with no resolvable branch,
  asserting the anchor records a DEFERRED holder rather than the literal `branch=unknown`, and that the
  holder check admits a same-host self-hand-off. **PROPOSED TASK:** reserve the id AFTER checkout (or pass
  the intended branch explicitly), and give `evaluateReservationHolderConflicts` a documented
  self-hand-off path. **KILL TRIGGER:** two consecutive cycles in which no harvest line names an
  operator-only remedy — the fleet's remedies are all in-band and the entry closes. ★ **Per its own bar,
  that trigger names an ARTIFACT-free condition on purpose and is therefore labelled in advance: closing
  on it would be TRIGGER-ONLY unless the reserve-after-checkout change has shipped.**


- **★★★ P76 (instrument + golden; R53's mint; rank language now HISTORICAL — the ranking was deleted by
  R53-4's fired arm. ★ **R54's reading: `verification`×2 = `blocked_ci`×2 → RENAME, `specification`×0 →
  EMPTY, so 1 of 1 non-empty categories is a rename a SECOND consecutive cycle — and the zero row
  LITERALLY satisfies this entry's own falsifier, which is REFUSED and rewritten below**) — THE ONLY FAILURE
  TAXONOMY THE FLEET HAS IS A ONE-TO-ONE RENAME OF THE VERDICT COLUMN, SO IT ADDS NO INFORMATION AND
  PRESCRIBES THE WRONG REMEDY.** GROUND TRUTH (mechanical, from this gather alone, no network read):
  `plan/mast-mapping.yaml`'s census prints `specification`×**7** (trend **+7**) and `verification`×**4**
  (trend **±0**); the verdict census in the same gather prints `blocked_budget`×**7**, `blocked_ci`×**4**,
  `incomplete`×**13** and `merged`×**4**. **Each category count equals exactly one verdict class's count**,
  and the two mapped categories cover 11 of the 24 non-merged verdicts with `incomplete`×13 listed under
  *Unmapped verdict classes*. **MECHANISM:** the mapping is keyed on VERDICT CLASS with no member-level
  evidence, so a category can only ever be a synonym for a class — it re-partitions the population exactly
  as the verdict column already did, and cannot discover a cause the verdict name does not already carry.
  **CONSEQUENCE, and it is the rank argument:** (i) the taxonomy adds nothing a `uniq -c` of the verdict
  column would not; (ii) it files `blocked_budget` — a RESOURCE ceiling — under `specification`, whose
  remedy is *rewrite the task spec*, **pointing the fix at the task author when the cause is a cap**, and
  a WRONG owner is worse than no owner; (iii) **it bounds P42**, which has asked for one `incomplete` row
  in this same file for twenty-eight cycles — that row would have bought a THIRD rename. **WHY THIS IS NOT
  AN EXISTING ENTRY:** **P42** is a COVERAGE gap (a class maps to nothing); P76 is about the MAPPED side
  being information-free. **DR-28** is *a verdict class is not a failure class until you check which of its
  members merged* — about members that shipped, not about a category that is a synonym for its class.
  **P57(a)** is a wrong LABEL on one credit rejection; P76 is a taxonomy that is definitionally correct
  and useless. **P73/P75** are one quantity published as two counts; P76 is one count published under two
  names. **THE HONEST COUNTER-ARGUMENT, STATED:** with only two non-merged classes mapped, a 1:1
  correspondence could be small-number coincidence. **The claim survives it and is narrower:** the
  mapping's own trend column reads `+7` on `specification` in the very cycle `blocked_budget` goes 0 → 7,
  and **the category's prescribed remedy is wrong for its members whether or not the equality is chance.**
  **PROPOSED BAR, REGISTERED VERBATIM AS R53-2:** *for every category the MAST mapping prints, state its
  count beside the verdict-class count(s) composing it, and declare the category a RENAME (1:1 with one
  verdict class) or a genuine CLASSIFICATION.* **FALSIFIER — SHARPENED BY R54 AGAINST ITS OWN
  AUTHOR:** a cycle printing a category **with a NON-ZERO count** matching no single verdict class — a
  real many-to-one or one-to-many map — makes P76 a window artefact and **RETIRES** it. *(The original
  wording said only "any category", and R54's gather satisfied it with `specification`×0. An EMPTY
  category satisfies any condition — the identical defect R53 struck out of P75 one cycle before writing
  it into this mint — so the zero-count reading is REFUSED and excluded in the text.)* **PROPOSED GOLDEN (plan + test, NOT FILED):** a gather fixture with two verdict
  classes and one category spanning both, asserting the rendered mapping prints the composition
  (`specification ← blocked_budget 5 + incomplete 2`) rather than a bare count. **PROPOSED TASK:**
  `plan/mast-mapping.yaml` maps on run EVIDENCE (the block's own cause string) rather than on verdict
  class; the gather prints each category's composition; and `blocked_budget` maps to a RESOURCE category
  whose remedy is a cap or routing change, never `specification`. **KILL TRIGGER:** two consecutive cycles
  in which every printed category's composition is stated and at least one is genuinely many-to-one — the
  taxonomy is classifying, and the entry closes.

- **P75 — CLOSED 2026-09-15 BY ITS OWN KILL TRIGGER (R52's mint, R53's demotion and struck falsifier,
  R54's closure); prose DELETED, git holds it.** *One lane's merge count is published three times in one
  gather and no two columns are required to agree.* It asked for ONE artifact — a DECLARED AUTHORITATIVE
  COLUMN — and closed on two consecutive cycles of `src` reading **4 / 4 / 4** while `retro`/`triage` were
  empty or REFUSED. **No column was ever declared, no join was ever built, and no code changed: the
  closure is TRIGGER-ONLY** (R54-5's label, applied first to this rung's own entry). The durable lesson
  outlived the entry and is kept: **a kill trigger written over a READING is discharged by a quiet
  window — write it over the ARTIFACT that must exist.** The three-way disagreement itself is NOT
  retired: it is live in **P47** (one population seen twice across time) and **P73** (the same disease on
  dollar totals, now at a **56×** spread), and its remedy is still unbuilt.
  ★ **R55, ONE CYCLE LATER — THE DEFECT RECURRED AND THE TOMBSTONE TAKES THE EVIDENCE, NOT A RE-MINT.**
  `src` reads **3 (type table) / 4 (`merge source`) / 4 (closure table)** and `retro` **0 / 1 / 1**, and
  the disagreement is now visible INSIDE one row: `turns/merge` 197.25 and `output tokens/merge`
  130,599.25 are 789 / **4** and 522,397 / **4**, derived columns dividing by a denominator the same row's
  `merged` cell puts at **3**. **The id stays CLOSED** — reopening an id on fresh evidence is how the
  #125/#118 collision was made — and the reading is re-registered as **R55-2** instead. This is the
  cleanest proof this log holds that a TRIGGER-ONLY closure closes a record and not a defect.
  ★ **R56, ONE CYCLE LATER STILL — THE READING REVERSES AND THE ID IS STILL NOT RE-OPENED.** `src` now
  reads **5 / 5 / 5** and its derived columns divide by the 5 the row states; `triage` **0 / 0 / 0**;
  only `retro` disagrees, at **0 / 1 / 1**. **Nothing shipped between the disagreeing and agreeing
  readings**, so the reversal is weather in the same sense the recurrence was (rule 18) — and the
  tombstone takes this evidence too, because an id that stays closed through evidence that flatters it is
...
- **★★ P69 (credit + golden; R46's mint, rank 2 — ★ R51 CONFIRMS IT A SIXTH TIME AND THE FINDING ROUTES
  ...
  the Discrepancies section carries **13 rejections of the form** *"GitHub trailer names
  …/pull/5483 but its head branch (`run-RETRO-1789358574280`) is not this run's own branch
  (`run-RETRO-<epoch>`) — stale/foreign trailer, never credited"*. Each reads as a per-run accident.
  **They are one event, and the target has now MOVED SIX TIMES, each time to the lane's newest merge:
  #5183 (`codex/retro-preflight-child-env`) → #5439 → #5449 → #5455 → #5471 → #5483, the last five being
  the retro lane's own immediately preceding plan-sync PRs, merged gate-side.** **The constant key does
  not
  inherit one stale trailer; it re-binds to whatever the lane most recently merged, so every successful
  retro guarantees the next cycle's rejections.** **★ THE DISPOSITION IS THE PRE-COMMITTED ONE A THIRD
  TIME:** R48-2's arm made *the remedy's absence* the finding rather than P69 again, and the target moved
  again. **So P69 HOLDS at rank 2 and P71 takes the finding** — a sixth confirmation buying no rank,
  which is what a pre-registration is for.
  **CONSEQUENCE, PRICED — AND THE PRICE COLUMN HAS FAILED TOO:** the `retro` lane reads **1 run /
  0 merged / $0.000** in the type table while the same gather prices `retro.synthesized` at **$158.85
  across 45 rows**; its 0% is a key defect wearing the grammar of a merge rate, and its $0.000 is
  **P73's** hole wearing the grammar of a free lane. **THE COUNT IS KEPT CLEAN, AND IT IS NOT A
  SEVERITY MEASURE:** 13 rejections against a 1-run type-table row is ONE binding pulled back across the
  lane's whole in-window history, so the number tracks window LENGTH — 28 became 13 as the window went
  ~2.5 days → ~30 hours, with nothing repaired. **Nine further rejections name a PR
  on a LATER run of the SAME per-task id** (W1-T3389, W1-T3390, W1-T3394, W1-T3395, W1-T3525 and FOUR
  `TRIAGE-fb-*`) —
  re-dispatch (**P62/P64**, and for the triage members **P74**) — and **three more name a branch that
  was never any run's at all** (**P60**); folding either in would nearly double P69 with members whose
  fix differs (DR-8). **WHY THIS IS NOT
  P33 OR P61:** P33 is a
  ...
  VERBATIM AS R46-1, SCORED A HIT THREE TIMES, ITS R47-2 SECOND ARM FIRED BY R48 (promoting P69 above
  P67) AND ITS R48-2 THIRD ARM FIRED BY R49 (the named PR moved a FOURTH time, which by that arm's own
  terms promotes the REMEDY'S ABSENCE — P71 — and leaves P69 where it stands):** *a Discrepancies rejection whose stated cause is that the run's trailer resolves
  ...
  (i) a resolver fixture in which two runs share one task key and ONLY a run's own head branch may
  satisfy its trailer; (ii) a dispatch-side check that a lane key is unique per run, so `RETRO` becomes
  `RETRO-<epoch>` at the point the trailer is written, not only in the branch name; (iii) the retro's own
  PR body carries the per-run id it is already branched on. **KILL TRIGGER:** two consecutive cycles in
  which the retro lane's rejections read zero WHILE the retro lane ran — an idle lane and a fixed lane
  print the same, so the row is UNRESOLVABLE without the second clause.
- **★★ P68 (dispatch + golden; R44's mint, rank 8 after P74's entry — ★ THE RANK LINE WAS CORRECTED BY R50, WHICH FOUND IT
  READING "PROPOSED at 3 directly under P67" AGAINST A LIVE RANKING THAT HAS PLACED IT AT 7 FOR FOUR
  CYCLES. This section's own header declares the ranking line the ONE place proposals are ordered, so a
  per-entry rank is a SECOND source that can drift — and it did. Recorded rather than quietly fixed,
  because it is P73's defect at the altitude of prose: two figures for one quantity, in one document,
  with nothing asserting they agree**) — THE RECON RUNG IS
  DISPATCHED WITHOUT THE TASK RECORD IT EXISTS TO SCOPE, AND THE HARNESS PAYS TO REDISCOVER IT.**
  GROUND TRUTH (**mechanical, from R44's own follow-up harvest; the eleven verbatim quotes were printed
  in full in the cycle that minted this entry and are FOLDED TO THEIR COUNT BY R50 — git holds them**):
  **five of eleven runs reported, unprompted, that recon had no task spec** (W1-T2613, W1-T2619,
  W1-T2625, W1-T2629, W1-T2623 — the last naming the likely mechanism, *"likely in daemon task DB or
  `TaskGet`/`TaskList`, not in this repo's tracked files"*), while **the other five reported the
  opposite equally explicitly** (W1-T2617, W1-T2618, W1-T2628, W1-T2620, W1-T2621 — two of them
  re-deriving line numbers *from the task file's own instruction to do so*). **The population splits in
  half, and the half that lacks a record spends its recon turns proposing follow-ups that ask for it.**
...
- **★ P55 (measurement + golden; R31's mint — ★ CLAUSE (i) REFUTED BY R32's DELETION EXPERIMENT AND
  ...
  the corpus it reads. **R32's AND R33's DELETION EXPERIMENT AND R34's BYTE-SLICE READING — FOLDED TO
  THEIR STATEMENTS BY R50; each was argued at length once and git holds it.** R32 DELETED the
  retro-authored prose (the only experiment DR-9 allows) and the line **PERSISTED**, so clause (i) —
  *exclude retro-authored regions from the corpus* — is **refuted and struck**, and the rung was telling
  the truth about a genuinely unwired symbol (`mainHealthFromRollup`, T2204/#2691) all along. R34 then
  caught the rung quoting a **mid-word byte slice** as its "capability sentence": it cites a window of
  characters containing the symbol, never a claim. **What survives is clause (ii)**, and those two
  readings are its whole case. **KILL TRIGGER NOT FIRED through R49; UNEXERCISED at R50**, which reads
  SHIPS-UNWIRED CLEAN a sixth consecutive cycle — an empty returned set cannot change, so the trigger
  had nothing to measure (P48: recorded as unexercised, never as passed).
...
- **★ P42 (measurement; sibling of P40; PROMOTED TO RANK 4 BY R46, now rank 7 after P73's and P74's
  ...
  **★ R66: HELD, WITH A REASON-PRESERVATION TEST.** MAST leaves `blocked_transient`×1,
  `blocked_transient:openweight_error`×2 and `incomplete`×19 unmapped. The generic and reasoned
  transient labels are one P42 mechanism because neither prints a run exemplar; a seeded golden must
  retain the `openweight_error` suffix when the mapping and exemplar path are added.
  ...
  **★★ R46: THE INPUT ARRIVED, AND IT ARRIVED AS THE MAJORITY.** `incomplete` is **26 of 40 runs — 65%
  ...
  entry was recorded as unexercised because the mix never produced the input** (an ABSENCE, never a
...
- **★★★ P44 (plan + golden; **AGE 7**. ★ **R59's reading: `blocked_budget` returns at
  1 of 40, resetting the two-consecutive-in-band-zero falsifier. MAST maps the one unmerged failure to
  `specification`, but the gather still omits the run identity, limit id, cap value and spend-at-block.
  PROPOSED GOLDEN: a synthetic limit trip emits and renders all four fields. PROPOSED TASK: persist those
  fields atomically with the verdict and refuse an anonymous MAST join. No new proposal id is needed.**
  ★ **R58's prior zero advanced the falsifier to 1 of 2 before this reset.**) — PROMOTED 37 → 3 BY R53 ON AN INVERTED MEASUREMENT — THE
  ANCHOR, NOT A MEASUREMENT, AND IT IS NOW FIRING.** GROUND TRUTH (mechanical, measured at `dcbe275`):
  **254 of 315 tasks carry `budget_usd: 100.00` exactly**, and the entire plan uses **13 distinct values,
  every one a round multiple**. Against that constant the observed figures were avg **$4.915**/run, a
  most-expensive single run of **$17.676**, R12's peak $35.384, and **0 / 34 trips** — the zero that kept
  this entry in the tail for twenty-six cycles. **★ R53's READING INVERTS IT: `blocked_budget`×7 of 28
  runs (25%), the window's LARGEST failure class**, while the same gather prices the entire ~12-hour
  window at **$99.311** against a published **$500/day** committed ceiling (capability snapshot,
  `resolveDailyCostCeiling`) — **so the limit that fired was a per-task/per-spawn cap, not the fleet
  ceiling, and seven runs produced nothing.** Peak credited run cost in the same window is **$9.136**:
  **the "$100 tripwire holds for the whole population" line this rung published for nineteen cycles was
  measuring the SURVIVORS**, and it is deleted rather than incremented. **★ AND THE INSTRUMENT HALF, NEW
  WITH THE PROMOTION:** the gather prints the CLASS COUNT and nothing else — **no limit id, no cap value,
  no spend-at-block reading** — so the largest remediable failure class in the window is also the only one
  nothing can act on. **BAR, REGISTERED VERBATIM AS R53-1:** *state `blocked_budget`'s count and share,
  the window's total measured spend, and the published daily ceiling; then state whether the gather names
  which limit fired, its value, and the run's spend at that moment — or declare those unavailable.*
  **FALSIFIER:** `blocked_budget` at zero for two consecutive windows at comparable dispatch volume makes
  the 7 fires a window artefact and **returns this entry to the tail it was promoted from.** The plan already concedes the failure in BOTH directions: W1-T3 died `blocked_budget`
...
- **★ P38 (plan + golden; RANK 9, HELD as of R30) —
  ...
  **★★★ R48 EVIDENCE, IN PLACE — THE STRONGEST MEMBER YET: THE JUDGE REVERSED AN ARM WHILE ITS OWN
  CONFIDENCE ROSE.** `askuserquestion-auto-resolves-empty-headless` was **PROPOSED for `user-overall`
  at 0.89** last cycle and is **DECLINED as project-specific at 0.95** this cycle, over a corpus still
  frozen at **79 entries, `0 added`, a TWENTY-THIRD cycle**. Every prior member of this series was a
  score wandering under a stable verdict; this one is the VERDICT flipping while the score moves the
  WRONG WAY for it. `a-rigged-probe-measures-nothing` moved **0.95 → 0.82** (promoted both times) and
  `a-roster-error-needs-a-second-derivation` **0.70 → 0.75 → 0.68** (declined throughout).
  **A wandering score is a wobbly instrument; an arm that flips while its own confidence rises is a
  judge whose published number carries no information about the entry it scores** — which settles
  R40-6's separability test in the direction R40 feared: no threshold on `c` can order these decisions.
  ...
  **★★ CLAUSE (iv), R29's — HARDENED BY R30 FROM *UNSTABLE* TO *OSCILLATING*, REWRITTEN BY R33 TO
  *NOISE*, AND FOLDED TO THIS STATEMENT BY R48 (R31's drift and R32's period-2 characterisations both
  died in the cycles that proposed them; git holds the arguments).** The series it was argued on —
  `body-fetch-guards-on-http-not-size`, judged seven times over a corpus frozen at 79 for zero writes —
  **has now LEFT the promotion block entirely**, so re-printing its seven readings each cycle prices a
  corpus that no longer contains it. **What survives is the clause, not its first exhibit:** a single
  reversal is consistent with a corrected mistake and justifies only (iv-a); a round trip is not, which
  is what makes **(iv-c)** — *report the DISAGREEMENT, never the later verdict* — load-bearing.
...
## 4C. External scan (2026-09-02): OpenClaw 2.0 and Grok Bot — taken, refused, already held (★ FOLDED BY R57)

**★ FOLDED FROM 79 LINES TO THIS BLOCK BY R57** — the scan's argument was made once, in the pass that
filed it, and git holds it; what outlives it is WHAT WAS TAKEN, WHAT WAS REFUSED, and the rule the
refusals share. [research: openclaw-2026.8.1, grok-bot-2026-08-11]
**TAKEN — NINE SHARDS, FILED 2026-09-02, each naming its source:** **W1-T2694** ratification pins ·
**W1-T2695** the derived authority table · **W1-T2696** signed single-use option links in the escalation
ping · **W1-T2697** an actor stamp + operator-verb miner ("the operator teaches by doing") ·
**W1-T2698** the credential-reach probe (shrink-only baseline per host class) · **W1-T2699** secrets
injected only at the provider boundary · **W1-T2700** the untrusted-content envelope, ratcheted ·
**W1-T2701** learning-provenance gates (taint at promotion, forget-by-origin as `contested`) ·
**W1-T2702** a bundle that carries routines as staged proposals.
**REFUSED, WITH THE REASON:** shared multi-user sessions (the 2026-08-18 one-operator-per-VM ruling);
desktop egress routing and Chrome-profile import (this harness drives no browser); screen-recorded skills
(the ledger is the better recorder — W1-T2697); widget dashboards with per-widget grants (W1-T2695's
table first); an xAI provider rung (no shard is filed on a plan nobody holds); standing intents (merge
holds and typed escalation options cover the first cases).
**THE PARITY CLAIMS THAT SURVIVE THE FOLD:** compaction doctrine (§8B) against full-transcript-every-turn;
one worktree + one containment per worker against one cloud computer per account; executable acceptance
proofs against evidence-as-a-user-rule; `origin:`-labelled policy rows; the retro's consolidation against
Grounded Dreaming, whose taint gate is the one delta this scan took.

## 4D. Concept scan (2026-09-03): a compact dialect for the fleet — taken, refused, already held (★ FOLDED BY R57)

**★ FOLDED FROM 69 LINES TO THIS BLOCK BY R57**, on the same doctrine. [research: compact-dialect-2026-09-03]
**THE MEASURED FINDING THAT OUTLIVES THE SCAN:** the fleet ALREADY speaks a compact dialect — a rendered
prompt of six named parts (`implementPromptParts`, `outputContractLines`) out, and a code-parsed marker
grammar back (`PR_URL:`, `DECISION_REQUEST`, `QUESTION`, typed `## Follow-ups`, `ALREADY_SATISFIED:`,
`SHIPS-UNWIRED:`, the `Remudero-Task:` trailer), with the proof dialect as a third machine language.
**No human-readable file in this repository reaches a dispatched worker** — `spawnWorker` passes
`settingSources: []`. **THE STANDING RULE:** every compact form here has a code parser and a
plain-language rendering, or it is prose; a form only a model can decode is the Shogtongue failure.
**TAKEN — EIGHT SHARDS, FILED 2026-09-03:** **W1-T2759** (the worker lane never paid the context tax —
corrects a claim this plan held) · **W1-T2760** `LEARNINGS_USED:` · **W1-T2761** the headline index as a
policy-gated seventh prompt part · **W1-T2762** the proof dialect rendered from the parser's constants ·
**W1-T2763** one tracked macro table (`tddr`, `grfp`) · **W1-T2764** the ledger step-name registry ·
**W1-T2765** the prompt-manifest digest · **W1-T2766** the skill-workshop lane (whose drafts this retro's
gather still workshops unfiled).
**REFUSED, WITH THE REASON:** a token counter (budgets here are bytes by design); LLMLingua-style
compression of injected learnings (learnings are instructions, not context); model-invented shorthand;
shorthand as console commands ("A REPLY IS AN INPUT, NEVER A COMMAND", W1-T2496); renaming step names or
verbs (query keys, ruled untouchable 2026-08-15); Chain-of-Draft output limits; KV-cache sharing across
...
29. **A RUN IDENTIFIER THAT IS CONSTANT ACROSS RUNS IS NOT AN IDENTIFIER.** Any lane whose runs share
   one task key (`RETRO`, and the `TRIAGE-fb-*` family one level down) cannot be credited per run:
   the first merged PR that ever carries that key becomes the trailer match for every run of the
...
- verify-human:W1-T3186 (dissolve NEEDS ME into classified ASK/RECORD routing: shared classifier, INBOX for asks, CHANGE MANAGEMENT for records) — RATIFIED 2026-09-09 -> W1-T3394/W1-T3395/W1-T3396.
