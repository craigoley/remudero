...
- **ci-gate REQUIRED checks**: 22 — ci, lint-plan, depcruise, containment-probe, coverage-ratchet, mutation-ratchet, jscpd-gate, claims, learnings-budget-ratchet, commitlint, api-client-drift, no-hand-rolled-fetch, scan-pr / osv-scan, License Review, leak-grep, assertion-discrimination, task-id-existence, acceptance-author-gate, unwired-gate, comment-load-ratchet, source-size, coverage-session-blanking — source: `REQUIRED` (.github/workflows/ci-gate.yml, job `ci-gate`)
