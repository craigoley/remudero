...
export function renderFixPrompt(opts: {
  ...
  if (mode === "merge-conflict") {
    ...
    return [
      ...
      `still relies on. If EITHER side DELETED something, or the conflict is SEMANTIC, you are the`,
      `bounded repair worker: inspect the ACTUAL conflict hunks, merge-base, surrounding callers and`,
      `tests; preserve each side's intended behavior. Never mechanically take an entire side with`,
      `--ours/--theirs, and never force-push. If those facts do not support a defensible semantic`,
      `resolution, leave the branch unchanged and report the exact hunk and why; do not manufacture`,
      `a resolution. A pushed head must then pass fresh review and CI before it can auto-merge.`,
      ...redundantBlock,
      "",
  ...
  if (mode === "proof-discrimination") {
    ...
    return [
      ...
      `with exactly this block, one numbered entry per proof named below, claim and old_proof copied`,
      `byte-for-byte from the listing:`,
      "",
      "PROOF_AMENDMENT:",
      "1. claim: <the claim text, copied exactly>",
      "   old_proof: <the proof text, copied exactly>",
      "   new_proof: <your replacement proof>",
      "",
      ...rendered,
      ...footer,
    ].join("\n");
