...
/** Checks an open-weight worker may run, as fixed argv — never a command string (W1-T3615).
...
function executeOpenWeightTool(name: string, args: Record<string, unknown>, cwd: string): unknown {
  switch (name) {
    ...
    case "run_check": {
      // W1-T3615. Argv BUILT FIRST, so an unlisted check refuses before anything spawns; execFileSync
