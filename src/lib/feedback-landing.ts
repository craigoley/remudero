...
/**
 * Resolve the identity for this commit at the moment the bridge writes it. The container
 * entrypoint exposes the same variables for every other git writer; reading process.env here
 * keeps the landing bridge from bypassing that deployment-specific, Vercel-verifiable identity.
 * The purpose-scoped bridge identity remains the safe default for unconfigured environments.
 */
