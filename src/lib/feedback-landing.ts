...
const CI_LEARNING_REL_DIR = "plan/tasks.d";
...
function ciLearningPrBody(unlanded: string[]): string {
  return [
    `Lands ${unlanded.length} pending \`plan/tasks.d/**\` CI-learning shard(s)`,
...
const CI_LEARNING_LANDING_KIND: LandingKind = {
  branch: CI_LEARNING_LANDING_BRANCH,
  ownedDir: CI_LEARNING_REL_DIR,
...
function ciLearningPendingRelPaths(stateRoot: string): string[] {
  return listRelFiles(ciLearningPendingRoot(stateRoot), CI_LEARNING_REL_DIR).sort();
}
...
function ciLearningShardRelPath(draft: CiLearningShardDraft, taskId: string): string {
  const stem = kebabSlug(draft.title, CI_LEARNING_SLUG_MAX).replace(/-+$/, "");
  return `plan/tasks.d/${taskId}${stem ? `-${stem}` : ""}.yaml`;
}

function readPendingCiLearningInputs(stateRoot: string): LandContentInput[] {
  return ciLearningPendingRelPaths(stateRoot).map((relPath) => ({
    relPath,
    content: readFileSync(ciLearningPendingAbsPath(stateRoot, relPath), "utf8"),
  }));
}

function acknowledgeMergedCiLearningShards(stateRoot: string, git: GitExec): void {
  for (const relPath of ciLearningPendingRelPaths(stateRoot)) {
    try {
...
export function landCiLearningShards(
  ...
  checkoutRoot: string,
  deps: LandCiLearningShardsDeps,
): CiLearningFilingResult {
  const git = deps.git ?? defaultGit(checkoutRoot);
  const held = new Set([...deps.planOrigins, ...ciLearningPendingOrigins(deps.stateRoot)]);
  const skipped: string[] = [];
  const refused: { findingId: string; reason: string }[] = [];

  try {
    git(["fetch", "origin", "--quiet"]);
    acknowledgeMergedCiLearningShards(deps.stateRoot, git);
  } catch {
  ...
  for (const draft of drafts) {
    ...
    const relPath = ciLearningShardRelPath(draft, taskId);
  ...
  const inputs = readPendingCiLearningInputs(deps.stateRoot);
  if (inputs.length === 0) return { filed: [], skipped, refused };

  const landing = landContent(checkoutRoot, CI_LEARNING_LANDING_KIND, inputs, deps);
  try {
    acknowledgeMergedCiLearningShards(deps.stateRoot, git);
  } catch {
