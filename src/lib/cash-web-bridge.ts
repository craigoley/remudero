...
// network.
//
// The cash adapter implements a closed set of functions (`OPENWEIGHT_FUNCTIONS`) and deliberately
// refuses any declared tool outside it, so every lane whose tool set names `WebSearch` — triage
// most visibly — is unroutable to cash today. The worker itself must stay off the network: it has
// no forge authority and no shell, and granting it either to reach a search engine would undo the
// containment the whole provider is built on.
//
// So the DAEMON performs the search on the worker's behalf and hands back a result. The worker
// never holds the credential, never opens a socket, and cannot reach a host the daemon did not
// choose. What it receives is a document the daemon has already checked for attribution.
//
// THREE PROPERTIES THIS MODULE EXISTS TO HOLD, none of which a prompt can enforce:
//
//   1. ATTRIBUTION. A reply is accepted only when the provider both reports having searched
//      (a `web_search_call` item) AND returns at least one `url_citation`. Un-attributed prose
//      from a search tool is indistinguishable from the model answering out of its own weights,
//      which is the one thing a search is supposed to rule out. See {@link cashWebSearchGrounding}.
//
//   2. THE CHARGE SURVIVES THE REFUSAL. Every outcome carries its token counts, including the
//      refusals. A search that timed out, overran the size cap, or came back un-attributed was
//      still billed by the provider, so the caller must settle it against the day cap exactly as
//      it settles an accepted one. An outcome type that could not express "refused, and it cost
//      this much" would leak spend on precisely the failures we expect most.
//
//   3. A REFUSAL IS A TOOL RESULT, NOT AN EXCEPTION. One failed search must not destroy a worker
//      run that was otherwise progressing — the model can reword the query or proceed without it.
//      That is why this module throws nothing for a bad search and returns a discriminated
//      outcome instead; the adapter renders a refusal into the tool message the model reads.
