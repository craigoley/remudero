...
/** Ruling 2026-09-24 (amended twice): only a DECLARED danger band keeps risk:high on Opus; the rest is routine, mid tier. */
export const ROUTINE_ROUTE_CLASS = "routine";

export const implementRouteClass = (task: { type: string; risk: string; band_meaning?: string }, taskClass: string): string =>
  task.type === "implement" && taskClass !== DESIGN_TASK_CLASS && task.risk === "high" && task.band_meaning !== "blast-radius"
    ? ROUTINE_ROUTE_CLASS
    : taskClass;
