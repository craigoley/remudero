...
export interface RegistryInstance {
  ...
  live: boolean;
  /** Shadow instances exercise the pipeline without publishing a merge. */
  mode: "shadow" | "live";
}
...
/** A target with no registry row keeps the historic live behaviour. */
export function instanceMode(registry: InstanceRegistry, ownerRepo: string): "shadow" | "live" {
  return registry.instances.find((row) => row.live && row.repo.toLowerCase() === ownerRepo.toLowerCase())?.mode ?? "live";
}
