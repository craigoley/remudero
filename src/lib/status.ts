 ...
 *
 * Precedence, highest first: `correction.provenance` (DECLARED, SUPREME); (a) `pr.opened`; (b) an explicit
 * `pr:` field; (b2) `manual.completed`; (c) a merged PR with the anchored `Remudero-Task:` trailer. First rung
 * to resolve a MERGED PR wins. INVARIANT: an open-PR ledger claim is the WEAKEST evidence here, so every rung
 * is checked for a merged credit before one may stand (W1-T116).
 ...
 *  (W1-T119, W1-T1029). Why: a false `source: "none"` mis-filed W1-T116 as not-merged */
...
/**
 * Classify a failed `gh` invocation's status, stderr and Node error `code`. Pure, and exported so both gateways
 * and the tests share ONE rule. THE TRAP: a `maxBuffer` overflow has no stderr — Node kills the child first —
 * so it is detected from `code === "ENOBUFS"`, checked FIRST. Why: as "unknown" it hid the 2026-07-20 outage
 */
export function classifyGhFailure(
  ...
  // A KILLED-ON-TIMEOUT CHILD CARRIES NO STDERR either: MEASURED, `execFileSync` past its `timeout` throws
  // ETIMEDOUT with empty stderr. "transport", a call that never returned being network-class (W1-T181).
...
/** True iff `err` classifies as `rate_limit` (W1-T468) — the one place that decision is made. */
 ...
 * these calls are synchronous, a hang is not an error, and nothing is logged. Sized so it cannot fire on a
...
export interface PrRef {
   ...
   *  call. Omitted ⇒ the branch is treated as unowned. */
  headRefName?: string;
  /** The exact current head commit, same board fetch (W1-T2727). Omitted means unknown, never an exact match. */
  headRefOid?: string;
  /** The PR's raw body text (W1-T2392), riding the SAME batched fetch. Omitted ⇒ title alone. */
  body?: string;
}
...
export interface StatusProjection {
  taskId: string;
  /** W1-T2392 — A MERGED BUILD NOBODY CREDITED: not merged, yet a merged PR touching `src/` names this task in
   *  prose. A WARN, never a block — see {@link uncreditedBuildWarning}. */
  uncreditedBuild?: UncreditedBuildWarning;
  /** W1-T2397 — AN OPEN, UNMERGED BUILD ON ANOTHER BRANCH. A REPORT ONLY: see {@link openSiblingBuild}. */
  openSiblingBuild?: OpenSiblingBuild;
  /** Derived label, DELIBERATELY within {@link TaskStatus}'s closed set, which daemon.ts's `reconstructOrphan`
   *  and openapi/daemon.yaml depend on. The finer taxonomy rides the sparse fields below (W1-T155). */
  status: TaskStatus;
  ...
  prUrl?: string;
  prState?: string;
  /** LEGIBILITY (P16 / W1-T69): trailer hits REJECTED by rung (c), with reasons, so a false trailer is VISIBLE. */
  rejected_candidates?: Array<{ pr: string; reason: string }>;
  /** CURRENT phase of an in-flight run (W1-T155). A fresh `run.start` resets the scan to `recon`, so an earlier
   *  run's phase never leaks in — the named falsifier. */
  phase?: Phase;
  /** ISO-8601 timestamp of the in-flight run's `run.start` ledger line. Present iff `phase` is. */
  startedAt?: string;
  /** Milliseconds since `startedAt`, as of THIS derivation (`deps.now()`) — re-derived fresh, never cached. */
  elapsedMs?: number;
  /** W1-T944: the run's CURRENT `worker.state`, off the SAME scan `phase` came from, and only once the run has
   *  emitted such a row — absent renders "state unknown", not a healthy default. */
  ...
  /** The live issue state could NOT be confirmed OPEN. FAIL-CLOSED (W1-T182): an unreadable state KEEPS the
   *  row — the opposite direction from W1-T181's merged-count boundary. */
  escalationUnverified?: true;
  /** `ts` of the escalation's own `escalation.issue_opened` line (W1-T159). "How long has this needed a human"
   *  MUST use this, never `startedAt`: they name different events. */
  escalationOpenedAt?: string;
  /** The current OPEN PR already has auto-merge armed, observed via the SAME batched fetch (W1-T155). */
  armedAwaitingMerge?: true;
  /** INDETERMINATE (W1-T119): `source: "throttled"` — the read FAILED rather than resolving to "no evidence".
   *  A caller gating dispatch MUST treat this as DO NOT ACT. */
  indeterminate?: true;
  /** The CLASSIFIED reason behind an `indeterminate` projection (W1-T119) — throttle, auth-expiry or outage. */
  unavailableReason?: GhFailureReason;
  /** MONOTONIC UNDER DARKNESS (W1-T179): precedence fields carried forward because this cycle's read failed.
   *  The stamp is the START of the current unbroken run of failures; only a successful read clears it.
   *  Why: the 12:24->12:58 fail-open regressed credited tasks to `queued` */
  githubUnobservableSince?: string;
  /** LIVENESS BOUND (W1-T179): DISPATCHED with no terminal verdict, and neither an open PR nor activity within
   *  the bound backs it up — a crashed worker's stale trace. Why: the W1-T1 27h21m spin-loop fixture */
  orphaned?: true;
  /** W1-T485: NOT merged, yet one of this task's own proof symbols shipped to a declared path under ANOTHER
   *  task's trailer. AN OBSERVATION, NEVER A VERDICT: `merged`/`status`/`source` are untouched beside it. */
  supersededBy?: SupersessionEvidence;
  /** W1-T507: filed `verify: human` and not merged — the exclusion from machine dispatch, made visible to a
   *  person. Set by {@link deriveStatus} itself, a pure function of this call's own resolved state, which keeps
   *  every caller DERIVATION-EQUIVALENT. Why: `needsHuman` here would render an affordance with nothing to
   *  click */
  verifyHumanPending?: true;
  /** W1-T951 DELIVERABLE B: this MERGED task's credit rests on EXACTLY ONE path, one of them a branch ref
   *  GitHub deletes on merge. Why: 18 ids were credited by head-branch alone */
  singlePathCredit?: true;
}

/** The GitHub queries deriveStatus needs, behind an interface so unit tests can inject fixtures for all three
 *  precedence sources without touching the network. */
export interface GitHub {
  ...
  /** EVERY merged PR carrying `taskId`'s anchored trailer, newest-first (W1-T441) — the candidates {@link
   *  findMergedByTrailer} discards by returning only the first, the CLOSE path manufacturing duplicates that
   *  displace the implementation. null on a FAILED read, [] on a genuine miss.
   *  Why: 16 of 496 trailered ids have more than one PR — docs/forensics/status.md */
   ...
   *  task's run prefix — deterministic, NOT the body index, where an exit-0 EMPTY result is INDETERMINATE.
   *  Callers RE-ASSERT ownership; null on FAILURE, distinct from [].
   *  Why: one body-index miss caused four spurious re-dispatches */
  ...
  /** The PR's head branch name, or undefined when unresolvable. Backs rung (c)'s ownership-assert (P16). */
  headRefName(prUrl: string): string | undefined;
  /** The PR's raw body text, or undefined when unresolvable. Backs rung (c)'s anchored-trailer verify (P16). */
   ...
   *  surface, off the SAME memoised index. NOT A RELAXATION of {@link creditsByAnchoredTrailer}: the index
   *  extracts an exact anchored line and refuses every token the run-id grammar rejects. FAILS CLOSED. */
   ...
   *  be evidence FOR a credit, whereas WITHDRAWING one would re-dispatch finished work. */
   ...
   *  most recently COMPLETED attempt. Why: asking used to PERFORM the read */
  readFailed?(): boolean;
  /** The CLASSIFIED reason the most recent failed read failed (W1-T119); consulted only after `readFailed()`,
   *  absent ⇒ `"unknown"`. NEVER FORCES A FETCH. Scoped to the PR channel; the issue channel has its own. */
  readFailureReason?(): GhFailureReason | undefined;
  /** W1-T2219: the state the failure pair cannot express — see {@link GhReadState}. Never forces a fetch, and
   *  sits beside the pair rather than in place of it. */
  readState?(): GhReadState;
  /** The most recent read SUCCEEDED but only PARTIALLY, hitting {@link BOARD_MAX_PAGES} (W1-T415). A THIRD
   ...
   *  Why: the measured cold-walk cost is in docs/forensics/status.md */
  resetFailureFlags?(): void;
  /** Resolve an escalation issue's LIVE state and title by url (W1-T182). THE FAIL-SOFT DIRECTION INVERTS every
   *  other optional method here: omitted, or `null`, means "cannot confirm this is closed", so the escalation
   *  stays `needsHuman`, marked unverified. */
  issueByUrl?(url: string): { state: string; title?: string } | null;
  /** The most recent {@link issueByUrl} read genuinely FAILED — scoped to the independent issue fetch, so the
   *  two flags never conflate a PR outage with an issue-read outage. */
  issueReadFailed?(): boolean;
  /** W1-T2219: the CLASSIFIED reason the most recent failed issue fetch failed — classified and logged all
   *  along, but with no accessor. Absent ⇒ `"unknown"`. */
  issueReadFailureReason?(): GhFailureReason | undefined;
  /**
   * W1-T914: the `remudero-review` commit-status state for `prUrl`'s CURRENT head — the SAME vocabulary
   * lib/review.ts posts, off the combined-status endpoint open-prs-rest.ts documents. FOUR ANSWERS THAT MUST
   * NOT COLLAPSE: `"none"` is a successful read with no entry; `"not-applicable"` (W1-T2235) is a MERGED or
   * CLOSED row, answered with no call; `undefined` is a read that could not be trusted — collapsing it into
   * `"none"` renders an outage as a fact about the PR.
   */
...
/** Default store location: a sibling of the ledger this call is already scoped to, so the common case needs no
 ...
 *  the sole route to a credit, so a read failure degrades to "nothing durable yet". */
 ...
 *  a reader mid-write never sees a torn store. BEST-EFFORT: a write failure is swallowed, the projection that
 *  found this credit being already correct. */
...
/** DELIVERABLE B: every id credited by EXACTLY ONE of the two paths. The defect is not that single-path credit
 ...
 *  NON-ENUMERABLE own property (W1-T206) — non-enumerable so `assert.deepEqual` against a plain array literal
 *  keeps working, exactly like a real array's own `.length`. */
export type LedgerLines = Array<Record<string, unknown>> & {
   ...
   * defect `torn` was added for, W1-T206). NOT SET BY AN INJECTED READER, so `undefined` reads as "did not
   * report presence", never "absent". Why: a dead daemon and an absent ledger both read `daemonLive: undefined`
   */
  readonly present: boolean;
};

/** Attach {@link LedgerLines}' non-enumerable read metadata. Both properties describe THIS read, never the
 *  file's contents — hence riding the returned array rather than a wrapper no existing call site could take. */
...
export function readMergeCreditedTaskIds(
  ...
  // cheap if the stop test is cheap too.
 ...
 * call, so every call — checked by {@link ledgerReadIntentViolations} — must declare `ledger-read-intent: live`
 ...
 *  value changes the return: declaring `"union"` does not make a bare call read rotations. This only makes the
 *  CHOICE visible (W1-T1262). */
export type LedgerReadIntent = "live" | "union";

/** One undeclared {@link readLedgerLines} call site, named — file and line — never folded into a bare count
 *  (W1-T1262's fourth acceptance claim: "the check names the offending reader"). */
 ...
 *  unless an intent is declared. Both markers PASS equally: this polices whether a choice was DECLARED, never
 *  which was correct (W1-T1262). */
 ...
 *  bytes from `start` to EOF — never the whole file. Property-accessed at call time, as above. */
...
/** Persistent state a {@link readLedgerTail} caller holds ACROSS calls — one per long-lived route, never
 *  rebuilt per render. `lines` is the SAME array reference every call and only ever appended to, so a prior
 *  return value stays valid. */
export interface LedgerTailCache {
  ...
  /** Cumulative count of unparseable/torn lines dropped across every read this cache has ever done (W1-T206) —
   *  only incremented, so it survives everything `lines` survives, a rotation event included. */
  torn: number;
}
 ...
 *  UNCHANGED file costs one `statSync`. A file SHORTER than last observed degrades safely by rescanning from
 *  byte 0. Why: a console refresh degraded into an O(history) operation, behind the 2026-07-20 latency outage */
export function readLedgerTail(
  ...
  for (const raw of segments) {
    ...
    try {
      ...
      // Loud, not silent — a torn append must never be masked as a fabricated `{}` standing in for the record.
...
/** The PR-precedence fields ONLY from a prior {@link StatusProjection} (W1-T179), EXCLUDING the taxonomy layer,
 *  which is RE-DERIVED from the ledger. Carrying it forward unfiltered would leak a STALE `needsHuman`/`phase`
 *  a later, unobserved event already superseded, since `deriveStatus` never clears a stale `true`. */
...
/** Parse a PR number off the END of a ref or url (W1-T130) — pure text parsing, never a gateway call. Backs
 *  ONLY the correction rung's decoration: a correction's url is trusted TEXT, never re-resolved, so there is no
 *  `PrRef` to read `.number` off. Undefined degrades the decoration, never the verdict. */
 ...
 * but each of the ten helpers then walks that whole array once per task. BUCKETED ON THE TWO ID FIELDS, NOT
 * ONE: nine helpers filter on `task_id` but one filters on `task`. Each helper re-filters its own bucket, so
 * the bucket need only be a SUPERSET in ledger order — which makes the substitution an identity, and a drifted
 * index falls back to a full scan. Why: 776 ms per projection at 1,393 tasks x 29,567 rows
 */
...
export interface ManualCompletion {
  actor: string;
  ts: string;
  /** OPTIONAL: a FULL PR url, self-describing its own owner and repo, never a bare number — the shape that lets
   *  this credit name a PR in ANY repository, not just the calling gateway's. */
  prUrl?: string;
}
 ...
 *  a completion with no PR at all. A line qualifies ONLY with both `actor` and `ts` non-empty. REVERSIBLE
 *  without deletion, since a later `correction.provenance` row is checked above this rung. */
...
/** PER-TASK DISPATCH CIRCUIT BREAKER (MASTER-PLAN P29(ii)) — policy-as-data: how many times the SAME task may
 *  be dispatched with no NEW owned PR since. §9's per-WORKER tripwire's per-TASK dual, and the BACKSTOP that
 *  makes P29(i)'s fix safe to get wrong. Why: the W1-T1 storm ran ~130 dispatches / ~$130 / ~10h */
export const DEFAULT_MAX_TASK_DISPATCHES = 5;

/**
 * Does this ledger line record a MERGE CREDIT for its task? ONE DEFINITION, TWO CONSUMERS: the defect it was
 * extracted for is two mechanisms holding the same fact and never comparing notes. CORRECTED 2026-08-13 — this
 * comment used to conclude "so rotation cannot drop either out from under a reader", AND THAT WAS FALSE; it is
 * the belief that hid a live defect for months. "Was this EVER credited" needs {@link readMergeCreditedTaskIds}.
 */
...
/**
 * W1-T2425 — THE PRIOR COUNT THIS PROCESS DID NOT LIVE THROUGH. The breaker's regression guard compares against
 * an in-memory Map rebuilt PER INVOCATION, so a rotation plus a daemon RESTART was never covered: a new process
 * reads a shortened live file as forward progress. TWO KEYS, DELIBERATELY: the reset lines key on `task_id`,
...
export function seedCountFromCircuitBreak(
  ...
  for (const line of indexedTaskRows(lines, taskId, index)) {
    ...
    // A row with no usable count records that a refusal happened, not what it decided on: absence stays absence.
...
/** True once `taskId` has been dispatched {@link DEFAULT_MAX_TASK_LIFETIME_DISPATCHES} times EVER — unaffected
 *  by any `pr.opened`. Consulted ALONGSIDE the streak breaker, never in place of it. */
 ...
 *  (W1-T206): a task's count dropping without the `pr.opened` line that would explain it. In-memory only, so a
 *  RESTART starts empty; {@link isDispatchBreakerTripped} covers the cross-restart case. */
export interface DispatchBreakerCache {
  /** @internal highest per-task `dispatchesWithoutNewOwnedPr` count observed while the ledger was readable. */
...
/**
 * One dispatch-breaker outcome. `"indeterminate"` fires instead of a false `"clear"` in the two cases where
 * trusting a fresh count would be trusting an ABSENCE as proof (W1-T206): the ledger file is gone after this
 * cache has seen a nonzero count, or the fresh count is LOWER with no `pr.opened` line to explain the drop. The
 * caller treats it as skip-this-tick — never escalates on it alone, never folds it into escalate-now.
 */
 ...
 *  and nothing else. Every field is a value the decision CONSUMED, from the one evaluation that produced
 *  `state`, never a second call that could answer differently. */
export interface DispatchBreakerDetail {
   ...
   *  have been seeded from the newest breaker-refusal row (see {@link seedCountFromCircuitBreak}), so this can
   *  be present on a process's very first call. */
  priorCount?: number;
  /** Whether a `pr.opened` line exists — the regression check's second term. */
  hasNewOwnedPr: boolean;
  /** The three-way corroboration answer. ABSENT when it was never consulted; absent and `"unreadable"` are
   *  different facts, and this repo has collapsed that distinction repeatedly. */
  corroboration?: "corroborated" | "not-corroborated" | "unreadable";
}

/** The core evaluator — the two public entry points are thin `.state` reads over it. Corroboration runs only
 *  when the caller PASSES the key: presence, not value, because `undefined` is a real answer meaning "the
 *  gateway offered no read", reported as `"unreadable"`. */
export function evaluateDispatchBreakerDetailed(
  ...
  // observation outranks the ledger's), and only from the lines already read here.
  ...
  // already tripped. Resolving the thunk at the call site would evaluate it on every call, clear tasks
  // included. An array, null or undefined passes through untouched.
 ...
 *  ledger has no local `pr.opened` line? The streak reset is a statement about a BRANCH NAME, visible
 *  identically from every host. THREE-WAY, NOT BOOLEAN: collapsing `"unreadable"` into `"not-corroborated"`
 *  would let a network blip silently withdraw an already-legitimate reset. */
 ...
 *  verdict is handed back. ONLY `"tripped"` is reconsidered, and only in ONE direction. UNREADABLE FALLS BACK
 *  TO THE LOCAL COUNT — this can only relax a trip, never add one. */
...
/** The open-head-branch list, or a thunk producing it on demand (W1-T2318), so the boot path gets the ABILITY
 *  to walk the board without paying for the walk: the only site that resolves it sits past the already-tripped
 *  guard. Array, `null` and `undefined` behave exactly as before. */
 ...
 *  event): the operator has established the credit is wrong, so it must never re-surface. */
 ...
 * directly and checked BEFORE rungs (a)/(b)/(c), a stale `pr.opened` being no more trustworthy than the fuzzy
 * search this outranked. The actual PR is NOT re-subjected to the ownership and anchor asserts — a deliberate
 * human act SUPERSEDES them, and the real PR is often hand-authored from a non-`run-` branch. Last wins.
 */
...
/** Extract the task id a fleet-dispatched run branch claims, or `undefined`. The ONE named extractor for a
 *  shape tested three other ways: {@link ownsBranch} VERIFIES a known id and cannot discover one, and
 ...
 * read, having no `run-` prefix (W1-T2629). PURE: the caller decides whether the id is credited. TRAP 1,
 * REUSED RATHER THAN REINVENTED: a bare prefix match would let `W1-T15` credit from a `W1-T152` branch, so
 * normalisation DELETES the hyphen boundary and one check replaces it — a candidate matches at the START of the
 * normalised head AND the next character is NOT A DIGIT. LONGEST MATCH WINS, the candidates being unordered.
 */
 ...
 *  opened from THIS task's own branch. LOAD-BEARING for the fix rung too (W1-T76), which amends the SAME run
 *  branch. SCOPE NARROWED 2026-07-30 (operator ruling): for a MERGED PR an anchored trailer is sufficient and
 *  the branch NAME may not veto it, but this REMAINS the gate for non-merged PRs and for rung (c2).
 *  Why: the old rule stranded W1-T64's real PR #115 permanently */
...
export interface BranchFacts {
  name: string;
  /** The most decisive PR state for this head: merged beats closed beats open. `"unknown"` means the caller
   *  could not prove a complete read, and must NEVER be produced by treating one as `"none"` (W1-T2246). */
  prState: "merged" | "closed" | "open" | "none" | "unknown";
  /** The tip is already an ancestor of main. `"unknown"` means the check could not be evaluated, and is
   *  DELIBERATELY NOT `false`, which is a decided "not an ancestor" (W1-T2246). */
  tipInMain: boolean | "unknown";
  /** The branch NAME appears in `src/`, `scripts/`, `deploy/` or `.github/`. */
  namedInSource: boolean;
  /** PATCH-ID EQUIVALENCE (W1-T2247): the cherry check decides only a quarter of a held set alone and MISLABELS
   *  A SQUASH-MERGE AS ABSENT, so it is never consulted once `prState` has resolved a branch. It exists for the
   *  OTHER shape: no PR at all, but commits already equivalent to main's. Omitted = not proven equivalent. */
  ...
  namedTaskId?: string;
  /** WHETHER {@link namedTaskId}'s task is merge-credited, per the caller's OWN existing read — never a second
   *  definition of merge credit. Omitted beside a present `namedTaskId` reads as "not proven credited". */
 ...
 *  spot must not be reported as a genuine "no PR ever". The two named-task reasons (W1-T2629) are NOT a sixth
 *  deletable disjunct: a task shipping elsewhere proves nothing about THIS branch's commits. */
...
export interface BranchReapPlan {
  ...
  guarded: string[];
  hold: string[];
  /** SUBSET OF `hold` (W1-T2246): held because one of the two facts could not be PROVEN, not because they were
   *  decided. A manifest calling both cases "no PR" cannot tell an operator which held branches are the only
   *  copy. */
  undetermined: string[];
  /** Branches the NAME GREP protects that the declared list does not — the drift signal. */
  undeclaredGuards: string[];
  /** DECLARED, but no branch of that name is on origin (W1-T2228). `facts` holds only branches that EXIST, so
   *  this is the one comparison that must run against the full declared list. Citation status is irrelevant
   *  here: an ephemeral guard is routinely absent between landings, so "no branch on origin" is not drift. */
  missingBranches: string[];
  /** EVERY BRANCH NAMES THE TEST THAT PLACED IT (W1-T2247): one reason per entry, WITHIN whichever bucket the
   *  name landed in, so a reader greps it instead of re-deriving "why". */
 ...
 *
 * PROTECTION IS EVALUATED FIRST AND WINS OUTRIGHT — the whole safety argument: a landing branch can be both a
 * MERGED PR head and a live code constant, so the other order would delete a branch its own source names.
 ...
 * Why: `heartbeat` and the ephemeral landing branches — docs/forensics/status.md
 */
export function planBranchReap(
  ...
  // longer on origin. PURE and UNCONDITIONAL: this function has no citation information, so the caller, which
  // has the citation scan, decides a dead declaration from an ephemeral one (W1-T2228 (v)).
  ...
  for (const f of facts) {
    ...
    // squash-merged branch must never read "absent" merely because its patch id differs). `patchIdEquivalentIn-
    // Main` only ever SPLITS THE REASON for an already-deletable merged head.
    if (f.prState === "merged") {
    ...
    // THE FOURTH DISJUNCT (W1-T2247): no PR at all, and every commit patch-id equivalent to one already in main
    // — content-identical even where the tip check reads `false` or `"unknown"`, since a rebase changes the sha
    // without changing the patch. Removing the ref cannot lose information.
    ...
    // `"unknown"` PR state means merged/closed was never ruled out; `"none"` paired with an `"unknown"` tip
    // means the third disjunct's own precondition was never provable — "could not tell" (W1-T2246).
    if (f.prState === "unknown" || (f.prState === "none" && f.tipInMain === "unknown")) {
      ...
      // W1-T2629: the branch's name declares a task id the run-anchored extractor cannot read. NEITHER outcome
      // moves this branch out of `hold`: a task shipping by another route does not prove THIS branch's commits
      // are in main, so this is a mechanically-checkable CITATION for the same adjudication, never a verdict.
      plan.reasons[f.name] = f.namedTaskCredited === true ? "named_task_credited" : "named_task_open";
    } else {
      // Confirmed: no PR ever, tip not an ancestor, not patch-id equivalent, and no resolvable task named —
      // the residue W1-T2247 exists to size correctly (20, not 75).
 ...
 *  mechanically. A judgement without a falsifier is a guess: a citation names EITHER a plan task id the change
 *  would serve OR a symbol and path in the CURRENT tree, both re-checkable by a grep. */
...
/** THE VERDICT CARRIES ITS CITATION OR IT IS REFUSED (W1-T2247) — refused outright, never downgraded, because
 *  an accepted guess is indistinguishable from a checked answer downstream. PURE: it only checks that a
 *  citation is PRESENT and non-empty, leaving the walk of the plan and tree to the caller. */
 ...
 *  merged PR #793 is this shape. Kept separate so rung (c2), which credits BY branch, stays strict. */
 ...
 * `-<epochMs>` suffix", so such a PR vetoed its own credit permanently, a merged PR's head never changing. THE
 * TRAILING `-` IS THE ENTIRE PREFIX-COLLISION GUARD (TRAP 1), KEPT OUT OF `ownsBranch`: widening that would
 ...
 * DIFFERENT task — that, and ONLY that, still vetoes a trailer credit, a branch carrying no task claim vetoing
 * nothing. PREFIX COLLISION (TRAP 1) is held off by the BOUNDARY CHARACTER, not by avoiding prefix matching:
 * {@link isOwnedSlugBranch} does prefix-match, and the required hyphen is what makes it safe.
 */
 ...
 * a MERGED PR is sufficient evidence and the head-branch NAME may no longer veto it, WITH a guard that a branch
 * clearly belonging to a different task can never credit. TRAP 2 — NO NON-MERGED CREDIT PATH: `state` is
...
function creditsByAnchoredTrailer(
  ...
  taskId: string,
  /** W1-T2387: does the COMMIT surface carry this task's anchored trailer for this PR? The union searches two
   *  surfaces, so the re-verify must check both or it discards the candidate the search just produced. Defaults
   *  FALSE. Why: measured on #3005, where the search returned the PR and this function threw it away */
  commitCredits = false,
): boolean {
  // W1-T2387: EITHER anchored surface satisfies this, and NOTHING below is relaxed — the branch veto, the
  // unreadable-head fail-closed and the non-merged ownership check all still apply.
  ...
  // UNREADABLE HEAD FAILS CLOSED (W1-T119, unchanged by the ruling). An ABSENT head ref is a read that FAILED,
  // not a branch that carries no claim. The ruling removed the veto power of a branch NAME we can SEE.
  if (!head) return false;
  return !branchClaimsOtherTask(head, taskId);
}

/** `findMergedByTrailerAll`'s fetch bound (W1-T441). The measured worst case is six; this leaves an order of
 *  magnitude of headroom without becoming an unbounded list. */
 ...
 *  NOT in plan scope. Why: the narrow one isolates 3 of 16 multi-hit ids; this isolates all five generators */
 ...
 * KEPT: unreadable is missing information, not evidence. A SECOND LAYER (prefer a candidate touching `src/`)
 * WAS BUILT, MEASURED AND REMOVED — it changed the chosen PR in ZERO of the 16 multi-hit ids.
 * Why: this isolates 8 of the 16, including 5/5 generators
 */
...
/** RUNG (c) PLAN-ONLY REFUSAL (W1-T413): true iff `files` is non-empty and EVERY path is in plan scope ({@link
 *  isInPlanScope}, CALLED rather than re-derived) — such a PR filed or closed a task, it did not build one.
 *  EMPTY IS FALSE, deliberately: an empty list is what an unreadable read looks like, and "every element of
 ...
 *  (c) credit a worker's own PR without reading its file list — the pre-filter that keeps {@link
...
/** RUNG (c) ANCHORED-TRAILER VERIFY (P16 / W1-T69): the merged-trailer search is a fuzzy full-text body search,
 *  capable of matching a PR whose trailer names a DIFFERENT, prefix-sharing id — the "W1-T20c false-credit"
 *  class this rung ratifies. That hit is a first pass; this is the authoritative check. */
 ...
 *  MERGED and its trailer already credited the task, but its head was never an accepted claim form, so the gap
 *  leaves no trace. `"open-unattributable"`: still OPEN on an unaccepted head, so no `pr.opened` line is
 *  written and a second dispatch stays admissible until it merges. */
 ...
 * branch that claims the task in NONE of the three accepted forms? A PLAN FILING NAMES NOTHING (acceptance 3):
 * {@link hasAnchoredTrailer} is the FIRST gate, so a filing returns `undefined` before the head is inspected.
 * OWNERSHIP IS RE-USED, NEVER RE-DERIVED (4). A MERGED PR IS STILL CREDITED (1 REPORTS, never DENIES). PURE
 * (6), and NEVER CONSULTED BY DISPATCH (5).
 */
...
function derivePrPrecedence(task: Task, deps: DeriveDeps, ledgerLines: Array<Record<string, unknown>>): StatusProjection {
  ...
  // (b2) LEDGER-RECORDED MANUAL COMPLETION (W1-T1029) — rung (b) widened to the two shapes a `pr:` number
  ...
  // (a) ledger `pr.opened` -> query that PR. A MERGED resolution returns immediately; a NON-merged one is
  // stashed rather than returned — SIBLING CREDIT (P29(i)): a later redispatch's own closed or open PR must
  // never permanently mask an earlier sibling run's already-merged credit found at rung (c).
  // Why: this was the W1-T1 spin's mechanism — docs/forensics/status.md#second-pass-2026-09-06
  ...
  // (b) explicit `pr:` field — W1-T116: ALWAYS consulted, even when (a) captured a non-merged result. THE
  // INVARIANT: an open-PR ledger claim is the WEAKEST evidence in the system and must never outrank a
  // VERIFIED-MERGED credit found anywhere else. A non-merged (b) result still never displaces an existing one.
  // Why: docs/forensics/status.md#second-pass-2026-09-06
  ...
  // (c) a merged PR carrying the anchored `Remudero-Task:` trailer — ownership-asserted and correction-aware
  // (P16 / W1-T69). deriveStatus GATES DISPATCH, so a false credit makes the daemon BUILD against an unmet dep.
  // SIBLING CREDIT (P29(i)): reached even when (a)/(b) captured a non-merged result, ownership assert UNCHANGED.
  // (c2) HEAD-BRANCH CORROBORATION (W1-T256): rung (c) reads the eventually-consistent BODY index, where an
  ...
  const corroborateByBranch = (): StatusProjection | undefined => {
    ...
    // rung above returned early if it had it — so persist it now, and the NEXT derivation resolves from the
    // store even after GitHub deletes the head ref. Best-effort: a write failure never blocks this projection.
  ...
  /**
   * (c3) OPEN HEAD-BRANCH CORROBORATION (W1-T377) — the LAST rung, consulted only once every merged path has
   * declined. THE GAP IT CLOSES: rung (a) is the only route to an OPEN credit and it reads the LEDGER, while
   * (c2) covers MERGED only, so an open PR whose `pr.opened` line never landed is invisible. NEWEST WINS. FAIL
   * DIRECTION, deliberately: a false OPEN credit DEFERS a dispatch while a missed one DUPLICATES a build.
   */
  ...
  if (trailerPr && !debunkedTrailerUrls(ledgerLines, task.id, ledgerIndex).has(trailerPr.url)) {
    ...
    // W1-T2387: the union's own second surface, off the SAME memoised commit index the search used. Absent
    // gateway method ⇒ false ⇒ body-only, exactly as before.
    ...
    if (!planOnlyRefusal && wouldCredit) {
      ...
      // own-branch PR (TRAP 2's fallback), and the durable store records MERGE credit alone.
    ...
    // Rejected: a branch claiming ANOTHER task, a non-merged PR off a foreign branch, or an unanchored hit.
    // Corroborate by head branch first, so a foreign trailer hit cannot mask this task's own merged run, then
    // surface WHY (W1-T69) only when (a)/(b) found nothing either.
    if (!ownResult) {
      const branchCredit = corroborateByBranch();
      if (branchCredit) return branchCredit;
      // (c3) here too — this early return is a SECOND exit concluding `source: "none"`, and a task whose only
      // trailer hit was foreign or unanchored is exactly as dispatchable-looking as one with no hit at all.
      ...
      // the accept test did not act on: an unanchored body reports that regardless of branch.
  ...
  // corroborate by head branch first. A merged, ownership-asserted branch hit is a sibling credit exactly like
  // rung (c)'s, and WINS over (a)/(b)'s non-merged `ownResult`.
  const branchCredit = corroborateByBranch();
  if (branchCredit) return branchCredit;

  // No merged sibling credit found: fall back to (a)/(b)'s own non-merged resolution.
  if (ownResult) return ownResult;

  // (c3) Nothing merged anywhere and NO association of this task's own — the one state in which an open PR the
  // ledger never recorded is the best evidence available. Strictly BELOW every rung above, so it only ever
  // fills a hole.
  const openCredit = corroborateOpenByBranch();
  if (openCredit) return openCredit;

  // No GitHub evidence: not merged. EXCEPT (W1-T119) when that "no evidence" is really "GitHub could not be
  // read", which must defer — a PARTIAL read too (W1-T415): by this line every rung has credited anything the
  // truncated view DID contain, so this arm catches only an absence, as unsound partial as failed.
  if (deps.github.readFailed?.() || deps.github.readTruncated?.()) {
    ...
    // `queued`. Carry the prior conclusion forward and mark the gap. The stamp is the START of the current
    // unbroken run of failures, so consecutive failures report the same instant.
    const previous = deps.previousProjection?.(task.id);
    if (previous) {
      const now = deps.now ?? Date.now;
      return {
    ...
    // No prior observation to fall back on (this task has never been seen) — nothing to keep monotonic, so the
    // pre-W1-T179 shape stands.
    return {
      taskId: task.id,
      status: "queued",
      merged: false,
...
interface RunState {
  ...
  startedAt?: string;
  /** `ts` of the LATEST ledger line naming this task, any step (W1-T179's liveness heartbeat). Every append
   *  stamps `ts`, so this is a real proxy for "the worker is still doing something". */
  lastActivityTs?: string;
  /** W1-T944: the newest worker-state row's state for THIS run, reset to `undefined` on every fresh {@link
   *  LANE_START_STEPS} entry exactly like `phase` resets — an earlier run's state must never leak forward. */
  workerState?: WorkerState;
 ...
 *  single literal this used to switch on, MEASURED to be blind to the other six. A given id only ever carries
 *  ONE lane's steps, so one shared scan cannot conflate two lanes. */
 ...
 *  LANE_START_STEPS} (W1-T282: "the close side is not uniform and must not be pretended uniform"). The retro
 *  and triage lanes have only an error terminal, a SUCCESSFUL run of either logging none, so their close relies
...
function deriveRunState(
  ...
  for (const line of indexedTaskRows(lines, taskId, index)) {
    ...
    // Set-membership checks rather than case labels: this function's reads are display-only.
    ...
    // time. The comparison reads the bare local, for the harvesting reason above.
 ...
 *  redispatch (W1-T155). DELIBERATELY DOES NOT ANSWER "is the escalation still open": the ledger is
 *  append-only, so it can say one WAS opened but never that it has since closed — that needs {@link
 *  resolveEscalation}'s live join. An absent url never suppresses the escalation, only the join. */
export function latestEscalationLine(
  ...
  for (const line of indexedTaskRows(lines, taskId, index)) {
    ...
    if (line.step === "run.start") {
      ...
      // W1-T159: the escalation's OWN ledger-line ts, so a caller measures "how long has this actually needed a
      // human" against the escalation's real open time, never a run's `startedAt` — a DIFFERENT event.
 ...
 *  and the two must never be unified behind one "unreadable" policy. */
export function resolveEscalation(
  ...
  // No issue_url at all (a malformed or pre-W1-T8 line) ⇒ nothing to join against — the same fail-closed
  // treatment as an unreadable url, never a dropped row.
  let issue: { state: string; title?: string } | null = null;
  if (latest.issueUrl) {
    try {
      // A THROWING issueByUrl — an injected fixture that raises, or an unanticipated gateway — must NEVER
      // propagate out of deriveStatus and crash the whole projection instead of degrading this ONE task to
      // unverified. This call is EXTERNALLY supplied, so it gets its own catch.
  ...
  // sees lowercase ones for the SAME resource — two real, coexisting conventions.
...
/** Derive one task's FULL status taxonomy (W1-T155, MASTER-PLAN §7/§9): {@link derivePrPrecedence}'s merge-state
 *  layered with the in-flight phase and elapsed clock, the needs-human flag, and armed-awaiting-merge off the
 *  auto-merge state the batched gateway's single fetch already carries — zero extra calls. Pure over its deps. */
export function deriveStatus(task: Task, deps: DeriveDeps): StatusProjection {
  ...
  if (projection.status !== "blocked") {
    ...
    if (runState.inFlight && runState.phase) {
      ...
      // "running" while BACKED by an open PR — independent, stronger GitHub evidence, never subject to this
      // bound — OR by ledger activity within it. Absent both, it is a stale dispatch and must not render.
      ...
      // ALIVE by `isHolderStale` (W1-T368 — pid liveness alone is not enough, a recycled pid must not count).
      // Both halves are required and neither suffices; absent the dependency the disjunct is skipped.
      ...
      if (hasOpenPr || recentActivity || hasLiveLock) {
        ...
        // open PR is the ONLY reason this row entered the branch, the decoration is honest about resting on a
        // remote fact. Never flips the status word, and never turns a skipped lock read into "dead".
        if (hasOpenPr && !recentActivity && !hasLiveLock) {
        ...
        // lands here too: a stale lock must not resurrect the very defect this rung prevents.
  ...
  // rungs already resolved — never a second, independently-resolved PR reference.
  ...
  // task is theirs. COMPUTED HERE rather than at the projectPlan level: a pure function of `task` plus this
...
// THE GAP THIS FILLS: W1-T458's `unresolved_task_scope` advisory fires when a merging diff resolves NO task, so
// it answers "no task at all" and cannot answer "the WRONG task". A REPORT, NEVER A GATE: a merge-time refusal
// keyed on a trailer mismatch would fire on every FILING, since filings must omit the trailer (#1527).
 ...
 *  when the read FAILED — a failed search must never read as "no evidence". */
...
/**
 * A task's proof-derived symbols paired with every path the task declared. W1-T506 WIDENED THIS FROM
 * `grep:`-ONLY, which was dialect-gated and getting worse as convention moves proofs to `unit test:`, whose
 * body carries a TITLE and never a path; `files:` supplies one for EVERY task. WHY THE PROOF TEXT AND NOT THE
 * FILE PATH ALONE: the cheap predicate was measured and REJECTED — a task naming a busy file is flagged by a
 * file that moves daily, while a proof pattern is forced by the linter to be distinctive.
 * Why: both motivating cases were all-`unit test:` and invisible to the old predicate
 */
...
/**
 * The evidence that this task's substance shipped under a DIFFERENT task's trailer, or `undefined`. FOUR
 * OUTCOMES ARE DELIBERATELY COLLAPSED TO "NO EVIDENCE": `opts.merged` contradicts a stronger signal — and THIS
 * GUARD LIVES HERE, NOT ONLY AT THE CALL SITE (W1-T506); a commit with THIS task's trailer is CREDIT; one with
 * NO trailer is W1-T458's territory; and a `null` search is a FAILED READ. THE OLDEST QUALIFYING COMMIT WINS,
 * the search returning newest-first, so the first hit would credit whichever task most recently touched the
 * symbol rather than the one that introduced it.
 */
 ...
 *  injectable so the predicate is testable without a repository; a throwing exec yields `null`, never `[]`. NOT
 *  WIRED INTO ANY HOT PATH BY DEFAULT: one history search per task per projection would be the O(N)-subprocess
 *  cost W1-T187 and W1-T257 both had to remove. */
 ...
 *  called at TWO sites and the second passes a RUN ID (W1-T1012), so a union that indexed those would credit a
 *  run as though it were a task. The optional trailing letter is real, ids like `W1-T123a` being live.
 *  Why: this rejects 0 of 921 plan ids and 170 of 740 commit tokens, every rejected one a run id */
export const TASK_ID_TRAILER_RE = /^W[0-9]+-T[0-9]+[A-Za-z]?$/;

/** A squash merge's own PR number, off the `(#N)` suffix in the subject. A commit whose subject names no PR is
 *  skipped rather than guessed at: without a number there is no {@link PrRef} to return. */
...
/**
 * W1-T2387 — THE SECOND TRAILER SURFACE, AS AN INDEX: the commit trailer is MACHINE-written, the PR-body one
 * HAND-written, and the merged search read only the latter. A UNION, NEVER A COMPARISON — consulted only after
 * the body surface answers EMPTY, so it can only ADD credit. NO NEW FETCH. `null` on a failed read, never [].
 *
 * ⚠ THE SURFACE DEPENDS ON A REPO SETTING THIS FILE OTHERWISE NEVER NAMES (W1-T2447):
 * `squash_merge_commit_message`, an admin-only UI toggle with no commit, review or ledger row. MEASURED as
 * `COMMIT_MESSAGES`, the ONLY reason the amended trailer survives the squash; the other legal value `PR_BODY`
 * discards the branch commits, so flipping to it stops every FUTURE squash carrying a trailer and the index
 * stops growing while looking as healthy as today. FALSIFIER:
 * `test/commit-trailer-surface-squash-setting.test.ts`. Why: docs/forensics/status.md
 */
export function buildCommitTrailerIndex(opts: {
  ...
  return () => {
    // THE CHECKOUT MUST BE THIS GATEWAY'S OWN REPO, AND THIS IS A DEFECT THIS TASK SHIPPED ONCE AND CAUGHT. A
    // gateway is constructed for an explicit owner and repo, but `git log` reads whatever checkout the PROCESS
    // sits in, and the first draft consulted it unconditionally, so a fixture asking about one repo was
    // answered from another's history. AN EMPTY MAP, NOT `null`: a foreign checkout is a genuine ABSENCE (W1-T119).
    let originUrl: string;
    try {
      originUrl = exec(["config", "--get", "remote.origin.url"]).trim();
    } catch {
      // No `origin` remote, so this checkout cannot be SHOWN to be this gateway's repo — the foreign-checkout
      // case above, a genuine ABSENCE. An empty map, never `null`, which is reserved for a failed read.
      return new Map();
    }
    ...
    try {
      ...
      // `null` is this file's "unreadable", kept distinct from the empty map above (W1-T119).
      return null;
    }
    ...
    for (const record of raw.split("\x1e")) {
      ...
      // THE GRAMMAR CHECK IS LOAD-BEARING, not defensive tidiness: without it every run-id trailer becomes an
      // indexed "task".
...
export function projectPlan(
  ...
  // contents before overwriting them. Fails soft on a missing or corrupt cache.
  ...
  // READ THE LEDGER ONCE (W1-T187): `deriveStatus` parses the WHOLE ledger per call and the loop below calls it
  // once PER TASK — clocked at 5-8s per projection against the console's <2s budget. The path cannot change
  // mid-loop, so it is parsed once and every task is handed the SAME array.
  ...
  // EVERY projection — the 07-23 GraphQL-exhaustion multiplier. Fetch every merged PR's head ref ONCE and match
  // CLIENT-SIDE, on the STRUCTURED ref. A FAILED batch yields `null` for every task, so derivation falls back
  // per-task and, failing that, W1-T119 defers.
  ...
  // BATCHED rung (c3) CORROBORATION (W1-T377) — the same batch-once shape, over the OPEN slice of the SAME
  // fetch. Free on `buildBatchedGithub`; one extra list read on `ghGateway`. A FAILED read yields `null` for
  ...
  if (allOpen !== undefined) {
    ...
    // failed read stays `undefined` so the observation does not fire rather than reporting an unseen absence.
  ...
  // W1-T951: ONE flush for the whole plan sweep, skipped entirely when nothing new was discovered — the common
  // case once a plan's credits are mostly durable, so a steady-state poll loop costs zero writes.
  ...
  // and every existing test drives, is left byte-identical: an additive observation about already-resolved
  // tasks, not a new rung. Skips merged, and skips `indeterminate`, whose `merged: false` was never observed —
  // reporting off an unread state would be the fail-open direction W1-T119 prevents.
  ...
  if (supersessionSearch) {
    for (const task of plan.tasks) {
      ...
      // `merged` is always false here, the loop above having skipped a merged projection. Passed explicitly so
      // the invariant is the PREDICATE's, not just this loop's — see {@link findSupersessionEvidence}.
  ...
  if (cachePath) {
    ...
    // ATOMIC WRITE: four call sites share this path and a 250ms-polled reader can land mid-write. A torn read
    // parses as garbage and the reader fails soft to `undefined`, silently discarding the very "last
    // successfully observed projection" W1-T179 depends on. Temp file then rename, salted by pid and a random
    // suffix so two writers never collide.
 ...
 *  undefined on anything short of a well-formed prior write, never throwing. Not exported: a caller wanting a
 *  `previousProjection` for other reasons injects one directly. */
...
/**
 * W1-T2399 — THE THREE READINGS THAT USED TO COLLAPSE INTO ONE `undefined`: no required contexts, an empty
 * list, and a read that FAILED OUTRIGHT. The caller sets one boolean from it, so a repo-wide outage was
 * reported as a claim about the PR's own checks. The same split W1-T2370 made between `unverifiable` and
 * `refuted`. FAIL-SOFT IS UNCHANGED (W1-T176 (ii) is NOT reopened).
 */
...
/** W1-T2399: the classified read of a repo's REQUIRED status-check contexts, straight from GitHub branch
 *  protection (W1-T103, the #170 stuck-ambiguous fix) — read ONCE per repo and branch. */
...
export function ghRequiredStatusCheckContexts(owner: string, repo: string, branch = "main"): string[] | undefined {
  ...
  // `string[] | undefined` contract. `none` and `unreadable` both answer `undefined` here — precisely the
  // collapse the classified form avoids, so a caller needing them apart calls that directly.
 ...
 *  (W1-T119) is an INJECTABLE stand-in for the raw invocation. */
export function ghGateway(
  ...
  // task has missed on the body surface. NOT TTL'd, commits being append-only, so a stale index can only LACK a
  // very recent merge — the case the body surface answers.
  ...
  // since is untrustworthy as "absent". A short-lived gateway cannot know which calls shared the outage.
  let failed = false;
  ...
  // Same sticky-failure bookkeeping as the JSON path, for the ONE call reading `--jq`-filtered lines:
  // paginating without `--jq` emits one JSON array PER PAGE, which the parser rejects outright.
  ...
  // `ref` is either a bare PR number or a full PR URL. REST addresses a PR by number only, so both are folded
  // down here rather than at each call site.
  ...
  // GraphQL's search connection whatever flags are passed. The qualifier language is shared verbatim, so only
  // the transport changes. Sorting is pinned to creation order, which the pre-conversion callers relied on.
  ...
  return {
    prByRef(ref) {
      // "title" rides this SAME fetch (W1-T184) — a decoration, never an extra call.
    ...
    findMergedByTrailer(taskId) {
      // Body search for the exact trailer, merged only, newest first. Fuzzy (P16 / W1-T69): a first pass, never
      // the authority, so callers re-verify head ref and body before crediting.
      ...
      // nothing. `null` means the read FAILED, and an outage must keep reporting as one (W1-T119).
      if (list === null) return null;
      return commitTrailerFallback(taskId)[0] ?? null;
    },
    findMergedByTrailerAll(taskId) {
      // W1-T441: the SAME body search widened past the single-hit limit. Unlike the batched twin this DOES cost
      // a wider fetch, hence a separate method; {@link TRAILER_ALL_LIMIT} bounds it.
      const byBody = searchMergedPrs(`"Remudero-Task: ${taskId}" in:body`, TRAILER_ALL_LIMIT);
      // W1-T2387: same union, body first — see the sibling above.
    ...
    findMergedByHeadBranch(taskId) {
      // W1-T256's STRUCTURED qualifier. Fuzzy, since a prefix can over-match, so the head ref rides along and
      // the caller re-asserts ownership. REST's search result carries no head field, being an Issue shape, so
      // each bounded hit pays one more read to recover it.
    ...
    listMergedHeadBranches() {
      // The closed listing carries CLOSED-and-unmerged rows too, so `prStateFromRest` rather than a raw state
      // comparison separates the two. Contract: {@link GitHub.listMergedHeadBranches}.
    ...
    listOpenHeadBranches() {
      // W1-T377: the OPEN twin, off the deterministic LIST API. Contract: {@link GitHub.listOpenHeadBranches}.
    ...
    changedFiles(prUrl) {
      // W1-T413. The files endpoint has no field-selection story, so filenames come straight off `--jq`.
...
export interface BatchedPr {
  ...
  headRefOid?: string;
  body?: string;
  /** GitHub's raw auto-merge field (W1-T155): absent when not armed, an object when it is. Carried verbatim,
   *  never pre-reduced, so the gateway applies the SAME null test `ghGateway` and run-task.ts already use. */
 ...
 *  escalation counterpart to {@link BatchedPr}, so resolving 44+ escalated rows costs the SAME one call. */
...
/** The commit-status context the merge gate keys on — a LOCAL literal rather than an import, exactly like
 ...
 * its own gap state, which is the defect under a new name. ITS BLOCKING WAIT IS REAL EVERYWHERE EXCEPT UNDER
 * THE NODE TEST RUNNER: measured on test/status.test.ts alone, 1.3s before this existed, 64.4s with a plain
 * default.
 */
 ...
 *  can prove the DEFAULT is shared and overridable without depending on load order across that file's other
 *  ~140 tests. No production path calls this. */
...
// `GitHub.warm?(): void` is fire-and-forget and cannot be awaited. Before this task it called the two index
// builders DIRECTLY, and both walk the same synchronous call {@link GH_CALL_TIMEOUT_MS} names as parking the
// whole process: a COLD walk is 18-22s, so a request arriving during it queued behind a refresh it never asked
// for. Widening the signature to a promise reaches every implementer and caller, which this task does not own.
// SO THE WALK MOVES TO A SEPARATE OS THREAD, reusing the same exported fetch helpers UNCHANGED. THIS FILE IS
// LOADED TWICE, `isMainThread`/`workerData` gating the branch; `workerData` is structured-cloned, so an
// injected exec keeps the sync path.
const BOARD_PREWARM_WORKER_KIND = "remudero-board-prewarm-walk" as const;

/** What the main thread hands the worker — plain data only, per `workerData`'s structured-clone contract. The
 *  two `known` maps are the SAME delta caches the fetch helpers carry between refreshes; a Map clones
 *  structurally, so the worker's copy is a snapshot, never a live handle into this process's memory. */
...
/** One channel's outcome, classified exactly like `attemptFetch`'s own catch (same {@link classifyGhFailure}),
 *  so a worker-side failure reads identically to a synchronous one. */
 ...
 *  the response, mirroring W1-T181's "the catch classifies and marks, it never propagates". */
 ...
 *  calls it on the main thread, which keeps that fallback binary-aware and gives every line real coverage. */
function runPrewarmChannelsSync(req: PrewarmWorkerRequest): PrewarmWorkerResponse {
  ...
  // share one gap-tracking object. Priced, not hidden: a narrower rate-limit exposure, worth a follow-up, never
...
/**
 * THE WORKER BRANCH's whole body, as a named function rather than an inline statement. EXTRACTED FOR COVERAGE,
 * AND THE EXEMPTION ROUTE IS CLOSED BY DESIGN: Node's coverage instruments the PARENT only, so a statement
 * running solely inside a spawned `Worker` records zero hits, and diff-coverage refuses the process-boundary
 * directive unless the declaration is re-exec or exit glue. The guard below stays a ONE-LINE `if` on purpose.
 */
...
// with the matching kind; a normal main-thread load leaves it untouched.
if (!isMainThread && isPrewarmWorkerRequest(workerData)) postPrewarmWorkerResponse(parentPort, workerData);

/**
 * A GitHub gateway answering ALL of {@link GitHub}'s methods from ONE batched fetch of the repo's PRs, held in
 * memory with a short TTL, instead of one subprocess PER call: {@link ghGateway}'s trailer search runs one
 * search PER task, so `projectPlan` over 183 tasks made O(N) sequential subprocesses and took ~74s. Its trailer
 * search matches the ANCHORED line, so `W1-T1` never mis-selects a `W1-T15` PR.
 *
 * W1-T181: a THROWING fetch was the deep defect, the pre-fix catch converting "I could not read GitHub" into
 * "GitHub says there are zero PRs". The catch now wraps `fetchAll` itself, and the flags back `readFailed()`,
 * consulted BEFORE trusting an empty result. Why: ENOBUFS carries no stderr — docs/forensics/status.md
 */
export function buildBatchedGithub(
  ...
  opts: {
    ...
    now?: () => number;
    fetchAll?: () => BatchedPr[];
    /** INJECTABLE stand-in for the raw PR-list invocation (W1-T181, mirroring {@link ghGateway}'s seam). Tests
     *  inject a fake returning a large seeded JSON string, or one throwing a classified-failure-shaped error —
     *  without shelling out, and without bypassing the default's parse and byte-size-log wrapper. */
    exec?: (args: string[]) => string;
     ...
     *  OMITTED ⇒ W1-T1005's {@link defaultGhCallPacer}, never a fresh pacer per construction. */
    pacer?: GhCallPacer;
    /** W1-T2323 OPTION C — THE MERGED HALF'S OWN CLOCK, `ttlMs` now governing the OPEN half alone. DEFAULTS TO
     *  `ttlMs`, AND THE DEFAULT IS THE ARGUMENT: MEASURED, full walks come from COLD gateways with no cache to
     *  expire, not from TTL expiry on warm ones. THE WIN IS LAZINESS, NOT LENGTH. */
    mergedTtlMs?: number;
    /** W1-T2440: which binary the WORKER-based warm walk shells — `opts.exec`'s role for the synchronous path,
     *  but a path rather than a function, because `workerData` cannot carry a closure. */
    ghBin?: string;
    /** W1-T2440 TEST-ONLY SEAM: overrides the URL handed to `new Worker(...)`. Every REAL caller omits it. It
     *  exists solely so a test can point a REAL `Worker` at a deliberately throwing script and observe the
  ...
  // W1-T2323: A CACHE MUST SERVE FOR AT LEAST AS LONG AS IT COST TO BUILD. WHAT IT DOES NOT BUY, SAID PLAINLY:
  // the expensive walks are the FULL ones, and they come from COLD GATEWAY INSTANCES, not from TTL expiry, so
  // this DOES NOT REDUCE THAT COUNT. ONE FLOOR PER HALF, NOT ONE PER GATEWAY: a SHARED floor would let the
  // merged walk's duration govern the open clock. `ttlMs === 0` IS EXEMPT ON BOTH HALVES, being the suite's
  ...
  // falls back to the SAME module-scoped instance, rather than each discovering the secondary limit alone.
   ...
   * "GitHub says zero PRs" reading W1-T181 prevents, arriving by a new route. `readFailed()` reports their OR,
   * strictly no weaker than the single flag it replaces. Each reflects only the MOST RECENT attempt (W1-T119).
   */
  ...
  // W1-T2440: true from the moment `runPrewarmWorker` spawns a worker for this channel until its result is
  // applied. The row readers check these FIRST, so a request arriving mid-warm is served the existing cache
  // instead of racing the worker with a second, synchronous fetch on this thread.
  ...
  // W1-T2219: backs `readState()` — not attempted until the index first runs, in flight for one attempt
  // (observable only from a REENTRANT call), then mirroring the failure flag. Unlike that flag, never forced
  // true by an accessor: only an actual query or `warm()` advances this.
  ...
  // `false`. W1-T2323: retained PER HALF, since a later untruncated refresh of one must not clear a truncation
  ...
  // W1-T2323: IN SPLIT MODE THIS HOLDS CLOSED ROWS ONLY, so the stop test compares like with like.
  ...
  // discipline — an independent map, the two deltas having independent row shapes and failure modes.
  let knownIssues: Map<number, BoardIssueRest> | undefined = snapshotCache?.issueSeed();
  /** W1-T413: per-URL changed-file memo. `null` records a read that FAILED, so one unreachable PR is read once
   *  per gateway rather than once per task. */
   ...
   * rows, the guard consulting the cached state only on a cache HIT, so a terminal row fell through to a call
   ...
   *  walk for the 1 request it reads */
  const restFetchHalf = (half: BoardFetchHalf): BatchedPr[] => {
      ...
      // drove ~58% of the account's whole GraphQL budget, and when it ran out long-merged tasks stayed pinned
      // at the head of UP NEXT. The three decoration fields still ride the SAME fetch.
      let bytes = 0;
      ...
      // answer rather than resurrected from a cache. Seeding it would be the bug.
      ...
      // W1-T415: ledgered on every successful fetch and RETAINED so `readTruncated()` can surface it — a
   ...
   *  board in one call and every fixture counts on it being called exactly once per refresh, so such a gateway
   *  keeps the single combined clock it has always had. Real gateways go down the split path. */
  ...
  // reasons, and the fail-closed escalation join needs its OWN signal. Scoped to the needs-human label:
  // MEASURED 2026-08-24, that set is 523-524 rows (~3.04 MB).
  ...
  const fetchAllIssues =
    ...
    (() => {
      ...
      // every page inside one exec call and cannot be interrupted mid-walk. The delta cache carries the COLD PR
      // half's proven stop test over rather than a second mechanism.
      let bytes = 0;
      const fetchJson = (args: string[]): unknown => {
        const raw = run(args);
        bytes += Buffer.byteLength(raw, "utf8");
      ...
      // Reassigned only on a SUCCESSFUL return, exactly like `knownBoardPrs` — a throw skips this line, so a
      // transient failure leaves the previous complete snapshot intact and the NEXT call is still a cheap delta.
  ...
  const issueIndex = (): IssueIndex => {
    ...
    if (!issueCache || now() - issueCache.at >= ttlMs) {
      ...
      try {
        ...
        // failure flag: `issueReadFailed()` tells `resolveEscalation` this is an outage, never a clean miss.
        all = [];
      }
      issueCache = {
        at: now(),
  ...
  // Flexible ref resolution — accepts a full issue URL OR a bare number, mirroring the PR lookup below.
  // escalate.ts always writes a full URL, but a caller resolving by number should not silently miss.
  ...
  /** The two half-stamps `cache` was last composed from — a union rebuild is needed only when one moves, so a
   *  warm `index()` still costs no map construction at all. */
   ...
   * the same millisecond. A monotonic counter per half cannot collide. Why: MEASURED at 8-14 failures per 60
   */
  let openEpoch = 0;
  let mergedEpoch = 0;
   ...
   *  — a second copy is how a fail-closed contract rots. The flags mean "the most recent attempt", by EITHER
   *  half, deliberately the conservative reading. */
  const attemptFetch = (fetch: () => BatchedPr[], channel: "open" | "merged" | "both"): BatchedPr[] => {
      // #2998's start stamp, moved INTO the one shared attempt: there is no longer a single `index()` guard to
      // hang it off, and both halves must earn their own floor.
      ...
      // implementation, so an INJECTED `fetchAll` that throws is classified and marked exactly like a real
      // failure instead of propagating uncaught out of every GitHub method this gateway returns.
      let all: BatchedPr[];
      // W1-T2219: flips BEFORE the attempt so a REENTRANT `readState()` from inside an injected `fetchAll`
      // observes "in_flight" rather than whatever the PREVIOUS attempt left behind.
      fetchInFlight = true;
      try {
        ...
        // LOUD (W1-T181 design (ii)/(v)): the pre-fix catch was silent for hours, with zero error lines, ENOBUFS
        ...
        // BEFORE trusting an empty result sees a MARKED failure, never a bare absence.
        all = [];
      } finally {
        fetchInFlight = false;
        // #2998, PER CHANNEL. Recorded on BOTH arms: a failed fetch blocked the loop just as long as a
        // successful one, so it earns the same floor. On the COMBINED path both halves take the same value.
   ...
   *  COMPLETE answer, never a cache union. ON A FAILED FETCH THE HALF IS REPLACED WITH AN EMPTY ONE AND STAMPED
   *  (the W1-T181 pairing) — never handed out unpaired. */
  const openRows = (): BatchedPr[] => {
    if (!splitHalves) return bothHalves().open;
    // W1-T2440: a background worker already owns this channel's refresh, and serving the possibly stale cache
    ...
    if (!openHalf || now() - openHalf.at >= effectiveOpenTtlMs()) {
      ...
      // here would be a second, weaker opinion about a question the query already settled.
      openHalf = { at: now(), rows: fetched };
      openEpoch += 1;
      // W1-T2323: WHAT SEPARATE CLOCKS COST, AND THE ONE LINE THAT PAYS MOST OF IT BACK. The merged half can now
      // be older, so the merged searches can answer "not merged" about a PR that has in fact merged — and the
      // consumer acting on that decides a task's disposition, not a display. A PR IN THIS REPO ALWAYS MERGES
      // OUT OF THE OPEN SET, so a successful open pass returning without a number the previous one had IS the
      // merge or close, observed, and expiring the merged clock here makes the miss window ZERO. GUARDED ON A
      // SUCCESSFUL FETCH: a failed pass's `[]` read as "every open PR just merged" is the W1-T181 hazard again.
   ...
   *  read off this one index (W1-T377's design), and its walk still stops on the delta boundary with the same
   *  ceiling. The change is WHEN it is fetched: lazily, by a consumer that actually needs a merged row. */
   ...
   *  stamped alike. The pre-split behaviour unchanged, and what every existing fixture runs, so "the split" is
   *  a production behaviour rather than a rewrite of what the suite measures. */
  const bothHalves = (): { open: BatchedPr[]; merged: BatchedPr[] } => {
    if (!openHalf || !mergedHalf || now() - openHalf.at >= effectiveOpenTtlMs()) {
      ...
      // set on every path that lands here. The assertion states that invariant rather than carrying a fallback
      // that could never run — the dead branch coverage-ratchet flagged.
  ...
  /** THE UNION, for every consumer that needs a row of any state. Forces BOTH halves. Only
   *  `listOpenHeadBranches` is routed away from here, being the one method whose answer is a function of the
   *  open half alone — and the one the dispatch path reads. */
  const index = (): Index => {
    // W1-T2323: ON THE COMBINED PATH THIS MUST BE ONE CALL, NOT TWO. Asking for each half in turn would enter
    // `bothHalves()` twice, and at `ttlMs: 0` — which several fixtures use — the second entry sees an already-
    // stale stamp and fetches AGAIN, doubling the injected fetch count, the pacer's retry budget and the rows.
    const both = splitHalves ? undefined : bothHalves();
    const open = both ? both.open : openRows();
    const merged = both ? both.merged : mergedRows();
    // KEYED ON THE REFRESH COUNTERS, NEVER ON THE STAMPS — see `openEpoch`'s doc for the same-millisecond
    ...
    if (!cache || composedFrom?.open !== openAt || composedFrom?.merged !== mergedAt) {
      // MERGED LAST, so a row present in both halves resolves to its TERMINAL state, never the stale open one.
  ...
  /**
   * R-24 — THE PER-TICK VERDICT RESET; see {@link GitHub.resetFailureFlags} for the contract. PER HALF,
   * mirroring the outcome split (W1-T2323): a half whose last attempt SUCCEEDED keeps verdict and rows, so a
   * reset never costs a re-fetch it did not have to pay, while a FAILED half loses both together — the verdict
   * AND the stamped EMPTY half it is the only safe pairing for. The delta caches are DELIBERATELY UNTOUCHED:
   * they are the reason a caller holds one gateway across ticks.
   */
  ...
  // doc and same precedence. Built at most ONCE per gateway and only after a task has missed on the body index,
  // so a board whose PRs all carry the body trailer never shells out. Deliberately NOT TTL'd.
  let commitIndex: Map<string, PrRef[]> | null | undefined;
  const commitTrailerFallback = (taskId: string): PrRef[] => {
  ...
  // These three mirror the synchronous success and failure bookkeeping exactly — same fields, same log events,
  // same W1-T119/W1-T181 discipline. They exist because a worker's result arrives on a MESSAGE, not a return
  // value, so there is no call site left to wrap. Only WHEN it runs changes.
  ...
  const runPrewarmWorker = (): void => {
    ...
    worker.once("error", (err) => {
      ...
      // so every channel THIS call asked for is marked failed, W1-T181's "loud, classified, never silent".
      settle(() => {
        const elapsedMs = Math.max(0, now() - startedAt);
  ...
  return {
    ...
    changedFiles(prUrl) {
      // W1-T413, AND THE O(N) QUESTION ANSWERED RATHER THAN DODGED. The REST PR *list* does not carry changed
    ...
    reviewState(prUrl) {
      // W1-T914, over the SAME combined-status endpoint open-prs-rest.ts documents. Keyed off the PR's own head
      // BRANCH NAME, already on this index at zero cost, rather than its sha: the endpoint accepts any git ref.
      ...
      // any cache lookup and any call. That ordering was the whole defect: the head ref is a branch name GitHub
      // deletes on merge, so the call below 404s for every terminal row, and the catch caches nothing, so the
      // row re-failed on every paint forever.
    ...
    readTruncated() {
      // Same force-a-fetch-first shape `readFailed()` used to have (W1-T415, unchanged here): a caller that asks
 ...
 *  prose would be the over-crediting W1-T2387 ruled out — a task credited wrongly is never built at all. */
export interface UncreditedBuildWarning {
  /** The merged PR that names this task in its own prose. */
  prNumber: number;
  prUrl: string;
  /** Which prose surface carried the id. Measured at head: 5 of 19 name it in the title and 14 only in the
   *  body, so a title-only reader would have missed the instance this exists for. */
  namedIn: "title" | "body";
}

/** W1-T2392: the anchored id form, using an explicit digit class so `W1-T239` never matches a mention of
 *  `W1-T2392`. Built per lookup — see {@link indexProseNamedTaskIds} for why the scan is inverted. */
 ...
 * are read because the measurement says so: of the 19 uncredited builds naming an id at head, only 5 name it
 * in the TITLE.
 */
...
/**
 * W1-T2397 — WHAT AN OPEN SIBLING BUILD LOOKS LIKE, AND WHY IT IS ONLY EVER A REPORT. THE ASYMMETRY THIS
 * NAMES: the eligibility check already sees open PRs but attributes them by {@link ownsBranch} alone — ONE
 * surface, where the merged side now has three — so an operator-briefed build on a `fix/` branch is invisible
 * and the task is dispatched again. A WARN, AND THE MEASUREMENT IS WHY: the naive predicate fired four times in
 * 72 hours and THREE OF THOSE MERGED, and no staleness bound rescues it. IT MUST NEVER FEED THE ELIGIBILITY
 * SIGNAL. Why: the time-to-merge distribution is in docs/forensics/status.md
 */
...
/**
 * W1-T2397 — IS THERE AN OPEN PR BUILDING THIS TASK THAT IS NOT ITS OWN RUN BRANCH? FILE OVERLAP IS THE SIGNAL,
 * NOT THE PROSE, and that is what makes it quiet: of the four naive firings, two siblings were plan FILINGS and
 * two were builds of DIFFERENT tasks mentioning the id in passing, and requiring a path in the dispatched
 * task's own declared `files:` drops both classes by construction. THE TASK'S OWN RUN BRANCH IS EXCLUDED.
 * `changedFiles` UNREADABLE ⇒ NO OBSERVATION, never a guess.
 */
