...
/** W1-T2971 — ONE in-flight ticker at a time, process-wide. Before the retro was detached only one
...
function startInFlightTicker(
  ...
  // W1-T2971: a second concurrent ticker would double every light sweep — hand back an inert one.
  ...
  return {
    stop: async () => {
      active = false;
      // W1-T2971: release the process-wide holder BEFORE awaiting, so a slow or throwing pass can
...
export async function runDaemon(
  ...
  for (;;) {
    ...
    if (deps.checkRetroTrigger) {
      ...
      // W1-T2971 — the REFUSAL is decided BEFORE `retro_triggered` is written, so that row keeps
      ...
      if (decision?.fire && deps.runRetroTrigger && detachedActionInFlight("retro")) {
        ...
        if (deps.runRetroTrigger) {
          // DETACHED, NOT AWAITED (W1-T2971). This await was unbounded and sits ABOVE the dispatch
