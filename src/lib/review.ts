...
export const INSTRUMENT_SURFACE: readonly string[] = [
  ...
  // W1-T4400: the runner that executes ci-gate.yml's aggregation step as ci.yml's last job — the
  // REQUIRED ci-gate context comes through it, so a change to it changes what CI refuses.
...
export const INSTRUMENT_SURFACE_EXCLUSIONS: Readonly<Record<string, string>> = {
  ...
  "scripts/lint-plan-offline.mjs":
    ...
    "test does, through that alias). It RESTATES NO RULE: it is three lines that call the reviewer's " +
    "own `lintPlanCommand` with `{ offline: true }` and exit on its code, so a diff touching it " +
    "cannot change what any gate MEASURES — the same claim, for the same reason, as " +
