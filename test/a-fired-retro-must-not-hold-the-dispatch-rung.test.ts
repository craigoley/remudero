...
// W1-T2971 — A FIRED RETRO MUST NOT HOLD THE DISPATCH RUNG.
//
...
test("W1-T2971: a fired retro does not hold the tick, so dispatch still runs", async () => {
  ...
  assert.deepEqual(
    ...
    "THE CLAIM: the tick reached the dispatch rung while the retro was still running. Before " +
      "W1-T2971 this was empty and no dispatch row was ever written.",
  );
...
test("W1-T2971: a second retro is refused while one is still detached", async () => {
  const plan = fixturePlan();
  const lines: Array<{ step: string }> = [];
  let releaseRetro: () => void = () => {};
  const retroBlocked = new Promise<void>((resolve) => { releaseRetro = resolve; });
