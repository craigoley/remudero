...
test("W1-T3444 criterion 1: a source diff with no plan diff still computes base-side source existence", async () => {
  const fixture = isolatedCheckout(REPO_ROOT);
  try {
    ...
    assert.match(
      ...
      "W1-T3444 source-only fixture preserves zero-plan scope",
    );
  } finally {
    fixture.cleanup();
  }
