...
async function runLintPlanBase(planPathArg?: string, fixtureRoot?: string): Promise<{ exitCode: number; stdout: string }> {
  const logs: string[] = [];
  ...
  console.warn = () => {};
  try {
    const exitCode = await lintPlanCommand(
      planPathArg ? ["--plan", planPathArg, "--base", "HEAD"] : ["--base", "HEAD"],
      fixtureRoot ? { offline: true, repoRoot: fixtureRoot } : {},
    );
...
test("a design-only edit to a real shard raises the --base checked count from 0 to 1 — the #1544 shape locked out", async () => {
  ...
  try {
    const fixturePlan = join(fixture.root, "test", "fixtures", "live-plan-writers", "changed-tasks", "tasks.yaml");
    const target = join(fixture.root, "test", "fixtures", "live-plan-writers", "changed-tasks", "tasks.d", "FIXTURE-C2-design-block.yaml");
    const original = readFileSync(target, "utf8");
    // CONTROL first: with the plan tree clean vs HEAD, --base reports zero changed tasks. If
    // this isolated checkout's fixture is ever dirty at test time the control names it, instead
    // of a concurrent test's temporary write being misattributed to this assertion.
    const before = await runLintPlanBase(fixturePlan, fixture.root);
    ...
    const after = await runLintPlanBase(fixturePlan, fixture.root);
    assert.match(
      after.stdout,
...
test("a source diff with no plan diff still computes base-side source existence", async () => {
  const fixture = isolatedCheckout(REPO_ROOT);
  try {
    const sourceFile = join(fixture.root, "src", "run-task.ts");
    writeFileSync(sourceFile, `${readFileSync(sourceFile, "utf8")}\n// source-only lint-plan fixture diff\n`, "utf8");
    execFileSync("git", ["-C", fixture.root, "add", "src/run-task.ts"]);
    execFileSync("git", ["-C", fixture.root, "-c", "user.name=Fixture", "-c", "user.email=fixture@example.invalid", "commit", "--quiet", "-m", "source fixture"]);

    const changed = execFileSync("git", ["-C", fixture.root, "diff", "--name-only", "origin/main...HEAD"], { encoding: "utf8" })
      .split("\n")
      .filter(Boolean);
    assert.deepEqual(changed, ["src/run-task.ts"], "control: fixture must carry only the source diff that drives the base-side source lookup");
...
test("the changed-task base comparison runs against a fixture shard", () => {
  ...
  assert.match(targetLine!, /fixture\.root/, "the design-only edit must target an isolated fixture checkout");
