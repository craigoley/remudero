...
// W1-T3698. Operator intent, 2026-09-16: "$10 on a normal day and $25 on a day when the
...
test("W1-T3698: a plain number is the whole cap, squeezed or not — existing configs are untouched", () => {
...
test("W1-T3698: a pair uses `normal` for routine work and `squeezed` only when subscriptions are tapped", () => {
...
test("W1-T3698: an absent cap stays absent, so the transport still refuses to run uncapped", () => {
...
test("W1-T3698: a pair whose squeezed is BELOW normal is refused as a transposition", () => {
...
test("W1-T3698: equal figures are allowed — that is an operator opting OUT of the raise", () => {
  assert.equal(effectiveCashCapUsd({ normal: 10, squeezed: 10 }, { squeezed: true }), 10);
});

test("W1-T3698: a non-finite figure refuses rather than reaching the reservation as NaN", () => {
...
test("W1-T3698: the squeeze ceiling is claimed in exactly ONE place, and it is the blocked-auction fallback", () => {
...
test("W1-T3698: the adapter passes the flag to EVERY reservation the run makes, not just the first", () => {
