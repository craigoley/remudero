/**
 * `ledger-compact` must be classified before its COMMANDS entry lands. The console-parity ratchet
 * permits a baseline to sit ahead of the live registry as a reported shrink, so this instrument-
 * only prerequisite keeps the product PR out of Rule 25 entanglement.
 */
...
test("ledger-compact is pre-registered as cli-only without requiring its product command", () => {
  const baseline = JSON.parse(readFileSync(join(ROOT, mod.BASELINE_PATH), "utf8")) as {
    uncoveredVerbs: string[];
  };
  const classification = mod.classifyVerbs(["ledger-compact"], [], {}, mod.CLI_ONLY);

  assert.deepEqual(classification.cliOnlyVerbs, ["ledger-compact"]);
  assert.deepEqual(classification.unmapped, [], "the future command already has a stated CLI-only reason");
  assert.match(mod.CLI_ONLY["ledger-compact"] ?? "", /git-tracked plan\/ledger/i);
