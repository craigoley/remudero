...
// W1-T2971 — the retro is DETACHED now, so `runDaemon` returns while it is still in flight.
...
test("W1-T276: the light sweep runs while a fired retro is in flight, so the sweep is never blind for the retro's whole duration", async () => {
  ...
  const summary = await runDaemon(plan, {
    ...
    sweepLight: async () => {
      lightSweeps++;
      // W1-T2971 — gate on the TICKER's own ticks, not a shared `sleep` counter: the detached retro
