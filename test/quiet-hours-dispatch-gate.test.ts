...
test("W1-T2655: quiet hours defers new dispatch while the daemon sweep still runs in that tick", async () => {
  ...
  const summary = await runDaemon(
    onePlan(),
    {
      ...
      checkQuietHours: () => ({ deferred: true, detail: "quiet-hours test hold" }),
  ...
  assert.ok(lines.some((l) => l.step === "daemon.quiet_hours"), "the hold is ledgered under its own daemon step");
