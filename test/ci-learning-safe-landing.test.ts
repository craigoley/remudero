...
test("CI-learning idempotency reads shard-backed origins and names the canonical CI learning shard", () => {
  const root = mkdtempSync(join(tmpdir(), "rmd-ci-learning-origins-"));
  mkdirSync(join(root, "plan", "tasks.d"), { recursive: true });
  writeFileSync(join(root, "plan", "tasks.yaml"), "[]\n");
  writeFileSync(
    join(root, "plan", "tasks.d", "W1-T9001-existing-ci-learning.yaml"),
    [
      "- id: W1-T9001",
