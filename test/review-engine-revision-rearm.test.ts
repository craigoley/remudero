...
const DISPOSITION_NOW_4042_MS = Date.parse("2026-09-05T08:00:00.000Z");
const PR_BODY_4042 = [
...
test("W1-T2872: the exact PR #4042 old-engine row spends zero current-engine attempts and re-enters post-review", () => {
  ...
  const view: OpenPrView = {
    ...
    priorStrikes: 1,
    lastActivityAt: new Date(DISPOSITION_NOW_4042_MS - 139 * 60 * 1000 - 2766).toISOString(),
    headSha: HEAD_4042,
  ...
  const disposition = deriveDisposition(view, DEFAULT_SWEEP_POLICY, DISPOSITION_NOW_4042_MS);
