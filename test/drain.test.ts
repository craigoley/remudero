...
test("W1-T316 runDrain integration: lifetime pressure is observed without removing the task from the flow", async () => {
  const plan = fixturePlan(); // A -> B -> C (chain), D independent, H human-only
  const merged = new Set<string>();
  const ran: string[] = [];
  const capped: string[] = [];
  ...
  assert.deepEqual(ran, ["A"], "A remains dispatchable after lifetime pressure is observed");
  assert.deepEqual(capped, ["A"], "the caller's onLifetimeCapExceeded observed A exactly once");
  assert.equal(s.stopReason, "max_reached");
  const pressureLine = lines.find((l) => l.step === "dispatch.lifetime_pressure");
  assert.ok(pressureLine, "a dispatch.lifetime_pressure ledger line was emitted");
  assert.equal(pressureLine?.extra.task, "A");
});

test("W1-T316 the W1-T29 x10 shape: lifetime pressure does not spin or re-escalate after the task advances", async () => {
  const plan = fixturePlan(); // A -> B -> C (chain), D independent, H human-only
  const merged = new Set<string>();
  const ran: string[] = [];
  const capped: string[] = [];
  ...
  assert.deepEqual(ran, ["A", "B", "C", "D"], "the task and its dependents advance normally after the pressure observation");
  assert.equal(s.stopReason, "no_runnable");
  assert.deepEqual(capped, ["A"], "onLifetimeCapExceeded fired once for A, not once per poll");
  const pressureLines = lines.filter((l) => l.step === "dispatch.lifetime_pressure");
  assert.equal(pressureLines.length, 1, "pressure is recorded once for the material observation");
});
