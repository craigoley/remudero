...
test("gpt-oss-120b still REFUSES json_object — the original measurement is preserved, not reversed", () => {
...
test("an UNMEASURED deployment declares nothing and may not be asked", () => {
...
test("the refusal names the deployment and what it does support, so a caller can reroute", () => {
  try {
    openWeightResponseFormatField("gpt-oss-120b", "json_object");
