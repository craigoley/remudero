...
test("FAST_GATE_STEPS: exactly seven census entries, each bound at the shared FAST_GATE_CENSUS_BOUND_MS constant — never a one-off number per entry", () => {
  ...
  // The seventh is `self-path-proof-census`, on the same terms again: it parses the tracked
  // plan/tasks.d corpus plus the monolith and holds a per-file baseline, and MEASURED at 1063ms —
  // under the shared bound. It is admitted because the defect it holds is a PR-stopper: a shard
  // acceptance proof that greps its own file passes at the merge base, grades executed_stale, and
  // leaves a correct implementation at FAIL-unmet (#5256, repaired by #5303 and #5310).
  assert.equal(CENSUS_STEPS.length, 7, "four of the six measured census suites, plus authority-census, ledger-literal-census and self-path-proof-census");
...
test("runPreflightFast: run for real (unmocked, real spawn, real package.json) over ONLY the census entries, every one measures under the bound and passes on this HEAD", () => {
  ...
  // The seventh is `self-path-proof-census` (see the count pin above): admitted on the same terms,
  // MEASURED at 1063ms, and it holds a defect that stops PRs rather than one that merely tidies.
