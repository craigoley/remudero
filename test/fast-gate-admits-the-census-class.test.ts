...
test("FAST_GATE_STEPS: exactly four census entries, each bound at the shared FAST_GATE_CENSUS_BOUND_MS constant — never a one-off number per entry", () => {
  ...
  assert.equal(NON_CENSUS_STEPS.length, 8, "the seven pre-existing npm-script gates plus W1-T2491's branch-shape gate are untouched by this task");
});
...
test("runPreflightFast: mocked end-to-end over all eleven FAST_GATE_STEPS, no spawn call ever names test:ci, a bare npm test, or the test/**/*.test.ts glob", () => {
