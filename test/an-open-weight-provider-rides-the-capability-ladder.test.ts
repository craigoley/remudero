...
test("an unlisted openweight check refuses before it executes", async () => {
  // W1-T3615. The refusal must happen BEFORE a process is spawned, so a caller that sees it knows
...
test("no permitted openweight check can reach the network or the forge", () => {
  // W1-T3615. ENUMERATES THE TABLE, never a sample: the failure this guards is someone ADDING a row,
