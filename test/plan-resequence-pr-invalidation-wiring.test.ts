...
test("W1-T3585 criterion 3: buildSweepHook (full sweep) closes the plan-invalidated task-owned PR via the SAME isMerged/readMainPlan seam runSweep's stale-close effect uses", async () => {
...
test("W1-T3585 criterion 3: buildSweepLightHook (light sweep) reaches the SAME current-plan eligibility verdict via the identical isMerged/readMainPlan seam — the pre-existing dangerous-lane restriction still defers the close itself", async () => {
