...
// the jobs .github/workflows/ci.yml actually gates a merge on. This suite proves the
 ...
 *  runCiParity's table has the workflow's job set worth of steps, and most tests below only care about
...
test("ci-parity table: every REAL ci.yml job has an entry, either mirrored (with a run()) or excluded (with a reason) — no entry is silently absent", () => {
  ...
  assert.ok(jobs.length > 0, `expected ci.yml to define at least one job, got ${jobs.length}`);
