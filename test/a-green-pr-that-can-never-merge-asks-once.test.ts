/**
 * W1-T3733 — THE GREEN PR THAT CAN NEVER MERGE, AND SAID SO TO NOBODY.
 *
...
test("W1-T3733: a capped, un-overridden verdict at this head is TERMINAL", () => {
  assert.equal(terminalArmRefusal([posted()], "W1-T1", HEAD), true);
});

test("W1-T3733: everything that CAN still arm reads false, not terminal", () => {
...
test("W1-T3733: ABSENT IS NOT TERMINAL — an unreadable ledger cannot manufacture a question", () => {
...
test("W1-T3733: an operator override makes the refusal non-terminal again", () => {
  const withOverride = [
    posted(),
...
test("W1-T3733: the terminal PR escalates instead of being called mergeable", () => {
...
test("W1-T3733: POSITIVE MATCH ONLY — absent or false still reaches mergeable, unchanged", () => {
...
test("W1-T3733: the row cannot fire on a PR that is not already green and reviewed", () => {
...
test("W1-T3733: the escalation reason names the remedy, not just the refusal", () => {
