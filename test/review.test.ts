...
// `fail` — mirrors scripts/test-with-retry.mjs (W1-T255); see execWhitelistedProof's own doc
// comment for the full rationale. ──────────────────────────────────────────────────────────

/** A {@link ProofSpawner} that throws a clean-nonzero-exit error on its first N calls, then
 * returns `stdout` on every call after — so a test can prove the retry actually happened by
 * COUNTING invocations, the same discipline `countingSpawner` (above) uses for the fast path.
 * Returns the mutable `state` object itself (never a snapshot) so the caller reads `state.calls`
 * AFTER invoking `execWhitelistedProof` — a destructured value would freeze at its pre-call 0. */
function failThenSpawner(failCount: number, stdout: string): { spawner: ProofSpawner; state: { calls: number } } {
  const state = { calls: 0 };
  const spawner: ProofSpawner = () => {
    state.calls++;
    if (state.calls <= failCount) {
      const err = new Error("clean nonzero exit") as NodeJS.ErrnoException & { status?: number; stdout?: string };
      err.status = 1;
      err.stdout = stdout;
      throw err;
    }
...
test("W1-T202 round 2: a whole-file test proof that fails ONCE then passes on retry reports 'pass' — the retry actually ran, counted", () => {
  const dir = mkdtempSync(join(tmpdir(), "rmd-w202-retry-"));
  const wp: WhitelistedProof = {
    kind: "test",
    command: "node",
    args: ["--test", "--import", "tsx", "test/some-flaky-file.test.ts"],
    label: "test/some-flaky-file.test.ts",
  };
  const { spawner, state } = failThenSpawner(1, "");
  assert.equal(execWhitelistedProof(wp, dir, 60_000, spawner), "pass");
  assert.equal(state.calls, 2, "the first attempt failed, the retry ran, and it passed");
});

test("W1-T202 round 2: a whole-file test proof that fails on BOTH the first attempt AND the retry still reports 'fail' — the retry never masks a genuine break", () => {
  const dir = mkdtempSync(join(tmpdir(), "rmd-w202-retry-"));
  const wp: WhitelistedProof = {
    kind: "test",
    command: "node",
    args: ["--test", "--import", "tsx", "test/some-genuinely-broken-file.test.ts"],
    label: "test/some-genuinely-broken-file.test.ts",
  };
  const { spawner, state } = failThenSpawner(2, "");
  assert.equal(execWhitelistedProof(wp, dir, 60_000, spawner), "fail");
  assert.equal(state.calls, 2, "exactly one retry — never a retry storm");
});

test("W1-T202 round 2: the retry is NEVER applied to a name-filtered proof (already handled by nameFilteredOutcome) or a grep proof (deterministic)", () => {
  const dir = mkdtempSync(join(tmpdir(), "rmd-w202-retry-"));
  mkdirSync(join(dir, "test"));
  writeFileSync(join(dir, "test", "foo.test.ts"), 'test("a genuinely present title", () => {});\n');

  const nameFilteredWp = parseWhitelistedProof("unit test: a genuinely present title");
  assert.equal(nameFilteredWp?.nameFiltered, true, "precondition");
  const nameFilteredSpawn = failThenSpawner(1, "not ok 1 - a genuinely present title\n# duration_ms 4");
  // nameFilteredOutcome reads the FIRST attempt's own stdout even off a thrown nonzero exit —
  // a retry here would just re-run the SAME leak-prone full-glob fallback a second time for no
  // benefit, so it must never fire.
  execWhitelistedProof(nameFilteredWp!, dir, 60_000, nameFilteredSpawn.spawner);
  assert.equal(nameFilteredSpawn.state.calls, 1, "a name-filtered proof's failure is never retried");

  const grepWp: WhitelistedProof = { kind: "grep", command: "grep", args: ["-rn", "--", "NEEDLE", "."], label: "NEEDLE in ." };
  const grepSpawn = failThenSpawner(1, "");
  execWhitelistedProof(grepWp, dir, 60_000, grepSpawn.spawner);
  assert.equal(grepSpawn.state.calls, 1, "a grep proof's failure is deterministic and is never retried");
});
