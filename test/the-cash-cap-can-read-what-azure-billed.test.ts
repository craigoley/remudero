...
// ── the remaining refusal arms, each of which returns a DIFFERENT reason on purpose ─────────────
// Every one of these is a way the reading can fail while looking like a quiet day. They are tested
// individually because the reason text is the only thing an operator gets: a single "unavailable"
// with a generic message would have left today's two-day outage just as invisible as it was.

test("a config with no workerProviders block is UNAVAILABLE, not a $0 day", async () => {
  const out = await readCashActuals({ claudeBin: "/unused", root: "/tmp" } as unknown as Config, {
    env: { [CASH_METRICS_RESOURCE_ENV]: "/subscriptions/s/x" },
    clock,
    fetchImpl: (async () => { throw new Error("never called"); }) as unknown as typeof fetch,
  });
  assert.equal(out.kind, "unavailable");
  assert.match(String((out as { reason: string }).reason), /no workerProviders block/);
});

test("IMDS answering without an access_token is UNAVAILABLE, and says so specifically", async () => {
  const out = await readCashActuals(cfg, {
    env: { [CASH_METRICS_RESOURCE_ENV]: "/subscriptions/s/x" },
    clock,
    fetchImpl: (async () => new Response(JSON.stringify({ not_a_token: 1 }), { status: 200 })) as unknown as typeof fetch,
  });
  assert.equal(out.kind, "unavailable");
  assert.match(String((out as { reason: string }).reason), /no access_token/);
});

test("IMDS answering non-200 is reported as an IMDS status, distinct from a metrics status", async () => {
  // The two hops fail for different reasons -- a broken identity vs a missing role -- and an
  // operator needs to know WHICH, because the remedies are unrelated.
  const out = await readCashActuals(cfg, {
    env: { [CASH_METRICS_RESOURCE_ENV]: "/subscriptions/s/x" },
    clock,
    fetchImpl: (async () => new Response("no identity", { status: 400 })) as unknown as typeof fetch,
  });
  assert.match(String((out as { reason: string }).reason), /IMDS returned HTTP 400/);
});

test("a hung endpoint becomes a TIMEOUT reason, never an open-ended wait", async () => {
  const out = await readCashActuals(cfg, {
    env: { [CASH_METRICS_RESOURCE_ENV]: "/subscriptions/s/x" },
    clock,
  ...
  assert.equal(out.kind, "unavailable");
  assert.match(String((out as { reason: string }).reason), /did not complete within/);
});

test("the cache bypass can be turned OFF, for a caller that knows its token is current", async () => {
  let seen = "";
  await readCashActuals(cfg, {
    env: { [CASH_METRICS_RESOURCE_ENV]: "/subscriptions/s/x" },
    clock,
    bypassTokenCache: false,
    fetchImpl: (async (url: string) => {
      if (String(url).startsWith("http://169.254.169.254")) { seen = String(url); return new Response(JSON.stringify({ access_token: "t" }), { status: 200 }); }
      return new Response(JSON.stringify(livePayload), { status: 200 });
    }) as unknown as typeof fetch,
  });
  assert.doesNotMatch(seen, /bypass_cache/, "opting out must really omit it");
  assert.equal(seen, IMDS_TOKEN_URL, "and otherwise use the documented endpoint unchanged");
});
