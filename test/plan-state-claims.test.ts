...
//   4. an examined set that is empty on the shipped side, or the not-shipped side, exits non-zero
//      as UNEXAMINED rather than rendering as a clean pass (empty-shipped-log.md /
//      empty-not-shipped.md);
...
test("plan-state-claims: an examined set empty on the not-shipped side exits non-zero as UNEXAMINED, never a clean result", () => {
