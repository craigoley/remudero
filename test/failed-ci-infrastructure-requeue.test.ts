...
test("only the complete upload-then-FinalizeArtifact intermediary 403 classifies as retryable infrastructure", () => {
  assert.equal(
    classifyCiInfrastructureFailure({ ciConclusion: "FAILURE", text: TRANSCRIPT }),
    ARTIFACT_FINALIZE_INTERMEDIARY_403,
  );
  for (const text of [
    "403 Forbidden",
    ...
    assert.equal(
      classifyCiInfrastructureFailure({ ciConclusion: "FAILURE", text }),
      undefined,
      `fail closed for ${JSON.stringify(text)}`,
    );
  }
  assert.equal(
    classifyCiInfrastructureFailure({ ciConclusion: "ERROR", text: TRANSCRIPT }),
    undefined,
