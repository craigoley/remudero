...
/**
 * Is THIS site's allocation cleaned up — a `trap … EXIT` or an `rm -f` that names ITS OWN
 * variable?
 *
 ...
 * own `rm -f` left the census GREEN. The falsifier caught it, not review.
 */
export function siteIsCleanedUp(script: string, site: MktempSite): boolean {
  const v = allocatedVar(site.text);
  if (v === undefined) return false;
  const ref = new RegExp(`\\$\\{?${v}\\b`);
  return hostScopedLines(script)
    .filter(({ n, text }) => n !== site.n && !/^\s*#/.test(text))
...
test("W1-T2915: no operator-run deploy script writes to or reads a literal /tmp path", () => {
  ...
  assert.ok(scripts.length > 5, `the census must SEE the corpus (found ${scripts.length} scripts)`);
  ...
  assert.deepEqual(
    ...
    "a fixed /tmp path in a script run on a sticky, world-writable, two-account host is " +
      ...
      offenders.join("\n") +
      `\n  THE REMEDY: TMP=$(mktemp "\${TMPDIR:-/tmp}/${REAPABLE_PREFIX}<name>.XXXXXX") with a ` +
      "trap installed BEFORE it, exactly as deploy/host-update.sh does.",
  );
});

test("W1-T2915: every host-scoped temp-dir mktemp carries the reapable prefix, and its script cleans up after itself", () => {
  ...
  for (const f of deployScripts()) {
    ...
    for (const s of tempDirSites) {
      ...
      if (!siteIsCleanedUp(src, s)) offenders.push(`  ${f}:${s.n}: nothing removes ${allocatedVar(s.text) ?? "this allocation"} — no rm -f naming it`);
    }
  }
