...
// W1-T3701. An Architect that exists only while the Claude subscription has headroom is a single
...
test("W1-T3701: the live table still validates — ranking a model does not move any seat", () => {
  assert.doesNotThrow(() => validateMounts(live(), {}));
});

test("W1-T3701: terra may hold the Architect seat, and the Judge seat, under squeeze", () => {
...
test("W1-T3701: luna may NOT hold the Architect seat — the invariant still bites", () => {
...
test("W1-T3701: sol is deliberately unranked, so it cannot be selected by accident", () => {
  const m = live();
...
test("W1-T3701: every ranked model resolves a capability, which the loader requires", () => {
  const m = live();
