# proof-queue-audit.ts forensics

The measured forensics, incident narratives and design arguments removed from
`src/lib/proof-queue-audit.ts` when its comments were compacted to the plain-language standard.
...
where the history mattered. Base revision: origin/main at 7ce3b525569bd7cfa98f5f858874e8667274d1dc;
the line numbers below are that revision's.

## Module header

### Base lines 1-98 — lib/proof-queue-audit.ts — W1-T1053.…
...
## ProofQueueAuditCause

### Base lines 109-115 — The three ways a proof…
...
### Base lines 122-125 — Every cause this module knows…
...
### Base lines 132-133 — All four causes, for a…
...
## ProofQueueAuditOffender.relocatedTo

### Base lines 147-151 — W1-T2280: set ONLY on a…
...
## ProofQueueAuditReport.relocated

### Base lines 166-172 — W1-T2280 note (vii): a `grep-path-absent`…
...
### Base lines 177-179 — The reviewer's OWN `resolveNameFilteredCandidates` (lib/review.ts),…
...
### Base lines 181-183 — Does a repo-relative path exist…
...
### Base lines 185-190 — W1-T2280: the merge-credited task ids,…
...
### Base lines 192-197 — Does `symbol` occur in `path`'s…
...
### Base lines 201-209 — Does this `grep:` criterion's symbol…
...
### Base lines 223-228 — Resolve every criterion's proof in…
...
### Base lines 244-248 — refused-parse: the proof DECLARED an…
...
### Base lines 256-259 — A literal test-file PATH (nameFiltered…
...
### Base lines 268-270 — Only `absent` is POSITIVE evidence…
...
### Base lines 277-278 — kind === "grep": args =…
...
## CreditedAmendmentFact

### Base lines 308-309 — One credited task's shard-file coverage…
...
## CreditedAmendmentFact.shardPath

### Base lines 312-315 — The repo-relative `plan/tasks.d/<id>-<slug>.yaml` this…
...
## CreditedAmendmentReport.unmeasurable

### Base lines 322-324 — Credited tasks declared inline in…
...
## CreditedAmendmentReport.flagged

### Base lines 326-329 — Task ids amended after their…
...
### Base lines 333-344 — W1-T2280: names a credited task…
...
## creditedAmendmentVisibility(opts.amendedSinceCredit)

### Base lines 348-354 — Was `shardPath` touched by a…
