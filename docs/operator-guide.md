...
| `rmd board [--repo <owner/repo> ...]` | Print what is open, and what is RED, across every fleet repository — the survey no verb offered, so the queues had to be read one `gh` call at a time (W1-T3685). Read-only: it merges nothing and reviews nothing. |
