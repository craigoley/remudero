...
| `rmd skill effectiveness <approved-skill>` | Reads the approved skill's measured selection and terminal-verdict cohort from the complete ledger union. |
| `rmd skill lifecycle <approved-skill>` | For a reviewed, horizon-complete RETIRE-CANDIDATE only, stages one evidence-fingerprinted retirement proposal. It cannot edit a skill; an explicit `rmd approve` opens the normal reviewable deletion PR. |
