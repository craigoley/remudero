...
| `rmd credit-audit [--plan <path>]` | Reports every open task the two credit surfaces disagree about: dispatch credits the `Remudero-Task:` trailer plus a `run-<id>-<digits>` head branch, while `lint-plan` credits the trailer plus the id in a non-filing commit subject. Neither is a superset, so a task can read implemented to one and unbuilt to the other. Proposes the `rmd correct` that would settle each; writes nothing. |
