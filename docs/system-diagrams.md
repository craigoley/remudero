...
flowchart TD
    ...
    SHARD["shard filed: plan/tasks.d/‹id›-‹slug›.yaml — lands through its own PR, same checks as any other. [operator gate] Note: the shard's stored status: field is decorative; merge-state is DERIVED from GitHub (src/lib/status.ts)"]
