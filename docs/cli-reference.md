...
usage:
  ...
  rmd next-task-id [--plan <path>] [--offline] [--no-reserve] [--audit] [--audit-age-days <days>] [--prefix <P> --repo <owner/name>]   # Atomically CLAIM the next free W1-T<n> task id. `--no-reserve` prints one without claiming it.
...
```
rmd next-task-id [--plan <path>] [--offline] [--no-reserve] [--audit] [--audit-age-days <days>] [--prefix <P> --repo <owner/name>]
```
