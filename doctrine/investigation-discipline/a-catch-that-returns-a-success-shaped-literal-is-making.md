- **A `catch` that returns a success-shaped literal is making a CLAIM: name the outcomes it
  collapses, and where two remedies differ, the answer is a third value the caller can see —
  never a comment asserting the collapse is fine. Reasoning vocabulary (fail-soft, degrade,
  treat as, unknown) does NOT track correctness; `loadLearningsCorpus` returns `[]` with
  comment `// no corpus directory yet`, reasoning-vocabulary-perfect and defective alike.
  diff-coverage is the gate for NEW conflations, forcing tests per arm and thus naming outcomes;
  it catches none of the 145 already on main, which is filed as W1-T1074.**
